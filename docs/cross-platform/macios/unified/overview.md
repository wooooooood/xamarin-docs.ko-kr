---
title: 통합 된 API 개요
description: Xamarin의 통합 된 API를 사용 하면 Mac, iOS 및 지원 32 / 64 비트 응용 프로그램을 같은 이진 파일 사이 코드를 공유할 수 있습니다.
ms.prod: xamarin
...
다양 한 개발자가 직면 하 게 기존 시스템 에서도 있는 문제가 해결 되었습니다 [메모리 관리](~/cross-platform/deploy-test/memory-perf-best-practices.md)합니다.

새 Refcount 클래식 API에도 사용할 수는 있지만 기본은 설정을 변경 하려면 사용자가 필요 하지 않습니다. 통합 api를에서는 기회의 기본값을 변경 하는 동시에 개발자가 모든 향상 된 기능을 제공 리팩터링 하 고 다시 해당 코드를 테스트 합니다.
