...
이 문서에서는 Objective-c Api를 바인딩하는 방법을 설명,이 대 한 표준.NET 메커니즘을 사용 해야 C Api만을 바인딩하는 경우 [P/Invoke 프레임 워크](http://mono-project.com/Dllimport)합니다.
...
[![](objective-c-libraries-images/00-sml.png "이 작업을 수행 Visual Studio가 Mac 용 iOS 라이브러리 바인딩 라이브러리 프로젝트 형식을 선택 하 여")](objective-c-libraries-images/00.png)
...
[![](objective-c-libraries-images/00vs-sml.png "iOS 바인딩 라이브러리 iOS")](objective-c-libraries-images/00vs.png)
...
자세한 내용은 API 파일 및 사용할 수 있는 특성의 형식에 대해서는 [API 정의 파일](~/cross-platform/macios/binding/objective-c-libraries.md) 아래 섹션.
...
이 차트는 파일 간의 관계를 보여 줍니다.
 ...
 [ ![](objective-c-libraries-images/screen-shot-2012-02-08-at-3.33.07-pm.png "이 차트는 파일 간의 관계를 보여 줍니다.")](objective-c-libraries-images/screen-shot-2012-02-08-at-3.33.07-pm.png)

API 정의 파일: 클래스, 열거형, 대리자 또는 구조체 포함 하지 않아야 하 고 네임 스페이스 및 인터페이스 정의 (인터페이스 포함 될 수 있는 모든 멤버)만 포함 됩니다. API 정의 파일에는 API를 생성 하는 데 사용 될 하는 계약을 단순히 되었습니다.
...
일반적으로 네이티브 라이브러리 "lib" 라는 단어로 시작 하 고 "포함" 확장명으로 끝납니다. 이 작업을 수행 하는 경우 Mac 용 Visual Studio 파일 두 개를 추가 합니다:는 `.a` 파일과 자동으로 채워지는 C# 파일 네이티브 라이브러리에 포함 하는 방법에 대 한 정보가 포함 된:

 [ ![](objective-c-libraries-images/screen-shot-2012-02-08-at-3.45.06-pm.png "일반적으로 네이티브 라이브러리 단어 라이브러리 글자와 확장.a 끝")](objective-c-libraries-images/screen-shot-2012-02-08-at-3.45.06-pm.png)
...
전체 LinkWith 특성을 사용 하는 방법에 대 한 자세한 내용은 우리의 [바인딩 형식 참조 가이드](~/cross-platform/macios/binding/binding-types-reference.md)합니다.
...
API 정의 파일 인터페이스의 수를 이루어져 있습니다. API 정의의 인터페이스를 클래스 선언에 사용할 수 및로 데코 레이트 되어야 합니다는 [[BaseType]](~/cross-platform/macios/binding/binding-types-reference.md) 특성을 클래스에 대 한 기본 클래스를 지정 합니다.
...
할 수 있는 가장 간단한 바인딩 메서드를 바인딩하는 합니다. 방금 C# 명명 규칙을 사용 하 여 인터페이스에서 메서드를 선언 하 고 사용 하 여 메서드를 데코레이팅하는 [[내보내기]](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다. [내보내기] 특성이 Xamarin.iOS 런타임에서 Objective-c 이름 사용 하 여 C# 이름을 링크입니다. 내보내기 특성의 매개 변수는 몇 가지 예 Objective-c 선택기의 이름:
...
위의 샘플에서 인스턴스 메서드를 바인딩할 수는 방법을 보여 줍니다. 사용 해야 정적 메서드에 바인딩하는 `[Static]` 다음과 같은 특성:
...
계약 인터페이스의 일부인 하며 인터페이스의 정적 포트 대 인스턴스 선언이 개념이 없습니다 이므로 특성을 사용 하 여 다시 한 번 필요 하기 때문에 이것이 필요 합니다. 바인딩을 통해 특정 메서드를 숨길 하려는 경우이 메서드를 데코레이팅 할 수 있습니다는 [[내부]](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다.

`btouch-native` 명령을 확인 합니다. null이 아니어야 하는 참조 매개 변수를 소개 합니다. 사용 하 여 특정 매개 변수에 대해 null 값을 허용 하려는 경우는 [[NullAllowed]](~/cross-platform/macios/binding/binding-types-reference.md) 특성에 다음과 같이 매개 변수:
...
참조 형식으로 내보낼 때의 `[Export]` 키워드 할당 의미 체계를 지정할 수도 있습니다. 이것은 데이터가 없는 유출 확인 하는 데 필요 합니다.

<a name="Binding_Properties" />

## <a name="binding-properties"></a>바인딩 속성

메서드와 마찬가지로 방금 Objective-c 속성 사용 하 여 바인딩됩니다는 [[내보내기]](~/cross-platform/macios/binding/binding-types-reference.md) 특성 및 C# 속성에 직접 매핑됩니다. 메서드를와 마찬가지로 속성으로 데코레이팅 될 수는 [[정적]](~/cross-platform/macios/binding/binding-types-reference.md) 및 [[내부]](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다.

사용 하는 경우는 `[Export]` 표지 btouch 네이티브 속성에는 특성에는 실제로 두 가지 방법 바인딩합니다: getter와 setter입니다. 내보낼가 제공 하는 이름을 **basename** 및 setter의 첫 글자를 설정 합니다. "설정" 이라는 단어 앞에 추가 하 여 계산 됩니다는 **basename** 대문자로 변환 하 고 수행 하는 선택기에는 인수입니다. 즉 `[Export ("label")]` 에 적용 된 속성에는 실제로 "label" 바인딩합니다 및 "setLabel:" Objective-c 메서드.

경우에 따라 Objective-c 속성 위에서 설명한 패턴을 준수 하지 않는 고 이름이 수동으로 덮어씁니다. 이러한 경우에 바인딩을 사용 하 여 생성 되는 방식을 제어할 수 있습니다는 `[Bind]` getter 또는 setter에 예를 들어 특성:
...
사용 하 여 정적 속성에 대 한 지원 외에도 `[Static]`를 사용 하 여 thread 정적 속성 데코레이팅 할 수 있습니다 [[IsThreadStatic]](~/cross-platform/macios/binding/binding-types-reference.md), 예:
...
메서드를 플래그가 지정 된 일부 매개 변수를 사용 하는 것 처럼 [[NullAllowed]](~/cross-platform/macios/binding/binding-types-reference.md)를 적용할 수 있습니다 [[NullAllowed]](~/cross-platform/macios/binding/binding-types-reference.md) 해당 null은 예를 들어 속성에 대 한 유효한 값을 나타내는 속성을에:
...
[[NullAllowed]](~/cross-platform/macios/binding/binding-types-reference.md) setter에서 직접 매개 변수를 지정할 수도 있습니다.
...
1. **바인딩 속성을 정적 이어야** -속성의 바인딩을 정의 하는 경우는 `Static` 특성을 사용 해야 합니다.
...
이 C#에 매핑되어 있는지 방법은 getter와 setter는 모두 기본 클래스에 추가 하지만와 setter에 플래그를 지정 하는 것을 `[NotImplemented]` 특성입니다.

를 변경할 수 있는 하위 클래스에 사용 된 `[Override]` 특성 속성을 속성의 부모 동작을 재정의 하 고 실제로에 합니다.
...
**btouch 네이티브** 도구 fours 생성자는 지정 된 클래스에 대 한 클래스에 자동으로 생성 됩니다 `Foo`를 생성 합니다.
...
API 디자인 문서의 섹션에 설명 된 대로 [모델 및 프로토콜에 논의](~/ios/internals/api-design/index.md), Xamarin.iOS로 플래그가 지정 된 클래스에 Objective-c 프로토콜을 매핑하는 [[모델]](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다. Objective C 대리자 클래스를 구현할 때 일반적으로 사용 됩니다.
...
에 대 한 정의에 선택적 메서드가 이므로 `UIAccelerometerDelegate` 즐길 수 있는 일은 없습니다. 프로토콜에 필요한 메서드를 추가 해야 하지만 [[추상]](~/cross-platform/macios/binding/binding-types-reference.md) 특성을 메서드에 합니다. 이렇게 하면 구현을 제공 하도록 사용자에 실제로 본문이 메서드에 대 한 시작 합니다.

일반적으로 프로토콜 메시지에 응답 하는 클래스에서 사용 됩니다. 일반적으로 이렇게 Objective C에는 프로토콜의 방법에 응답 하는 개체의 인스턴스를 "대리자" 속성에 할당 합니다.

Xamarin.iOS의 규칙은 모두의 Objective-c 느슨하게 지원 하기 위해 결합 된 스타일 지점의 인스턴스는 `NSObject` 와 할당할 수 있습니다는 대리자에도 노출 하 여 강력한 형식의 버전을 합니다. 이러한 이유로 일반적으로 "대리자" 속성을 강력한 형식 및 느슨하게 형식인 "WeakDelegate" 모두 제공 합니다. 일반적으로 내보내기, 포함 된 버전을 잘못 입력 한 바인딩한 및 사용 하 여는 [[줄 바꿈]](~/cross-platform/macios/binding/binding-types-reference.md) 특성을 강력한 형식의 버전을 제공 합니다.
...
포함 된 모든 정의 `[Protocol]` 특성은 실제로 크게 프로토콜을 사용 하는 방식을 개선 하는 세 가지 지원 클래스를 생성 합니다.
...
Objective C의 C#의 확장 메서드와 비슷합니다 개념상에서 새로운 메서드가 있는 클래스를 확장 하는 것이 같습니다. 사용할 수 있는 경우 다음이 방법 중 하나는 `BaseType` 특성으로 Objective-c 메시지 받는 사람으로 메서드를 나타내는 플래그입니다.
...
## <a name="binding-objective-c-argument-lists"></a>Objective C 인수 목록을 얻으려면 바인딩

Objective C variadic 인수가 지원 되며에 Zach Gris에 설명 된 다음과 같은 방법을 사용할 수 있습니다 [이 게시물](http://forums.monotouch.net/yaf_postst311_SOLVED-Binding-ObjectiveC-Argument-Lists.aspx)합니다.

Objective C 메시지는 다음과 같습니다.
...
파생 되지 않은 정적 클래스의 다양 한 필드를 배치 하려는 경우 `NSObject`를 사용할 수 있습니다는 `[Static]` 다음과 같이 클래스의 특성:
...
위의 예에서 데코레이팅 할 수 `void Perform (NSString mode);` 와 `[Internal]` 특성입니다. 이렇게 설정이 하면 **숨기기** 바인딩 소비자에서 상수 기반 API입니다.

그러나이 제한할는 더 좋은 API가 대체 사용으로 형식의 서브클래싱하는 `[Wrap]` 특성입니다. 이러한 생성 된 메서드는 아닙니다. `virtual`, 즉, 있습니다 won't 여부 될 수 있는 것을 무시할 수, 올바른 선택 합니다.

원래 표시 하는 대체 항목은 `NSString`-기반으로 정의 `[Protected]`합니다. 이렇게 하면 필요한 경우 사용할 서브클래싱을 wrap'ed 버전 계속 작동 되며 재정의 된 메서드를 호출 합니다.


## <a name="binding-nsvalue-nsnumber-and-nsstring-to-a-better-type"></a>NSValue NSNumber 및 NSString 더 나은 형식에 바인딩

[[BindAs]](~/cross-platform/macios/binding/binding-types-reference.md) 특성을 사용 하면 바인딩 `NSNumber`, `NSValue` 및 `NSString`(열거형)를 보다 정확 하 게 C# 형식으로. 특성 보다 효율적이 고, 더 정확 하 게 만드는 데 사용할 수는 네이티브 API 통해.NET API입니다.
...
참조 하십시오 [[BindAs] 설명서](~/cross-platform/macios/binding/binding-types-reference.md) 참조 하기 위해 변환 유형을 지원 합니다.
...
Xamarin.iOS 바인딩 생성기 알림 바인딩하는 개발자를 위한 지원을 제공 합니다. 설정 하면이 위해는 [[알림]](~/cross-platform/macios/binding/binding-types-reference.md) 되었습니다 되었습니다 속성에 대 한 특성으로 태그가 지정 된는 [[Field]](~/cross-platform/macios/binding/binding-types-reference.md) 속성 (수 공개 또는 개인).
...
사용할 수 있는 바인딩하려면는 `[Category]` 인터페이스 정의의 특성입니다.  때 범주를 사용 하 여 특성의 의미는 `[BaseType]` 특성이 기본 클래스를 확장 하, 확장 형식이 되도록 지정 하는 데 사용 되 고에서 변경 합니다.
...
사용할 수 있지만 `Category` 특성도 선언의이 장식 스타일에 대 한 수도 추가 하면 해당 클래스 정의에 모두 있습니다.  동일한 작업을 수행할 것 둘 다에 해당 합니다.
...
라는 메서드를 선언 하는 위의 설명 "*enumerateObjectsUsingBlock:*" 라는 하나의 인수를 사용 하는 *블록*합니다. 이 블록 현재 환경 ("this"이 포인터를 지역 변수 및 매개 변수에 대 한 액세스) 캡처에 대 한 지원을 있다는 점에서 C# 무명 메서드와 비슷합니다. 위의 메서드 `NSSet` 두 개의 매개 변수가 있는 블록을 호출는 `NSObject` ("id obj" 부분) 및 부울에 대 한 포인터 (의 "BOOL * 중지") 부분.
...
사용할 수는 `[Async]` void를 반환 하 고 마지막 인수가 콜백 메서드에 대 한 특성입니다.  이 메서드를 적용 하면 바인딩 생성기에서 접미사와 해당 메서드의 버전을 생성 `Async`합니다.  콜백 매개 변수를 사용 하는 경우 반환 값 됩니다는 `Task`, 콜백 매개 변수를 사용 하는 경우 결과 됩니다는 `Task<T>`합니다.  콜백이 여러 매개 변수를 사용 하는 경우 설정 해야는 `ResultType` 또는 `ResultTypeName` 생성 된 형식의 모든 속성을 포함 하는 원하는 이름을 지정할 수 있습니다.
...
## <a name="surfacing-strong-types-for-weak-nsdictionary-parameters"></a>약한 NSDictionary 매개 변수에 대 한 강력한 형식에서의 표시가

Objective C API의 여러 위치에서 매개 변수가 전달 같이 약하게 형식화 `NSDictionary` 특정 키와 값을 있지만 이러한 Api는 오류 (잘못 된 키를 전달 하 고 경고 없이 받을 수; 잘못 된 값을 전달 하 고 경고 없이 얻을 수 있습니다) 발생 하기 쉬운 하면 당황 스 러 및 사용 하려면 여러 ½ à ´ ï 가능한 키 이름 및 값을 조회 하는 설명서를 필요에 따라 합니다.

솔루션의 API 및 원리를 자세히 파악할수록 강력한 형식의 버전을 제공 하는 강력한 형식의 버전 매핑합니다 다양 한 기본 키와 값을 제공 하는 것입니다.

따라서 예를 들어 Objective-c API 허용 하는 경우는 `NSDictionary` 사용 "XyzVolumeKey" 키를 사용 하도록 문서화 되는 `NSNumber` 볼륨 값이 0.0에서 1.0 및 문자열을 사용 하는 "XyzCaptionKey" 사용자가 좋은 원할 다음과 같은 API:
...
사용자가 API는 덮어쓸 수 필요가 없는 경우 숨길 수 있습니다 안전 하 게 NSDictionary 기반 API를 사용 하 여는 [내부](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다.

사용 하 여 볼 수 있듯이 `[Wrap]` 특성을 새 API 진입점을 노출 하 고 강력한 형식의 XyzOptions 클래스를 사용 하 여 노출 했습니다.
래퍼 메서드가 null을 전달할 수도 있습니다.

언급 되지 않은 것 중 하나에 이제는 `XyzOptionsKeys` 값에서 제공 합니다.  일반적으로 API 화면 XyzOptionsKeys와 같은 정적 클래스에 다음과 같이 키를 그룹화 합니다.
...
강력한 형식의 사전을 만들려는 API의 인터페이스를 소개 하 고으로 데코레이팅는 [StrongDictionary](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다.  이 속성을 확인할 생성기를 만들도록 클래스에서 파생 되는 사용자 인터페이스와 동일한 이름을 가진 `DictionaryContainer` 것에 대 한 강력한 형식의 접근자를 제공 합니다.

`StrongDictionary` 특성은 하나의 매개 변수가 사전 키를 포함 하는 정적 클래스의 이름입니다.  다음 인터페이스의 각 속성에는 강력한 형식의 접근자 될 것입니다.  기본적으로 코드는 고 접근자를 만드는 정적 클래스에 "키" 접미사와 함께 속성의 이름을 사용 합니다.
...
참조 하는 데 필요한 경우에 프로그램 `XyzOption` 멤버 다른 필드 (즉 이름이 아니라 접미사를 사용 하 여 속성의 `Key`)를 사용 하 여 속성 데코레이팅 할 수 있습니다는 `Export` 사용 하려는 이름으로 특성입니다.
...
## <a name="simple-types"></a>@FSHO2@단순 형식
...
아이디어는 IDE에서이 사용자를 추측 또는 배열에 포함 된 개체의 실제 형식이 확인 하려면 설명서를 조회 하지 않고 실제 형식이 적절 한 코드 완성 기능을 제공 하면 강력한 형식의 C# 배열을 사용 하는 것입니다.
...
선택기는 Objective-c API에 특별 한 형식 "SEL"로 나타납니다. 선택기를 바인딩하는 경우에 형식을 매핑합니다 `ObjCRuntime.Selector`합니다.  일반적으로 선택기 개체, 대상 개체 및 선택 기가 포함 된 대상 개체에서를 호출 하는 API에서 노출 됩니다. 둘 다 기본적으로 제공 하는 C# 대리자에 해당:에서 메서드를 호출 하는 개체와 호출할 메서드를 모두 캡슐화 하는 것입니다.
...
바인딩이 더 좋은 C# 개발자를 하려면 일반적으로 입력할 것임 사용 하는 메서드는 `NSAction` C# 대리자와 람다 대신 사용할 수 있는 매개 변수는 `Target+Selector`합니다. 이 작업을 수행 하는 일반적으로 숨기면 "SetTarget" 메서드는 "내부" 특성으로 표시 하 여 하 고 다음과 같은 새로운 도우미 메서드를 노출 합니다.
...
일부 드문 경우에는 API는 C와 같은 문자열 노출 될 수 있습니다 (`char *`) Objective C 문자열 대신 (`NSString *`). 이 경우의 매개 변수를 주석을 달 수는 [ `[PlainString]` ](~/cross-platform/macios/binding/binding-types-reference.md) 특성입니다.
...
사용 하는 경우는 `[Export]` 특성을 전달 하는 호출 된 메서드가 유지 되어야 하는 데이터, 예를 들어 두 번째 매개 변수로 전달 하 여 인수 의미 체계를 지정할 수 있습니다.
...
사용할 수는 [[내부]](~/cross-platform/macios/binding/binding-types-reference.md) 공용 API에서 메서드를 숨기려면 특성입니다. 노출 된 API는 너무 낮은이 방법에 따라 별도 파일에서 높은 수준의 구현을 제공 하는 경우에서 이렇게 할 수 있습니다.
...
-  호스트 클래스에서를 추가 하면 `[BaseType]` 노출 대리자 및 C#에 이름을 작동 하는 형식을 선언 합니다. 위의 예에는 것은 "typeof (MyClassDelegate)" 및 "WeakDelegate" 각각입니다.
...
`EventArgs` 의 이름을 지정 하는 데 사용 되는 `EventArgs` 클래스를 생성 합니다. 서명 당 하나를 사용 해야 (이 예제는 `EventArgs` 형식 nint의 "와" 속성이 포함 됩니다).
...
콜백 이벤트 호출에서와 마찬가지로, 차이점은 여러 잠재적인 구독자 대신 (예를 들어 여러 방법을에 후크 할 수 "Clicked" 이벤트 또는 "다운로드 완료 되었습니다" 이벤트) 콜백을 한 구독자를 하나만 사용할 수 있습니다.

프로세스는 동일, 유일한 차이점은 생성 되는 EventArgs 클래스의 이름, 노출 하는 대신는 EventArgs 실제로 사용 하는 결과 C# 대리자 이름이 이름을 지정 합니다.

대리자 클래스의 메서드는 값을 반환 하는 경우 바인딩 생성기는 부모 클래스 대신 이벤트의에서 대리자 메서드에이 매핑됩니다. 이러한 경우에 반환 되어야 하는 메서드에서 사용자를 연결 하지 않는 경우 대리자에 기본값을 제공 해야 합니다. 이렇게 하면 사용 하 여 `[DefaultValue]` 또는 `[DefaultValueFromArgument]` 특성입니다.

DefaultValue는 반환 값을 하드 코드 하는 동안 `[DefaultValueFromArgument]` 입력된 인수를 반환할 지정 하는 데 사용 됩니다.
...
Xamarin.iOS 라이브러리를 연결 하는 방법을 알려 주어 야, 이렇게 사용 하 여 새 라이브러리와 연결 하는 방법을 지정 하는 몇 가지 추가 빌드 인수를 사용 하 여 mtouch 명령을 호출 하 여 빌드 구성을 변경 하 여 하나는 "-gcc_flags" 옵션 뒤 다음과 같이 프로그램에 필요한 모든 추가 라이브러리를 포함 하는 따옴표 붙은 문자열이:
...
궁금할 것, 필요 하십니까 "force_load" 명령 및 이유는-ObjC 플래그의 코드를 컴파일하고 하지만, 필요한 범주 (링커/컴파일러 데드 코드가 elimination 잘라냅니다)를 지 원하는 데 필요한 메타 데이터를 유지 하지 않습니다. Xamarin.iOS에 로드할 수 있습니다.
...
에 메시지를 표시 하 고이 처리 하는 전통적인 방법 "완료"가 이벤트를 생성 하는 클래스를 설치한 경우 예제는 다음과 같습니다.
...
를 이용 하려면이 클래스에 설정 이벤트 속성이 있어야 합니다.는 `[BaseType]` 선언도는 `KeepUntilRef` 변수 개체에는 다음과 같이 해당 작업을 완료 되었을 때 호출 되는 메서드의 이름으로 설정:
...
Xamarin.iOS v3.2 기준으로 표시 하는 프로토콜에서 상속 지원는 `[Model]` 속성입니다. 와 같이 이러한 특정 API 패턴에 유용 `MapKit` 여기서는 `MKOverlay` 프로토콜에서 상속 되는 `MKAnnotation` , 프로토콜 및 다양 한 클래스에서 상속 하 여 적용 됩니다 `NSObject`합니다.
