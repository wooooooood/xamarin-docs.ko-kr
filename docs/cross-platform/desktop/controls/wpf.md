...
- Xamarin.ios는 [XAML 2009 사양을](/dotnet/framework/xaml-services/xaml-2009-language-features/)지원 합니다. 이렇게 하면 `string`s, `int`s 등의 데이터를 보다 쉽게 정의할 수 있을 뿐만 아니라 제네릭 형식을 정의 하 고 생성자에 인수를 전달할 수 있습니다.

- 현재 WPF와 `XamlReader`같은 XAML을 동적으로 로드할 수 있는 방법은 없습니다. 그러나 [NuGet 패키지](https://www.nuget.org/packages/Xamarin.Forms.Dynamic/) 를 사용 하 여 동일한 기본 기능을 얻을 수 있습니다.
...
또한 XAML 2009 사양 `{x:Reference}` `{TemplateBinding}` 및 xamarin.ios에서 지원 되는의 `ControlTemplate` 특수화 된 버전에 사용 되는 태그 확장을 포함 합니다.

> [!WARNING]
> 동일한 이름을 가진 경우에도 지원되지않습니다.`ControlTemplate`

Xamarin.ios는 사용자 지정 태그 확장도 지원 하지만 구현은 약간 다릅니다. WPF에서는 추상 기본 클래스에서 `MarkupExtension` 파생 해야 합니다. Xamarin.ios에서 인터페이스로 `IMarkupExtension` 대체 되거나 `IMarkupExtension<T>` 유연성이 더 높습니다.

WPF와 마찬가지로, 단일 필수 메서드 `ProvideValue` 는 태그 확장에서 값을 반환 하는 메서드입니다.
...
바인딩 엔진을 사용 하는 프로세스는 WPF의 경우와 동일 합니다. 소스 개체에 연결 된 `Binding` 개체 (.net 형식) 및 선택적 속성 값 (생략 경우 WPF와 마찬가지로 소스 개체를 속성 자체로 처리 하는 경우)을 만들어 코드 숨김으로 활용할 수 있습니다. 그런 다음 any any `SetBinding` `BindableObject` 를 사용 하 여 바인딩을에 `BindableProperty`연결할 수 있습니다.

또는를 사용 하 여 `BindingExtension`XAML에서 바인딩 관계를 정의할 수 있습니다. WPF의 확장과 동일한 기본 값을 갖습니다.
...
`RelativeSource` 바인딩이 지원 되지 않습니다. WPF에서 XAML로 정의 된 다른 시각적 요소에 바인딩할 수 있습니다. Xamarin.ios에서 `{x:Reference}` 태그 확장을 사용 하 여이 기능을 구현할 수 있습니다. 예를 들어, 텍스트 속성을 포함 하는 "otherControl" 라는 컨트롤이 있는 경우 다음과 같이 바인딩할 수 있습니다.
...
`{RelativeSource Self}` 기능에 동일한 기능을 사용할 수 있습니다. 그러나 형식 (`{RelativeSource FindAncestor}`)을 기준으로 상위 항목을 찾는 것은 지원 되지 않습니다.

#### <a name="binding-context"></a>바인딩 컨텍스트

WPF에서 기본 바인딩 소스를 다시 `DataContext` 지정 하는 속성 값을 정의할 수 있습니다. 바인딩의 소스가 정의 되어 있지 않으면이 속성 값이 사용 됩니다. 값은 시각적 트리 아래로 상속 되므로 상위 수준에서 정의 된 다음 자식에서 사용할 수 있습니다.
...
WPF에는를 사용 `RoutedCommand` 하는 기본가 포함 되어 있습니다. Xamarin.ios에는 인터페이스 정의 외에도 기본 제공 되는 `ICommand` 명령 지원이 없습니다. 다양 한 MVVM 프레임 워크를 포함 하 여 MVVM를 구현 하는 데 필요한 기본 클래스를 추가할 수 있습니다.
...
또한 `SynchronziationContext` 두 환경 모두 및 `async` / 를지원하여적절한스레드마샬링을수행합니다.`await` WPF에는 `Dispatcher` 모든 시각적 요소에 대 한 클래스가 포함 되어 있습니다. Xamarin에 `Device.BeginInvokeOnMainThread` 는 플랫폼 간 코딩에 대 한 `SynchronizationContext` 기본 설정 이지만 사용할 수 있는 정적 메서드가 있습니다.

- Xamarin.ios에는 컬렉션 변경 `ObservableCollection<T>` 알림을 지 원하는가 포함 되어 있습니다.
- 를 사용 `BindingBase.EnableCollectionSynchronization` 하 여 컬렉션에 대 한 크로스 스레드 업데이트를 사용 하도록 설정할 수 있습니다. API는 WPF 변형과 약간 다릅니다. 자세한 내용은 [문서를 참조](xref:Xamarin.Forms.BindingBase.EnableCollectionSynchronization*)하세요.

## <a name="data-templates"></a>데이터 템플릿

데이터 템플릿은 `ListView` 행 (셀)의 렌더링을 사용자 지정 하기 위해 xamarin.ios에서 지원 됩니다. 콘텐츠 지향 컨트롤에를 `DataTemplate`사용할 수 있는 WPF와는 달리, xamarin.ios는 현재에 `ListView`만 사용 됩니다. 템플릿 정의는 인라인 ( `ItemTemplate` 속성에 할당 됨) 또는 `ResourceDictionary`의 리소스로 정의 될 수 있습니다.

또한 WPF에 상응 하는 것 만큼 유연 하지 않습니다.

1. 의 `DataTemplate` 루트 요소는 `ViewCell` _항상_ 개체 여야 합니다.
2. 데이터 트리거는 데이터 템플릿에서 완전히 지원 되지만 트리거와 연결 된 속성의 유형을 `DataType` 나타내는 속성을 포함 해야 합니다.
3. `DataTemplateSelector`도 지원 되지만에서 `DataTemplate` 파생 되므로 `ItemTemplate` 속성 (WPF `ItemTemplateSelector` 에서는 vs)에 직접 할당 됩니다.

## <a name="itemscontrol"></a>ItemsControl

Xamarin.ios에는 `ItemsControl` 에 해당 하는 기본 제공 기능이 없지만 [여기에서 사용할 수 있는 xamarin.ios에 대 한 사용자 지정](https://github.com/xamarinhq/xamu-infrastructure/blob/master/src/XamU.Infrastructure/Controls/ItemsControl.cs)이 있습니다.

## <a name="user-controls"></a>사용자 정의 컨트롤

WPF `UserControl`에서는 관련 된 동작을 포함 하는 UI 섹션을 제공 하는 데 사용 됩니다. Xamarin.ios에서는 동일한 용도로를 `ContentView` 사용 합니다. 둘 다 XAML에서 바인딩과 포함을 지원 합니다.

## <a name="navigation"></a>탐색

WPF에는 "브라우저 `NavigationService` 와 유사한" 탐색 기능을 제공 하는 데 사용할 수 있는 거의 사용 되지 않는이 포함 되어 있습니다. 대부분의 앱은이에 대해 알지 못했지만 대신 다른 `Window` 요소 또는 창의 다른 섹션을 사용 하 여 데이터를 표시 했습니다.
...
는 `NavigationPage` 가장 일반적인 방법 이며 모든 페이지 `Navigation` 에는 탐색 스택에서 페이지를 밀거나 끄는 데 사용할 수 있는 속성이 있습니다. 이는 WPF에서 `NavigationService` 발견 된와 가장 비슷한입니다.
