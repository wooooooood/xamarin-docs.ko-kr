# [플랫폼 간](index.yml)
## [시작](get-started/index.md)
### [Requirements](get-started/requirements.md)
### [설치](get-started/installation/index.md)
#### [Windows에서 Visual Studio에 Xamarin 설치](get-started/installation/windows.md)
#### [Mac용 Visual Studio 설치](/visualstudio/mac/installation/)
#### [Xamarin 방화벽 구성 지침](get-started/installation/firewall.md)
#### [Xamarin 제거](get-started/installation/uninstalling-xamarin.md)
### [모바일 개발 소개](get-started/introduction-to-mobile-development.md)
### [모바일 SDLC 소개](get-started/introduction-to-mobile-sdlc.md)
## [응용 프로그램 기본 사항](app-fundamentals/index.md)
### [액세스 가능성](app-fundamentals/accessibility.md)
### [플랫폼 간 응용 프로그램 빌드](app-fundamentals/building-cross-platform-applications/index.md)
#### [개요](app-fundamentals/building-cross-platform-applications/part-0-overview.md)
#### [1부 - Xamarin Mobile Platform 이해](app-fundamentals/building-cross-platform-applications/part-1-understanding-the-xamarin-mobile-platform.md)
#### [2부 - 아키텍처](app-fundamentals/building-cross-platform-applications/part-2-architecture.md)
#### [3부 - Xamarin 플랫폼 간 솔루션 설정](app-fundamentals/building-cross-platform-applications/part-3-setting-up-a-xamarin-cross-platform-solution.md)
#### [4부 - 다중 플랫폼 처리](app-fundamentals/building-cross-platform-applications/part-4-platform-divergence-abstraction-divergent-implementation.md)
#### [5부 - 실제 코드 공유 전략](app-fundamentals/building-cross-platform-applications/part-5-practical-code-sharing-strategies.md)
#### [6부 - 테스트 및 App Store 승인](app-fundamentals/building-cross-platform-applications/part-6-testing-and-app-store-approvals.md)
#### [사례 연구: Tasky](app-fundamentals/building-cross-platform-applications/case-study-tasky.md)
### [코드 공유 옵션](app-fundamentals/code-sharing.md)
### [이식 가능한 클래스 라이브러리](app-fundamentals/pcl.md)
### [공유 프로젝트](app-fundamentals/shared-projects.md)
### [.NET Standard](app-fundamentals/net-standard.md)
### [NuGet 패키지(자동화됨)](app-fundamentals/nuget-multiplatform-libraries/index.md)
#### [기존 라이브러리 프로젝트](app-fundamentals/nuget-multiplatform-libraries/existing-library.md)
#### [새 다중 플랫폼 라이브러리](app-fundamentals/nuget-multiplatform-libraries/single-codebase.md)
#### [새 플랫폼별 라이브러리](app-fundamentals/nuget-multiplatform-libraries/platform-specific.md)
#### [메타데이터 가이드](app-fundamentals/nuget-multiplatform-libraries/metadata.md)
### [NuGet 패키지(수동)](app-fundamentals/nuget-manual.md)
### [지역화](app-fundamentals/localization.md)
### [TLS(전송 계층 보안)](app-fundamentals/transport-layer-security.md)
## [플랫폼 기능](platform/index.md)
### [비동기 지원 개요](platform/async.md)
### [C# 6 언어 기능](platform/csharp-six.md)
### [F#](platform/fsharp/index.md)
#### [시작](platform/fsharp/overview.md)
#### [F# 샘플](platform/fsharp/samples.md)
### [Visual Basic.NET](platform/visual-basic/index.md)
#### [Xamarin iOS 및 Android](platform/visual-basic/native-apps.md)
#### [Xamarin.Forms](platform/visual-basic/xamarin-forms.md)
### [Razor HTML 템플릿](platform/razor-html-templates/index.md)
## [데이터 및 클라우드 서비스](data-cloud/index.md)
### [연결된 서비스](data-cloud/connected-services.md)
### [Azure Active Directory](data-cloud/active-directory/index.md)
#### [시작](data-cloud/active-directory/get-started/index.md)
##### [1단계. 레지스터](data-cloud/active-directory/get-started/register.md)
##### [2단계. 구성](data-cloud/active-directory/get-started/configure.md)
#### [Graph API에 액세스](data-cloud/active-directory/graph.md)
### [Azure Mobile Apps](data-cloud/mobile-apps.md)
### [웹 서비스 소개](data-cloud/web-services/index.md)
#### [해결 방법 - WCF 작업](data-cloud/web-services/walkthrough-working-with-wcf.md)

## [배포 및 테스트](deploy-test/index.md)
### [사용자 지정 링커 구성](deploy-test/linker.md)
### [성능](deploy-test/memory-perf-best-practices.md)
### [NuGet을 사용하여 NUnit 2.6.4 설치](deploy-test/installing-nunit-using-nuget.md)
### [다중 프로세스 디버깅](deploy-test/multi-process-debugging.md)
## [Apple 플랫폼](macios/index.md)
### [Apple 계정 관리](macios/apple-account-management.md)
### [Unified API](macios/unified/index.md)
#### [개요](macios/unified/overview.md)
#### [기존 앱 업데이트](macios/unified/updating-apps.md)
#### [기존 iOS 앱 업데이트](macios/unified/updating-ios-apps.md)
#### [기존 Mac 앱 업데이트](macios/unified/updating-mac-apps.md)
#### [기존 Xamarin.Forms 앱 업데이트](macios/unified/updating-xamarin-forms-apps.md)
#### [바인딩을 Unified API로 마이그레이션](macios/unified/update-binding.md)
#### [코드를 Unified API로 업데이트하는 팁](macios/unified/updating-tips.md)
### [Objective-C 바인딩](macios/binding/index.md)
#### [개요](macios/binding/overview.md)
#### [Objective-C 라이브러리 바인딩](macios/binding/objective-c-libraries.md)
#### [바인딩 유형 참조](macios/binding/binding-types-reference.md)
#### [Objective Sharpie](macios/binding/objective-sharpie/index.md)
##### [시작](macios/binding/objective-sharpie/get-started.md)
##### [도구 및 명령](macios/binding/objective-sharpie/tools.md)
##### [기능](macios/binding/objective-sharpie/platform/index.md)
###### [ApiDefinitions 및 StructsAndEnums](macios/binding/objective-sharpie/platform/apidefinitions-structsandenums.md)
###### [네이티브 프레임워크 바인딩](macios/binding/objective-sharpie/platform/native-frameworks.md)
###### [특성 확인](macios/binding/objective-sharpie/platform/verify.md)
##### [예제](macios/binding/objective-sharpie/examples/index.md)
###### [Xcode](macios/binding/objective-sharpie/examples/xcode.md)
###### [CocoaPod](macios/binding/objective-sharpie/examples/cocoapod.md)
###### [고급(수동)](macios/binding/objective-sharpie/examples/advanced.md)
##### [릴리스 기록](macios/binding/objective-sharpie/releases.md)
#### [문제 해결](macios/binding/troubleshooting.md)
### [네이티브 참조](macios/native-references.md)
### [네이티브 형식](macios/nativetypes.md)
### [32 및 64비트 플랫폼 고려 사항](macios/32-and-64/index.md)
#### [64비트로 Xamarin.Mac 통합 앱 업데이트](macios/32-and-64/mac-64-bit.md)
### [플랫폼 간 앱에서의 네이티브 형식 작업](macios/native-types-cross-platform.md)
### [HttpClient 스택 및 SSL/TLS 구현 선택기](macios/http-stack.md)
## [내부 요소](internals/index.md)
### [사용 가능한 어셈블리](internals/available-assemblies.md)
## [문제 해결](troubleshooting/index.md)
### [질문과 대답](troubleshooting/questions/index.md)
#### [Xamarin에서 Visual Studio 2017 릴리스 후보를 사용할 수 있나요?](troubleshooting/questions/visualstudio-2017-rc.md)
#### [PCL에서 지원되는 라이브러리를 어떻게 볼 수 있나요?](troubleshooting/questions/pcl-support-libraries.md)
#### [PCL 리플렉션 API](troubleshooting/questions/pcl-reflection.md)
#### [PCL 사례 연구: Microsoft TPL 데이터 흐름 NuGet 패키지용 System.Diagnostics.Tracing과 관련된 문제를 해결하려면 어떻게 해야 하나요?](troubleshooting/questions/pcl-case-study.md)
#### [NuGet을 업데이트하려면 어떻게 해야 하나요?](troubleshooting/questions/nuget-update.md)
#### [NuGet 패키지를 다운그레이드하려면 어떻게 해야 하나요?](troubleshooting/questions/nuget-package-downgrade.md)
#### [Nuget 패키지를 업데이트한 후 패키지 오류 없음](troubleshooting/questions/nuget-packages-missing.md)
#### [Google Play 서비스 구성 요소 및 NuGet 통합](troubleshooting/questions/gps-components-nuget.md)
#### [컴퓨터에 구성 요소가 저장된 위치는 어디인가요?](troubleshooting/questions/component-storage.md)
#### [버전 정보 및 로그는 어디에서 확인할 수 있나요?](troubleshooting/questions/version-logs.md)
#### [버그 보고서를 언제 어떻게 제출해야 하나요?](troubleshooting/questions/howto-file-bug.md)
#### [Xamarin에서 Jenkins가 지원되지 않는 이유는 무엇인가요?](troubleshooting/questions/xamarin-jenkins.md)
#### [디버거에 필요한 프로젝트 설정은 무엇인가요?](troubleshooting/questions/debugger-settings.md)
### [구성 요소 참조를 NuGet으로 업데이트](troubleshooting/component-nuget.md)
### [지원 옵션](troubleshooting/support-options.md)
### [레거시 라이선스 질문](troubleshooting/legacy-licenses/index.md)
#### [Xamarin 플랫폼 팀 관리](troubleshooting/legacy-licenses/team-management.md)
#### [내 계정의 전자 메일 주소를 변경하려면 어떻게 해야 하나요?](troubleshooting/legacy-licenses/change-email.md)
#### [Xamarin 라이선스를 수동으로 다시 동기화하려면 어떻게 해야 하나요?](troubleshooting/legacy-licenses/resync-licenses.md)
#### [일부 특정 라이선스 오류](troubleshooting/legacy-licenses/licensing-errors.md)
#### [Visual Studio 또는 Mac용 Visual Studio에서 Xamarin에 로그인할 수 없는 이유는 무엇인가요?](troubleshooting/legacy-licenses/login-troubleshooting.md)
## [샘플](samples/index.yml)
