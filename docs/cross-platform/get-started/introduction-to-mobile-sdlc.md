...
1.   **개념화 및 계획** – 하드웨어와 기능은 장치마다 다를 수 있으므로 특정 기능을 사용하는 애플리케이션은 특정 장치에서 제대로 작동하지 않을 수 있습니다. 예를 들어, 모든 장치에 카메라가 있는 것은 아니므로 화상 채팅 애플리케이션을 제작하는 경우 일부 장치는 동영상을 재생할 수는 있지만 촬영할 수는 없습니다.
