...
- 테스트하고 디버깅할 Xamarin 모바일 응용 프로그램을 USB를 통해 실제 장치에 배포할 수 있습니다(Apple Watch 앱은 연결된 iPhone에 먼저 배포됨).
...
테스트하고 디버깅할 Xamarin 모바일 응용 프로그램을 USB 또는 무선으로 실제 장치에 배포할 수 있습니다(iOS 장치는 Visual Studio를 실행 중인 컴퓨터가 아니라, Mac 컴퓨터에 연결해야 함).
