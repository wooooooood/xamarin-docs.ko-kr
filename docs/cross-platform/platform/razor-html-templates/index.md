...
Razor 템플릿 엔진 express 레이아웃 및 CSS 스타일 시트 및 Javascript를 쉽게 통합할 수 있도록와 C# # 표준 HTML 구문을 확장 합니다. 템플릿에서 모델 클래스, 사용자 정의 형식일 수 있습니다 및 속성을 가진 서식 파일에서 직접 액세스할 수 있습니다를 참조할 수 있습니다. 주요 이점 중 하나는 HTML 및 C# # 구문 쉽게 혼합 하는 기능입니다.
...
다음 C# #를 다음 코드로 최종 HTML 출력을 생성할 수 있습니다. Note 문자열 "Hello World" 렌더링 된 템플릿 출력에 통합 됩니다는 모델을 지정 했습니다.
...
사용 하 여 코드를 묶어 복잡 한 한 줄 C# 식 (예: 연령 서식)를 작성할 수 있습니다 `@()`합니다.
...
서식 파일에는 Razor 포함 **.cshtml** 에 HTML/Javascript와 C# 간의 데이터 통신을 위해 코드를 기록한 사전 파일. 이렇게 하면 수 하 하지 않는 모델에서 읽기 전용 데이터를 표시 하지만 HTML의 사용자 입력을 허용 전달 정교한 하이브리드 앱 처리 또는 저장에 대 한 C# 코드를 다시 빌드입니다.
...
C# #에 콜백할 수 렌더링된 웹 보기에서 통신 웹 보기에 대 한 URL을 설정 하 고 다음 웹 보기를 다시 로드 하지 않고 네이티브 요청을 처리 하는 C# #의 요청을 차단 하 여 수행 됩니다.
...
이를 만든 사용자 지정 체계를 사용 하 여 URL 웹 보기를 탐색 하려고 (`hybrid:`)
...
첫째, 웹 보기를 로드 하려고 하는 URL을 확인 했습니다 경우에 사용자 지정 체계 문자로 시작 하지 않습니다 (`hybrid:`), 정상적으로 탐색을 허용 합니다.

이 사용자 지정 URL 체계에 취급 하는 모든 URL의 체계 사이 및 "?" 으로 (이 경우 "UpdateLabel")에서 처리 해야 할 메서드 이름입니다. 쿼리 문자열의 모든 개체에서 메서드 호출에 매개 변수로 처리 됩니다.
...
이 샘플에서 UpdateLabel 않습니다 textbox 매개 변수에서 최소한의 문자열 조작 (앞에 추가 "C# # 라는 '" 문자열에), 웹 보기를 다시 호출 합니다.

우리의 URL 처리의 끝에서는 중단 탐색 하 여 웹 보기는 사용자 지정 URL을 탐색 완료 하려고 시도 하지 않습니다.
