...
공유 코드에서 UI 스레드 호출을 수행 합니다 [IDispatchOnUIThread 예제](https://www.slideshare.net/follesoe/cross-platform-mobile-apps-using-net) (인용 [ @follesoe ](https://twitter.com/follesoe)). 선언 하 고 프로그램을 `IDispatchOnUIThread` 공유 코드에 인터페이스 및 다음 다음과 같이 플랫폼 관련 클래스를 구현 합니다.
