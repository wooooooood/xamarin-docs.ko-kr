...
* 최대 세부 정보 표시를 사용 하는 전체 빌드 로그 (예: **추가 mmp 인수**에 `-v -v -v -v`)
...
#### <a name="mm0003-application-name-0exe-conflicts-with-an-sdk-or-product-assembly-dll-name"></a>MM0003: 응용 프로그램 이름 '{0}'이 (가) SDK 또는 제품 어셈블리 (.dll) 이름과 충돌 합니다.
...
#### <a name="mm0008-you-should-provide-one-root-assembly-only-found-0-assemblies-1"></a>MM0008: 하나의 루트 어셈블리만 제공 하 고, {0} 어셈블리를 찾았습니다. '{1}'
...
#### <a name="mm0020-the-valid-options-for-0-are-1"></a>MM0020: '{0}'에 대해 유효한 옵션은 '{1}'입니다.
...
#### <a name="mm0026-could-not-parse-the-command-line-argument-0-1"></a>MM0026: 명령줄 인수 '{0}'을 (를) 구문 분석할 수 없습니다. {1}
...
#### <a name="mm0056-cannot-find-xcode-in-any-of-our-default-locations-please-install-xcode-or-pass-a-custom-path-using---sdkrootpath"></a>MM0056: 기본 위치에서 Xcode를 찾을 수 없습니다. Xcode를 설치 하거나--sdkroot =\<경로를 사용 하 여 사용자 지정 경로를 전달 하세요 >
...
#### <a name="mm0060-could-not-find-the-currently-selected-xcode-on-the-system-xcode-select---print-path-returned-0-but-that-directory-does-not-exist"></a>MM0060: 시스템에서 현재 선택 된 Xcode를 찾을 수 없습니다. ' xcode '가 '{0}'을 (를) 반환 했지만 해당 디렉터리가 없습니다.
...
#### <a name="mm0073-xamarinmac--does-not-support-a-deployment-target-of--the-minimum-is--please-select-a-newer-deployment-target-in-your-projects-infoplist"></a>MM0073: Xamarin.ios \*은 \*의 배포 대상을 지원 하지 않습니다 (최소 \*). 프로젝트의 info.plist에서 최신 배포 대상을 선택 하세요.
...
`--runregistrar`를 실행 하는 경우 하나 이상의 루트 어셈블리를 제공 해야 합니다.

<a name="MM0131" />

#### <a name="mm0131-product-assembly-0-not-found-in-assembly-list-1"></a>MM0131: '{0}' 제품 어셈블리를 '{1}' 어셈블리 목록에서 찾을 수 없습니다.

`--runregistrar`를 실행 하는 경우 어셈블리 목록에 product assembly, Xamarin.ios, XamMac이 포함 되어야 합니다.
...
#### <a name="mm0133-found-more-than-1-assembly-matching-0-choosing-first-1"></a>MM0133: '{0}'에 일치 하는 어셈블리를 하나 이상 찾았습니다. '{1}'을 (를) 선택 합니다.
...
#### <a name="mm1402-the-assembly-0-is-not-compatible-with-this-tool"></a>MM1402: '{0}' 어셈블리는이 도구와 호환 되지 않습니다.

<a name="MM1403" />

#### <a name="mm1403-0-1-could-not-be-found-target-framework-0-is-unusable-to-package-the-application"></a>MM1403: {0} '{1}'을 (를) 찾을 수 없습니다. 대상 프레임 워크 '{0}'은 (는) 응용 프로그램을 패키지 하는 데 사용할 수 없습니다.
...
#### <a name="mm1405-usefullxammacframework-must-always-target-framework-net-45-not-0-which-is-invalid"></a>MM1405: useFullXamMacFramework는 항상 잘못 된 '{0}'이 아닌 프레임 워크 .NET 4.5을 대상으로 해야 합니다.
...
#### <a name="mm1407-mismatch-between-xamarinmac-reference-0-and-target-framework-selected-1"></a>MM1407: Xamarin.ios 참조 '{0}'과 대상 프레임 워크가 일치 하지 않습니다. '{1}'이 (가) 선택 되었습니다.
...
#### <a name="mm1603-unknown-format-for-fat-entry-at-position-0-in-1"></a>MM1603: {1}의 위치 {0}에서 fat 항목에 대 한 알 수 없는 형식입니다.

<a name="MM1604" />

#### <a name="mm1604-file-of-type-0-is-not-a-macho-file-1"></a>MM1604: {0} 형식의 파일이 MachO 파일이 아닙니다 ({1}).
...
#### <a name="mm2003-option-0-will-be-ignored-since-linking-is-disabled"></a>MM2003: 연결이 사용 하지 않도록 설정 되어 있으므로 '{0}' 옵션이 무시 됩니다.
...
#### <a name="mm2011-unknown-tlsprovider-0--valid-values-are-default-or-appletls"></a>MM2011: 알 수 없는 TLSProvider '{0}입니다.  유효한 값은 default 또는 사과 etls입니다.
...
#### <a name="mm2013-failed-to-resolve-the-reference-to-0-referenced-in-1-the-app-will-not-include-the-referenced-assembly-and-may-fail-at-runtime"></a>MM2013: "{1}"에서 참조 되는 "{0}"에 대 한 참조를 확인 하지 못했습니다. 앱은 참조 된 어셈블리를 포함 하지 않으며 런타임에 실패할 수 있습니다.
...
#### <a name="mm2016-invalid-tlsprovider-0-option-the-only-valid-value-1-will-be-used"></a>MM2016: 잘못 된 TlsProvider `{0}` 옵션입니다. 유효한 값 `{1}`만 사용 됩니다.
...
#### <a name="mm202x-binding-optimizer-failed-processing-"></a>MM202x: 바인딩 최적화 프로그램이 `...`를 처리 하지 못했습니다.
...
오류 메시지에서 문제의 원인이 되는 어셈블리의 이름을 지정 합니다. 이 문제를 해결 하려면 자세한 정보 표시를 사용 하는 전체 빌드 로그 (즉, **추가 mtouch 인수**에 `-v -v -v -v`)와 함께 [버그 보고서](https://bugzilla.xamarin.com) 에 어셈블리를 제공 해야 합니다.
...
#### <a name="mm2106-could-not-optimize-the-call-to-blockliteralsetupblockunsafe-in--at-offset--because-"></a>MM2106: \*때문에 오프셋 \*에서 \*의 BlockLiteral. SetupBlock [Unsafe] 호출을 최적화할 수 없습니다.

링커는 `BlockLiteral.SetupBlock` 또는 `Block.SetupBlockUnsafe`에 대 한 호출을 최적화할 수 없을 때이 경고를 보고 합니다.

메시지는 `BlockLiteral.SetupBlock[Unsafe]`를 호출 하는 메서드를 가리키고 호출을 최적화할 수 없는 이유에 대 한 단서를 제공할 수도 있습니다.
...
링커는 개발자가 동적 등록자의 제거를 요청 하는 경우 (mmp에 `--optimize:remove-dynamic-registrar` 전달)이 경고를 보고 하지만 링커가 안전 하지 않은 것으로 확인 합니다.

이 경고를 제거 하려면 mmp에 대 한 최적화 인수를 제거 하거나 무시 하려면 `--nowarn:2107`를 전달 합니다.

기본적으로이 옵션은 가능 하 고 안전 하 게 사용할 수 있도록 자동으로 설정 됩니다.

<a name="MM2108" />

#### <a name="mm2108-0-was-stripped-of-architectures-except-1-to-comply-with-app-store-restrictions-this-could-break-exisiting-codesigning-signatures-consider-stripping-the-library-with-lipo-or-disabling-with---optimize-trim-architectures"></a>MM2108: '{0}'은 (는) 앱 스토어 제한을 준수 하기 위해 '{1}'를 제외한 아키텍처를 제거 했습니다. 이렇게 하면 기존 코드 서명 서명이 손상 될 수 있습니다. Lipo로 라이브러리를 제거 하거나--optimize =-trim 아키텍처를 사용 하 여 비활성화 하는 것이 좋습니다.
...
`lipo` 사용 하 여 원본 라이브러리에서 불필요 한 아키텍처를 영구적으로 제거 하는 것이 좋습니다. 응용 프로그램을 앱 스토어에 게시 하지 않을 경우 `--optimize=-trim-architectures`를 추가 MMP 인수로 전달 하 여이 제거를 비활성화할 수 있습니다.
...
#### <a name="mm3010-exclusion-of-aot-of-0-was-requested-but-was-not-found"></a>MM3010: AOT '{0}'의 제외를 요청 했지만 찾을 수 없습니다.
...
#### <a name="mm4001-the-main-template-could-not-be-expanded-to-0"></a>MM4001: 주 템플릿을 `{0}`확장할 수 없습니다.

### <a name="mm41xx-registrar"></a>MM41xx: 등록자

<a name="MM4134" />

#### <a name="mm4134-your-application-is-using-the-0-framework-which-isnt-included-in-the-macos-sdk-youre-using-to-build-your-app-this-framework-was-introduced-in-osx-2-while-youre-building-with-the-macos-1-sdk-this-configuration-is-not-supported-with-the-static-registrar-pass---registrardynamic-as-an-additional-mmp-argument-in-your-projects-mac-build-option-to-select-alternatively-select-a-newer-sdk-in-your-apps-mac-build-options"></a>MM4134: 응용 프로그램에서 앱을 빌드하는 데 사용 하는 MacOS SDK에 포함 되지 않은 '{0}' 프레임 워크를 사용 하 고 있습니다 .이 프레임 워크는 MacOS {1} SDK를 사용 하 여 빌드하는 동안 OSX {2}에 도입 되었습니다. 이 구성은 정적 등록자 (프로젝트의 Mac 빌드 옵션에서 선택할 수 있는 추가 mmp 인수로 사용 됨)에서 지원 되지 않습니다. 또는 앱의 Mac 빌드 옵션에서 최신 SDK를 선택 합니다.
...
1. 관리 되는 대리자의 형식은 `System.Delegate` 또는 `System.MulticastDelegate`입니다. 이러한 형식은 특정 서명을 표시 하지 않습니다. 즉, 등록 자가 해당 네이티브 서명을 계산할 수 없습니다. 이 경우 해결 방법은 블록에 특정 대리자 형식을 사용 하는 것입니다. 또는 프로젝트의 Mac 빌드 옵션에 `--nowarn:4173` 추가 mmp 인수로 추가 하 여 경고를 무시할 수 있습니다.
...
#### <a name="mm5103-failed-to-compile-error-code---0-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MM5103: 컴파일하지 못했습니다. 오류 코드-{0}. 버그 보고서를 파일에 입력 하십시오 http://bugzilla.xamarin.com
