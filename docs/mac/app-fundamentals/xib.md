...
다음으로 `MainWindow` 클래스에 다음 partial 클래스를 추가 합니다.
...
뷰를 사용 하 고 표시 하는 창의 xib 파일을 편집 합니다. 코드에 의해 C# 메모리에 로드 된 후 보기에 대 한 컨테이너 역할을 하는 **사용자 지정 보기** 를 추가 하 고 `ViewContainer` 라는 콘센트에 노출 합니다.
