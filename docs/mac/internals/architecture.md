...
- 기존 목표-C 클래스를 래핑하는 모든 클래스의 경우,이 클래스는 목표-c 멤버를 사용 하 여 새 목표 C 클래스를 만들고, `[Export]` 특성이 있는 모든 관리 되는 멤버를 미러링 합니다.
...
관리 코드에는 개체를 목표 C에 노출 해야 한다는 것을 알 수 있도록 등록 자가 사용 하는 `[Register]` 및 `[Export]`특성이 포함 될 수 있습니다. [Register] 특성은 생성 된 기본 이름이 적절 하지 않은 경우 생성 된 목표-C 클래스의 이름을 지정 하는 데 사용 됩니다. NSObject에서 파생 된 모든 클래스는 목표-C에 자동으로 등록 됩니다. 필수 [Export] 특성에는 생성 된 목표-C 클래스에서 사용 되는 선택기 인 문자열이 포함 되어 있습니다.
...
Xamarin.ios에서 `bmac` 이라는 생성기는 이러한 정의 파일을 사용 하 고 .NET 도구를 사용 하 여 임시 어셈블리로 컴파일합니다. 그러나이 임시 어셈블리는 목표-C 코드를 호출할 수 없습니다. 그러면 생성기에서 임시 어셈블리를 읽고 런타임에 사용할 C# 수 있는 코드를 생성 합니다. 예를 들어, 사용자가 정의 .cs 파일에 임의 특성을 추가 하면 출력 코드에 표시 되지 않습니다. 생성기는이에 대해 알지 못합니다. 따라서 `bmac` 임시 어셈블리에서이를 검색 하 여 출력 하는 것을 알 수 없습니다.

Xamarin.ios를 만든 후에는 패키지를 `mmp`하 여 모든 구성 요소를 함께 묶을 수 있습니다.
...
그런 다음 사용자 코드를 Xamarin.ios를 참조 하는 어셈블리로 컴파일하고 `mmp` 실행 하 여 패키지를 만드는 사용자 빌드 프로세스의 일부로 실행 됩니다.
