...
- JavaSet [\<E >](https://developer.android.com/reference/java/util/Set.html) 를 시스템 형식 [ICollection\<t >](xref:System.Collections.Generic.ICollection`1), 도우미 클래스 Android. t [\<](xref:Android.Runtime.JavaSet`1)에 매핑합니다.

- JavaList [\<E >](https://developer.android.com/reference/java/util/List.html) 은 시스템 형식 [IList\<t >](xref:System.Collections.Generic.IList`1), 도우미 클래스\<[>](xref:Android.Runtime.JavaList`1)에 매핑됩니다.
...
- JavaCollection [\<E >](https://developer.android.com/reference/java/util/Collection.html) 은 시스템 형식 [ICollection\<t >](xref:System.Collections.Generic.ICollection`1), 도우미 클래스\<[>](xref:Android.Runtime.JavaCollection`1)에 매핑됩니다.

이러한 유형의 더 빠른 복사를 용이 하 게 하는 도우미 클래스를 제공 했습니다. 가능 하면 [`List<T>`](xref:System.Collections.Generic.List`1) 또는 [`Dictionary<TKey, TValue>`](xref:System.Collections.Generic.Dictionary`2)와 같이 프레임 워크에서 제공 하는 구현 대신 제공 된 컬렉션을 사용 하는 것이 좋습니다. [Android Runtime](xref:Android.Runtime) 구현은 내부적으로 네이티브 Java 컬렉션을 사용 하므로 android API 멤버에 전달할 때 네이티브 컬렉션 간에 복사 하지 않아도 됩니다.

인터페이스 구현을 해당 인터페이스를 허용 하는 Android 메서드에 전달할 수 있습니다. 예를 들어 [Arrayadapter&lt;int&gt;(Context, int, IList&lt;int&gt;)](xref:Android.Widget.ArrayAdapter`1) 생성자에 `List<int>`를 전달 합니다. *그러나*android runtime 구현을 *제외한* 모든 구현에 대해 Mono VM에서 android 런타임 VM으로 목록을 *복사* 하는 작업이 포함 됩니다. 나중에 Android 런타임 내에서 목록을 변경 하는 경우 (예: [Arrayadapter&lt;t&gt;를 호출 합니다. Add (T)](xref:Android.Widget.ArrayAdapter`1.Add*) 메서드)는 관리 코드에 표시 *되지* 않습니다. `JavaList<int>` 사용 하는 경우 이러한 변경 내용이 표시 됩니다.
...
- Java 메서드 쌍 `T getFoo()` 및 `void setFoo(T)`는 `Foo` 속성으로 변환 됩니다. 예: [Activity.](xref:Android.App.Activity.Intent)

- `getFoo()` Java 메서드는 읽기 전용 Foo 속성으로 변환 됩니다. 예: [PackageName](xref:Android.Content.Context.PackageName).
...
1. 에 `void` 반환 형식이 있습니다.

1. 는 하나의 매개 변수만 허용 하 고 매개 변수 형식은 인터페이스 이며 인터페이스에는 메서드가 하나 뿐 이며 인터페이스 이름은 `Listener` (예: [View. OnClick *수신기*](xref:Android.Views.View.IOnClickListener))로 끝납니다.

또한 수신기 인터페이스 메서드의 반환 형식이 **void**가 아닌 **부울** 인 경우 생성 된 *EventArgs* 하위 클래스에는 *처리* 된 속성이 포함 됩니다. *처리* 된 속성의 값은 *수신기* 메서드의 반환 값으로 사용 되며 기본값은 `true`입니다.

예를 들어 Android [보기. setOnKeyListener ()](xref:Android.Views.View.SetOnKeyListener*) 메서드는 [onKey (view, int, KeyEvent)](xref:Android.Views.View.IOnKeyListener.OnKey*) 메서드를 사용할 때 [뷰](xref:Android.Views.View.IOnKeyListener) . onkeylistener 인터페이스를 허용 하 고, System.windows.forms.keyeventargs.handled&gt;에 해당 하는 [&lt;EventHandler ](xref:Android.Views.View.KeyEventArgs)이벤트 인 [KeyPress](xref:Android.Views.View.KeyPress) 이벤트를 생성 합니다.
...
모든 수신기 인터페이스가 [`Android.Runtime.IJavaObject`](xref:Android.Runtime.IJavaObject) 을 구현 합니다.
...
`Runnable` 인터페이스에는 단일 void 메서드인 [run ()](xref:Java.Lang.Runnable.Run)이 포함 되어 있습니다. 따라서를 System.object C# 로 바인딩하는 것이 자동으로 [수행](xref:System.Action) 됩니다. [View.Post (](xref:Android.Views.View.Post*) [)와 같이](xref:Android.App.Activity.RunOnUiThread*) 네이티브 api의 `Runnable`를 사용 하는 모든 API 멤버에 대해 `Action` 매개 변수를 허용 하는 바인딩에 오버 로드를 제공 했습니다.
...
`CubeWallpaper.CubeEngine`는 `CubeWallpaper`내에서 중첩 되 고 `CubeWallpaper`는 `WallpaperService.Engine`의 포함 하는 클래스에서 상속 되며, `CubeWallpaper.CubeEngine`에는 선언 형식을 사용 하는 생성자 (이 경우에는 위에 지정 된 대로)가 포함 됩니다.
...
*넣는* 인터페이스 상수는 [ParcelableConsts](xref:Android.OS.ParcelableConsts) 형식에 배치 됩니다. 중첩 된 [넣는 ClassLoaderCreator\<t >](https://developer.android.com/reference/android/os/Parcelable.ClassLoaderCreator.html) 및 [넣는\<t >](https://developer.android.com/reference/android/os/Parcelable.Creator.html) 형식은 현재 제네릭 지원의 제한 사항으로 인해 바인딩되어 있지 않습니다. 지원 되는 경우 해당 사용자는 *IParcelableClassLoaderCreator* 및 *IParcelableCreator* 인터페이스로 제공 됩니다. 예를 들어, 중첩 된 [DeathRecipient](https://developer.android.com/reference/android/os/IBinder.DeathRecipient.html) 인터페이스는 [IBinderDeathRecipient](xref:Android.OS.IBinderDeathRecipient) 인터페이스에 바인딩되어 있습니다.
...
결과적으로 Java 식 *Mediastore* 를 C# 식의 여러 java 설명서를 읽어 서 검색 하기 어려운 식 *mediastore* . d a. d. 1\.9에서 해당 하 C# 는 식은 [*Mediastore. videocolumns입니다*](xref:Android.Provider.MediaStore.Video.VideoColumns.Title).
...
예를 들어 `main.axml`(사용자 인터페이스 레이아웃), 국제화 테이블 문자열 (`strings.xml`) 및 일부 아이콘 (`drawable-*/icon.png`)을 포함 하는 샘플 Android 앱은 해당 리소스를 응용 프로그램의 "Resources" 디렉터리에 유지 합니다.
...
네이티브 Android Api는 파일 이름으로 직접 작동 하지 않고 대신 리소스 Id에 대해 작동 합니다. 리소스를 사용 하는 Android 응용 프로그램을 컴파일할 때 빌드 시스템은 배포를 위해 리소스를 패키지 하 고 포함 된 각 리소스에 대 한 토큰을 포함 하는 `Resource` 라는 클래스를 생성 합니다. 예를 들어 위의 리소스 레이아웃의 경우 R 클래스가 노출 하는 항목은 다음과 같습니다.
...
그런 다음 `Resource.Drawable.icon`를 사용 하 여 `drawable/icon.png` 파일을 참조 하거나 `layout/main.xml` 파일을 참조할 `Resource.Layout.main` 하거나 사전 파일 `Resource.String.first_string`에서 첫 번째 문자열을 참조 하는 `values/strings.xml`합니다.
