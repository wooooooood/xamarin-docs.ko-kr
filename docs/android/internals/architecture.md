...
Xamarin Android 개발자는 운영 체제의 다양 한 기능에 액세스할 수 있습니다. 즉, 낮은 수준의 액세스에 대해 알고 있는 .NET Api를 호출 하거나 android 런타임에 노출 되는 Java Api에 대 한 브리지를 제공 하는 Android 네임 스페이스에 노출 된 클래스를 사용 합니다.
...
인스턴스가 스레드 간에 실수로 공유 될 수 있는 경우 관리 되는 호출 가능 래퍼를 삭제할 때 주의가 필요 합니다. 인스턴스를 삭제 하면 다른 스레드의 참조에 영향을 줍니다. 최대 보안을 위해 `Dispose()` `new` 항상 새 인스턴스를 할당 하 고 캐시 된 인스턴스가 아닌 새 인스턴스 *를 할당* *하는* 인스턴스를 통해 할당 된 인스턴스만 스레드 간에 실수로 인스턴스가 공유 되도록 할 수 있습니다.

## <a name="managed-callable-wrapper-subclasses"></a>관리 되는 호출 가능 래퍼 서브 클래스

관리 되는 호출 가능 래퍼 서브 클래스는 모든 "흥미로운" 응용 프로그램별 논리가 존재할 수 있는 위치입니다. 여기에는 사용자 지정 [Android.](xref:Android.App.Activity) Activity1 서브 클래스 (예: 기본 프로젝트 템플릿의 [Activity1](https://github.com/xamarin/monodroid-samples/blob/master/HelloM4A/Activity1.cs#L13) 형식)가 포함 됩니다. 특히 [registerattribute](xref:Android.Runtime.RegisterAttribute) 사용자 지정 특성 또는 [registerattribute](xref:Android.Runtime.RegisterAttribute.DoNotGenerateAcw) 를 포함 *하지* 않는 DoNotGenerateAcw *하위 클래스입니다* . 기본값은 *false*입니다.
...
관리 되는 호출 가능 래퍼와 달리 인스턴스를 *삭제*하기 전에 해당 인스턴스를 삭제 하기 전에 *수행 해야 하는 일은 Java* 인스턴스 (Android 호출 가능 래퍼 인스턴스)와 관리 되는 인스턴스의 매핑을 해제 하는 것입니다.

### <a name="java-activation"></a>Java 활성화

Java에서 acw ( [Android 호출 가능 래퍼](~/android/platform/java-integration/android-callable-wrappers.md) )를 만든 경우 acw 생성자를 통해 해당 c # 생성자가 호출 됩니다. 예를 들어, *Mainactivity* 의 acw에는 *mainactivity*의 기본 생성자를 호출 하는 기본 생성자가 포함 됩니다. 이 작업은 ACW 생성자 내에서 *Typemanager. Activate ()* 호출을 통해 수행 됩니다.
...
(2)은 누설 추상화입니다. C #에서와 같이 Java에서 생성자의 가상 메서드 호출은 항상 가장 많이 파생 된 메서드 구현을 호출 합니다. 예를 들어 [TextView (Context, AttributeSet, int) 생성자](xref:Android.Widget.TextView#ctor*) 는 TextView [TextView getDefaultMovementMethod ()](https://developer.android.com/reference/android/widget/TextView.html#getDefaultMovementMethod())를 호출 합니다 .이 메서드는 [. defaultmovementmethod 속성](xref:Android.Widget.TextView.DefaultMovementMethod)으로 바인딩됩니다.
따라서 [Logtextbox](https://github.com/xamarin/monodroid-samples/blob/f01b5c31/ApiDemo/Text/LogTextBox.cs) 형식이 (1) [서브 클래스 TextView](https://github.com/xamarin/monodroid-samples/blob/f01b5c31/ApiDemo/Text/LogTextBox.cs#L26), (2) [재정의 TEXTVIEW](https://github.com/xamarin/monodroid-samples/blob/f01b5c31/ApiDemo/Text/LogTextBox.cs#L45)및 (3) [XML을 통해 해당 클래스의 인스턴스를 활성화](https://github.com/xamarin/monodroid-samples/blob/f01b5c31/ApiDemo/Resources/layout/log_text_box_1.xml#L29) 하 고, (3) Acw 생성자를 실행 하기 전에 재정의 된 *Defaultmovementmethod* 속성을 호출 하 고 c # 생성자를 실행 하기 전에 발생 합니다.

ACW LogTextBox 인스턴스가 먼저 관리 코드를 입력 한 다음 ACW 생성자가 실행 될 때 *동일한 인스턴스에서* [Logtextbox (Context, iattributeset, int)](https://github.com/xamarin/monodroid-samples/blob/f01b5c31/ApiDemo/Text/LogTextBox.cs#L41) 생성자를 호출 하면 [LogTextView (IntPtr, JniHandleOwnership)](https://github.com/xamarin/monodroid-samples/blob/f01b5c31/ApiDemo/Text/LogTextBox.cs#L28) 생성자를 통해 인스턴스 logtextbox를 인스턴스화하여이를 지원할 수 있습니다.
...
5. *monodroid ()* n_GetDefaultMovementMethod는 apidemo ()를 호출 하는 *logtextbox. n_getDefaultMovementMethod (* )를 호출 합니다 .이는 getDefaultMovementMethod *()* 를 호출 하며,이는 [ &lt; TextView &gt; (handle, JniHandleOwnership)](xref:Java.Lang.Object.GetObject*) 를 호출 합니다.

6. *Java. 개체. GetObject &lt; TextView &gt; ()* 는 이미 *핸들* 에 해당 하는 c # 인스턴스가 있는지 확인 합니다. 있는 경우 반환 됩니다. 이 시나리오에서는 *개체. GetObject &lt; t &gt; ()* 가 하나를 만들어야 합니다.

7. *개체. GetObject &lt; T &gt; ()* 는 *Logtextbox (IntPtr, JniHandleOwneship)* 생성자를 찾고 호출 하 고, *핸들과* 만든 인스턴스 간의 매핑을 만들고, 만든 인스턴스를 반환 합니다.

8. *N_GetDefaultMovementMethod TextView ()* 은 *Logtextbox. DefaultMovementMethod* 속성 getter를 호출 합니다.
...
## <a name="application-startup"></a>애플리케이션 시작

활동, 서비스 등을 시작 하면 Android는 먼저 활동/서비스를 호스트 하는 프로세스가 이미 실행 중인지 확인 합니다. 이러한 프로세스가 없으면 새 프로세스가 생성 되 고 [Androidmanifest](https://developer.android.com/guide/topics/manifest/manifest-intro.html) 을 읽고 특성에 지정 된 형식이 [/manifest/application/@android:name](https://developer.android.com/guide/topics/manifest/application-element.html#nm) 로드 되 고 인스턴스화됩니다. 그런 다음 특성 값으로 지정 된 모든 형식이 [/manifest/application/provider/@android:name](https://developer.android.com/guide/topics/manifest/provider-element.html#nm) 인스턴스화되고 해당 [contentprovider %28)](xref:Android.Content.ContentProvider.AttachInfo*) 메서드가 호출 됩니다. Xamarin Android는 mono를 추가 하 여이에 후크 합니다 *. *빌드 프로세스 중에 MonoRuntimeProvider *contentprovider* To AndroidManifest. *Mono입니다. AttachInfo ()* 메서드는 Mono 런타임을 프로세스로 로드 하는 작업을 담당 합니다. MonoRuntimeProvider.
이 시점 전에 Mono를 사용 하려는 시도는 실패 합니다. ( *참고*: Mono를 초기화 하기 전에 응용 프로그램 인스턴스가 생성 됨에 따라 [응용 프로그램의 서브 클래스](xref:Android.App.Application) 는 [(IntPtr, JniHandleOwnership) 생성자](https://github.com/xamarin/monodroid-samples/blob/a9e8ef23/SanityTests/Hello.cs#L103)를 제공 해야 하는 형식입니다.)

프로세스 초기화가 완료 되 면을 (를) `AndroidManifest.xml` 실행 하 여 작업/서비스/등의 클래스 이름을 찾습니다. 예를 들어 [ /manifest/application/activity/@android:name 특성](https://developer.android.com/guide/topics/manifest/activity-element.html#nm) 은 로드할 활동의 이름을 결정 하는 데 사용 됩니다. 활동의 경우이 유형은 [android. Activity](xref:Android.App.Activity)를 상속 해야 합니다.
