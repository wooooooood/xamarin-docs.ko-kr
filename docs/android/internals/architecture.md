...
-   *참고:* Assembly [. location](xref:System.Reflection.Assembly.Location) 및 [assembly](xref:System.Reflection.Assembly.CodeBase)
    와 같은 어셈블리 위치 정보는 릴리스 빌드에서*사용할 수 없습니다* . 이러한 항목은 고유 파일 시스템 항목으로 존재 하지 않으며 사용 가능한 위치를 갖지 않습니다.
...
인스턴스가 스레드 간에 실수로 공유 될 수 있는 경우 관리 되는 호출 가능 래퍼를 삭제할 때 주의가 필요 합니다. 인스턴스를 삭제 하면 다른 스레드의 참조에 영향을 줍니다. `Dispose()` 최대 안전을 *위해 항상 새* 인스턴스를 할당 하 고 캐시 된 `new` 인스턴스가 아닌 새 인스턴스를 할당 하는 인스턴스 *를 통해 할당* 된 인스턴스만 실수로 인스턴스를 공유할 수 있습니다. 임계값.
...
관리 되는 호출 가능 래퍼와 달리 인스턴스를 *삭제*하기 전에 해당 인스턴스를 삭제 하기 전에 *수행 해야 하* 는 일은 Java 인스턴스 (Android 호출 가능 래퍼의 인스턴스)와 관리 되는 인스턴스.
...
2.  Android는 Layout 개체 그래프를 인스턴스화하고 monodroid의 인스턴스를 인스턴스화합니다. logtextbox *의* acw는 *apidemo* 입니다.
...
5.  *monodroid () apidemo* ()를 호출 하는 *TextView ()* [를호출하는getDefaultMovementMethod()를호출합니다.&lt; TextView&gt; (handle, JniHandleOwnership DoNotTransfer)](xref:Java.Lang.Object.GetObject*) .
...
이 시점 전에 Mono를 사용 하려는 시도는 실패 합니다. ( *참고*: 이러한 이유로 Mono를 초기화 하기 전에 응용 프로그램 인스턴스가 만들어지기 때문에 JniHandleOwnership [하위 클래스](xref:Android.App.Application) 에서 [(IntPtr,) 생성자](https://github.com/xamarin/monodroid-samples/blob/a9e8ef23/SanityTests/Hello.cs#L103)를 제공 해야 하는 형식입니다.)
