...
- **`Adapter`**&ndash;늘어납니다 항목 레이아웃 (레이아웃 파일의 콘텐츠를 인스턴스화) 및에 표시 되는 뷰에 데이터를 바인딩합니다 `RecyclerView` . 또한 어댑터는 항목 클릭 이벤트를 보고 합니다.

- **`LayoutManager`**&ndash;내에서 항목 뷰를 측정 및 배치 `RecyclerView` 하 고 뷰 재활용에 대 한 정책을 관리 합니다.
...
`RecyclerView`, 및 클래스 간의 관계는 `LayoutManager` `Adapter` 다음 다이어그램에 나와 있습니다.
...
이 그림에서 설명 하는 것 처럼는 `LayoutManager` 와의 중개자로 간주할 수 있습니다 `Adapter` `RecyclerView` . 는를 대신 하 여 `LayoutManager` 메서드를 호출 합니다 `Adapter` `RecyclerView` . 예를 들어는 `LayoutManager` `Adapter` 에서 특정 항목 위치에 대 한 새 뷰를 만들 때 메서드를 호출 합니다 `RecyclerView` . 는 해당 `Adapter` 항목의 레이아웃을 늘어납니다 하 고 `ViewHolder` 인스턴스 (표시 되지 않음)를 만들어 해당 위치의 뷰에 대 한 참조를 캐시 합니다. 에서를 `LayoutManager` 호출 `Adapter` 하 여 특정 항목을 데이터 집합에 바인딩하면는 해당 `Adapter` 항목에 대 한 데이터를 찾아서 데이터 집합에서 검색 한 다음 연결 된 항목 뷰로 복사 합니다.

앱에서를 사용 하는 경우 `RecyclerView` 다음 클래스의 파생 형식 만들기가 필요 합니다.

- **`RecyclerView.Adapter`** 응용 프로그램 &ndash; 에 관련 된 응용 프로그램의 데이터 집합에서 내에 표시 되는 항목 뷰로의 바인딩을 제공 `RecyclerView` 합니다. 어댑터는의 각 항목 뷰 위치 `RecyclerView` 를 데이터 원본의 특정 위치에 연결 하는 방법을 알고 있습니다. 또한 어댑터는 각 개별 항목 보기 내에서 콘텐츠의 레이아웃을 처리 하 고 각 보기에 대 한 보기 홀더를 만듭니다. 또한 어댑터는 항목 보기에서 검색 된 항목 클릭 이벤트를 보고 합니다.
...
- **`RecyclerView.LayoutManager`**&ndash;내에 항목을 배치 `RecyclerView` 합니다. 미리 정의 된 여러 레이아웃 관리자 중 하나를 사용 하거나 고유한 사용자 지정 레이아웃 관리자를 구현할 수 있습니다.
    `RecyclerView`레이아웃 정책을 레이아웃 관리자에 위임 하므로, 앱을 크게 변경할 필요 없이 다른 레이아웃 관리자에 연결할 수 있습니다.

또한 필요에 따라 다음 클래스를 확장 하 여 앱에서의 모양과 느낌을 변경할 수 있습니다 `RecyclerView` .

- **`RecyclerView.ItemDecoration`**
- **`RecyclerView.ItemAnimator`**

및를 확장 하지 않으면 `ItemDecoration` `ItemAnimator` 에서 `RecyclerView` 기본 구현을 사용 합니다. 이 가이드에서는 사용자 지정 및 클래스를 만드는 방법에 대해서는 설명 하지 않습니다. `ItemDecoration` `ItemAnimator` 이러한 클래스에 대 한 자세한 내용은 [RecyclerView 데코레이션](https://developer.android.com/reference/android/support/v7/widget/RecyclerView.ItemDecoration.html) 및 [RecyclerView](https://developer.android.com/reference/android/support/v7/widget/RecyclerView.ItemAnimator.html)를 참조 하세요.
...
5. 화면에 표시 되는의 항목 목록에 재활용 된 보기가 추가 됩니다 `RecyclerView` .

6. 사용자가 `RecyclerView` 목록에서 다음 항목으로를 스크롤하면 재활용 된 보기가 화면에 표시 됩니다. 한편, 다른 보기는 위 단계에 따라 표시 되지 않으며 재활용 됩니다.

항목 뷰 다시 사용 뿐만 아니라 `RecyclerView` 도 다른 효율성 최적화를 사용 합니다. 뷰 소유자. *뷰 보유자* 는 뷰 참조를 캐시 하는 간단한 클래스입니다. 어댑터는 항목 레이아웃 파일을 늘어납니다 때마다 해당 하는 뷰 보유자를 만듭니다. 뷰 보유자는 `FindViewById` 를 사용 하 여 팽창 된 항목 레이아웃 파일 내의 뷰에 대 한 참조를 가져옵니다. 이러한 참조는 새 데이터를 표시 하기 위해 레이아웃을 재활용할 때마다 새 데이터를 뷰에 로드 하는 데 사용 됩니다.

## <a name="the-layout-manager"></a>레이아웃 관리자

레이아웃 관리자는 표시 항목의 위치를 지정 하는 것을 담당 합니다 `RecyclerView` . 프레젠테이션 유형 (목록 또는 그리드), 방향 (세로 또는 가로로 표시 되는 항목) 및 표시 되는 방향 항목 (보통 순서 또는 역순)을 결정 합니다. 또한 레이아웃 관리자는 **RecycleView** 디스플레이에서 각 항목의 크기와 위치를 계산 하는 일을 담당 합니다.
...
레이아웃 관리자를 지정 하려면 선택한 레이아웃 관리자를 인스턴스화하고 `SetLayoutManager` 메서드에 전달 합니다. 레이아웃 관리자는 *must* &ndash; `RecyclerView` 기본적으로 미리 정의 된 레이아웃 관리자를 선택 하지 않도록 지정 해야 합니다.
...
뷰 보유자는 뷰 참조를 캐시 하기 위해 정의 하는 클래스입니다. 어댑터는 이러한 뷰 참조를 사용 하 여 각 뷰를 해당 내용에 바인딩합니다. 의 모든 항목에 `RecyclerView` 는 해당 항목에 대 한 뷰 참조를 캐시 하는 연결 된 뷰 홀더 인스턴스가 있습니다. 뷰 홀더를 만들려면 다음 단계를 사용 하 여 항목당 정확한 뷰 집합을 보유할 클래스를 정의 합니다.

1. 서브 클래스 `RecyclerView.ViewHolder` 입니다.
2. 뷰 참조를 조회 하 고 저장 하는 생성자를 구현 합니다.
3. 어댑터에서 이러한 참조에 액세스 하는 데 사용할 수 있는 속성을 구현 합니다.

구현에 대 한 자세한 예제는 `ViewHolder` [기본 RecyclerView 예제](~/android/user-interface/layouts/recycler-view/recyclerview-example.md)에 나와 있습니다.
...
대부분의 `RecyclerView` 통합 코드는 어댑터에서 수행 됩니다. `RecyclerView`에서 파생 된 어댑터를 제공 하 여 `RecyclerView.Adapter` 데이터 원본에 액세스 하 고 각 항목을 데이터 원본의 콘텐츠로 채워야 합니다.
데이터 원본은 응용 프로그램 마다 다르기 때문에 데이터에 액세스 하는 방법을 이해 하는 어댑터 기능을 구현 해야 합니다. 어댑터는 데이터 원본에서 정보를 추출 하 여 컬렉션의 각 항목에 로드 합니다 `RecyclerView` .

다음 그림에서는 어댑터가 보기 소유자를 통해 데이터 원본의 콘텐츠를의 각 행 항목 내에 있는 개별 보기에 매핑하는 방법을 보여 줍니다 `RecyclerView` .
...
어댑터는 `RecyclerView` 특정 행 항목에 대 한 데이터가 포함 된 각 행을 로드 합니다. 예를 들어 행 위치 *p*의 경우 어댑터는 데이터 원본 내의 위치 *p* 에서 연결 된 데이터를 찾고이 데이터를 컬렉션의 *p* 위치에 있는 행 항목에 복사 `RecyclerView` 합니다.
예를 들어 위의 그리기에서 어댑터는 보기 소유자를 사용 하 여 해당 위치에서 및에 대 한 참조를 조회 `ImageView` `TextView` 하므로 `FindViewById` 사용자가 컬렉션을 스크롤하고 뷰를 다시 사용할 때 해당 뷰에 대해를 반복적으로 호출할 필요가 없습니다.

어댑터를 구현 하는 경우 다음 메서드를 재정의 해야 합니다 `RecyclerView.Adapter` .
...
레이아웃 관리자는 내에서 항목의 위치를 지정 하는 동안 이러한 메서드를 호출 합니다 `RecyclerView` .

## <a name="notifying-recyclerview-of-data-changes"></a>RecyclerView 데이터 변경 알림

`RecyclerView`는 해당 데이터 소스의 내용이 변경 될 때 자동으로 표시를 업데이트 하지 않습니다. 어댑터는 `RecyclerView` 데이터 집합이 변경 된 경우에 알려야 합니다. 데이터 집합은 여러 가지 방법으로 변경 될 수 있습니다. 예를 들어 항목 내의 내용이 변경 되거나 전체 데이터 구조가 변경 될 수 있습니다.
`RecyclerView.Adapter`는를 호출 하 여 `RecyclerView` 가장 효율적인 방식으로 데이터 변경에 대응 하는 여러 가지 메서드를 제공 합니다.
...
데이터 집합이 변경 된 방식을 정확히 알고 있는 경우 위의 적절 한 메서드를 호출 하 여 가장 효율적인 방식으로 새로 고칠 수 있습니다 `RecyclerView` . 데이터 집합이 변경 된 방식을 정확 하 게 알 수 없는 경우을 (를) 호출할 수 있습니다 `NotifyDataSetChanged` .이는 `RecyclerView` 사용자에 게 표시 되는 모든 보기를 새로 고쳐야 하기 때문에 더 효율적입니다. 이러한 메서드에 대 한 자세한 내용은 [RecyclerView](https://developer.android.com/reference/android/support/v7/widget/RecyclerView.Adapter.html)를 참조 하세요.
