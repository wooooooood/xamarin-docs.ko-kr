...
목록 뷰와 어댑터는 Android 응용 프로그램의 가장 기본적인 구성 요소에 포함 되어 있습니다. 클래스 `ListView` 는 짧은 메뉴 또는 긴 스크롤 목록 인지 여부에 관계 없이 데이터를 표시 하는 유연한 방법을 제공 합니다. 응용 프로그램에 대 한 모바일 친화적인 사용자 인터페이스를 빌드하는 데 도움이 되는 빠른 스크롤, 인덱스, 단일 또는 다중 선택 등의 유용성 기능을 제공 합니다. `ListView` 인스턴스에는 행 보기에 포함된 데이터를 사용하여 피드하는 *어댑터*가 필요합니다.

이 가이드에서는 Xamarin Android에서 `ListView` 및 다양 한 `Adapter` 클래스를 구현 하는 방법을 설명 합니다. 또한의 `ListView`모양을 사용자 지정 하는 방법을 보여 주고 메모리 사용을 줄이기 위해 행 다시 사용의 중요성에 대해 설명 합니다. 활동 수명 주기가 영향 `ListView` `Adapter` 을 주고 사용 하는 방법에 대해서도 설명 합니다. Xamarin.ios를 사용 하 여 플랫폼 간 응용 프로그램에서 작업 하는 경우 컨트롤 `ListView` 은 구조적 ios `UITableView` 와 비슷하며, `UITableViewSource`Android `Adapter` 는와 유사 합니다.

먼저 간단한 자습서에서는 기본 코드 예제 `ListView` 를 사용 하는를 소개 합니다. 다음으로, 실제 응용 프로그램에서 사용 `ListView` 하는 데 도움이 되는 고급 항목에 대 한 링크가 제공 됩니다.

> [!NOTE]
> 위젯은 보다 고급 및 유연한 `ListView`버전입니다. `RecyclerView` 는 `RecyclerView` `ListView` (및 `RecyclerView` `ListView` )의 후속 작업으로 설계 되었기 때문에 새 앱 개발 대신를 사용 하는 것이 좋습니다. `GridView` 자세한 내용은 [RecyclerView](~/android/user-interface/layouts/recycler-view/index.md)를 참조 하세요.

## <a name="listview-tutorial"></a>ListView 자습서

[`ListView`](xref:Android.Widget.ListView)는입니다.[`ViewGroup`](xref:Android.Views.ViewGroup)
스크롤 가능한 항목 목록을 만드는입니다. 목록 항목은를 [`IListAdapter`](xref:Android.Widget.IListAdapter)사용 하 여 목록에 자동으로 삽입 됩니다.
...
를 `MainActivity.cs` 열고 클래스를 수정 하 여 [`ListActivity`](xref:Android.App.ListActivity) (대신 [`Activity`](xref:Android.App.Activity))를 확장 합니다.
...
메서드에 대 한 [`OnCreate()`](xref:Android.App.Activity.OnCreate*)다음 코드를 삽입 합니다.
...
이는 작업 (일반적 [`SetContentView(int)`](xref:Android.App.Activity.SetContentView*)으로)에 대 한 레이아웃 파일을 로드 하지 않습니다.
...
이 메서드는에 [`ArrayAdapter<T>`](xref:Android.Widget.ArrayAdapter`1)배치 [`ListView`](xref:Android.Widget.ListView)될 목록 항목의 배열을 관리 하는를 사용 합니다.
여[`ArrayAdapter<T>`](xref:Android.Widget.ArrayAdapter`1)
생성자는 응용 프로그램 [`Context`](xref:Android.Content.Context), 각 목록 항목 (이전 단계에서 생성) `T[]` 에 대 한 레이아웃 설명, 또는[`Java.Util.IList<T>`](xref:Java.Util.IList)
에 삽입할 개체의 배열입니다.[`ListView`](xref:Android.Widget.ListView)
(다음에 정의 됨).

여[`TextFilterEnabled`](xref:Android.Widget.AbsListView.TextFilterEnabled)
속성은 [`ListView`](xref:Android.Widget.ListView)에 대 한 텍스트 필터링을 설정 하 여 사용자가 입력을 시작 하면 목록이 필터링 됩니다.

여[`ItemClick`](xref:Android.Widget.AdapterView.ItemClick)
...
에 대 한 [`ListAdapter`](xref:Android.App.ListActivity.ListAdapter)고유한 레이아웃 파일을 정의 하는 대신 플랫폼에서 제공 하는 목록 항목 디자인을 사용할 수 있습니다.
예를 들어 `Resource.Layout.list_item`대신를 `Android.Resource.Layout.SimpleListItem1` 사용해 보세요.
...
using System;
```
다음으로 다음 문자열 배열을의 `MainActivity`멤버로 추가 합니다.
...
에 배치 [`ListView`](xref:Android.Widget.ListView)되는 문자열 배열입니다.
...
위젯. 프로젝트 `string-array` **리소스/값/문자열 .xml** 파일의 리소스와 같이 외부 리소스에 의해 정의 된 문자열 배열을 참조 하는 것이 더 좋습니다. 예를 들어:
...
에 [`ArrayAdapter`](xref:Android.Widget.ArrayAdapter`1)이러한 리소스 문자열을 사용 하려면 원래[`ListAdapter`](xref:Android.App.ListActivity.ListAdapter)
다음 줄로 바꿉니다.
...
- [섹션 인덱스 (샘플)](https://developer.xamarin.com/samples/monodroid/SectionIndex/)
