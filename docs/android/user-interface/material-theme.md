...
재질 테마에서 파생 하 여 사용자 고유의 테마를 만들고 일부 또는 모든 색 특성을 재정의할 수 있습니다. 예를 들어에서 `Theme.Material.Light`파생 되는 테마를 만들 수 있지만, 앱 바 색을 재정의 하 여 브랜드 색과 일치 시킵니다. 또한 개별 보기의 스타일을 지정할 수 있습니다. 예를 들어 모퉁이가 더 둥글고 더 진한 배경색을 사용 하는 [CardView](~/android/user-interface/controls/card-view.md) 에 대 한 스타일을 만들 수 있습니다.
...
또는 **AssemblyInfo.cs** (또는 **Properties.cs**)에서 `Theme` 응용 프로그램 특성을 설정할 수 있습니다. 예를 들어:
...
활동에 테마를 지정 하려면 활동 선언 `Theme` 위의 `[Activity]` 특성에 설정을 추가 하 고 사용 하려는 재질 `Theme` 테마 버전에 할당 합니다. 다음 예제에서는를 사용 하 여 `Theme.Material.Light`작업을 테마에 사용 합니다.
...
브랜드&rsquo;색으로 앱에 스타일을 지정 하는 사용자 지정 테마를 만들어 브랜드를 향상할 수 있습니다. 사용자 지정 테마를 만들려면 기본 제공 재질 테마 버전에서 파생 되는 새 스타일을 정의 하 여 변경 하려는 색 특성을 재정의 합니다. 예를 들어에서 `Theme.Material.Light.DarkActionBar` 파생 되는 사용자 지정 테마를 정의 하 고 화면 배경색을 흰색이 아닌 베이 지로 변경할 수 있습니다.
...
이러한 변경 내용을 적용 하면 *mycustomtheme* 을 사용 하는 앱은의 `my_blue` `my_purple`및 UI 컨트롤에 앱 바 색을 표시 하지만 다른 모든 위치 `Theme.Material.Light` 에서 색 구성표를 사용 합니다.
...
이 예제에서 *mycustomtheme* 은 배경색, 상태 `Theme.Material.Light` 표시줄 및 텍스트 색에 대해 색을 활용 앱 바의 색을로 `my_blue` 변경 하 고 라디오 단추의 색을로 `my_purple`설정 합니다.
...
다음 스크린샷은 사용자 지정 테마 (오른쪽에 표시 `CardView` 됨)로 스타일이 지정 `CardView.MyBlue` 된와 비교 `CardView` 하 여 왼쪽에 표시 되는 기본값 (왼쪽에 표시 됨)의 예를 제공 합니다.
