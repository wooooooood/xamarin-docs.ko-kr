...
새 **list_item** 레이아웃이 디자이너에 표시 됩니다. **List_item** 에 대 한 *Design Surface* 왼쪽 창에 표시 되 고 해당 XML 소스가 오른쪽 창에 표시 &ndash; 두 개의 창이 표시 됩니다. 두 창 사이에 있는 **창 바꾸기** 아이콘을 클릭 하 여 **Design Surface** 및 **소스** 창의 위치를 바꿀 수 있습니다.
...
**도구 상자** 창이 표시 되지 않으면 왼쪽에 있는 **도구 상자** 탭을 클릭 합니다. **도구 상자**에서 **이미지 & 미디어** 섹션까지 아래로 스크롤하고 `ImageView`를 찾을 때까지 아래로 스크롤합니다.
...
또는 검색 창에 *Imageview* 를 입력 하 여 `ImageView`를 찾을 수 있습니다.
...
이 `ImageView`를 Design Surface으로 끌어 놓습니다 (이 `ImageView` 색 브라우저 앱에서 색 견본을 표시 하는 데 사용 됨).
...
그런 다음 **도구 상자** 의 `LinearLayout (Vertical)` 위젯을 디자이너로 끌어 옵니다. 파란색 윤곽선은 추가 된 `LinearLayout`의 경계를 나타냅니다. **문서 개요** 는 `imageView1 (ImageView)`아래에 있는 `LinearLayout`의 자식 임을 보여 줍니다.
...
디자이너에서 `ImageView`를 선택 하면 파란색 윤곽선이 `ImageView`를 둘러쌉니다. 또한 선택 항목이 **문서 개요**의 `imageView1 (ImageView)`로 이동 합니다.
...
그런 다음 **도구 상자** 에서 새로 추가 된 `LinearLayout``Text (Large)` 위젯을 끌어 옵니다. 디자이너는 녹색 강조 표시를 사용 하 여 새 위젯이 삽입 되는 위치를 표시 합니다.
...
다음으로 `Text (Large)` 위젯 아래에 `Text (Small)` 위젯을 추가 합니다.
...
두 `textView` 위젯이 `linearLayout1`내에 있지 않은 경우 **문서 개요** 에서 `linearLayout1`으로 끌어 오도록 하 고 이전 스크린 샷에 표시 된 것 처럼 나타나도록 지정할 수 있습니다 (`linearLayout1`아래에 들여쓰기).

### <a name="arranging-the-user-interface"></a>사용자 인터페이스 정렬

다음 단계는 왼쪽에 `ImageView`을 표시 하도록 UI를 수정 하 고 두 개의 `TextView` 위젯을 `ImageView`오른쪽에 누적 하는 것입니다.

1. `ImageView`를 선택합니다.

2. **속성 창**검색 상자에 *너비* 를 입력 하 고 **레이아웃 너비**를 찾습니다.

3. **레이아웃 너비** 설정을 `wrap_content`로 변경 합니다.

![콘텐츠 래핑 설정](designer-walkthrough-images/vs/15-wrap-content-w158.png)

`Width` 설정을 변경 하는 또 다른 방법은 위젯의 오른쪽에 있는 삼각형을 클릭 하 여 너비 설정을 `wrap_content`설정/해제 하는 것입니다.

![너비를 설정 하려면 끌어 옵니다.](designer-walkthrough-images/vs/15b-width-arrow-w158.png)

삼각형을 다시 클릭 하면 `match_parent``Width` 설정이 반환 됩니다. 그런 다음 **문서 개요** 창으로 이동 하 여 루트 `LinearLayout`를 선택 합니다.

[![루트 LinearLayout 선택](designer-walkthrough-images/vs/16-root-linearlayout-w158-sml.png)](designer-walkthrough-images/vs/16-root-linearlayout-w158.png#lightbox)

루트 `LinearLayout` 선택 하 고 **속성** 창으로 돌아간 다음 검색 상자에 *방향* 을 입력 하 고 **방향** 설정을 찾습니다. `horizontal`**방향** 변경:

![가로 방향 선택](designer-walkthrough-images/vs/17-horizontal-orientation-w158.png)

이 시점에서 디자이너 화면은 다음 스크린샷과 유사 합니다.
`TextView` 위젯은 `ImageView`오른쪽으로 이동 된 것을 볼 수 있습니다.
...
다음 단계는 UI의 안쪽 여백 및 여백 설정을 수정 하 여 위젯 사이에 더 많은 공간을 제공 하는 것입니다. 디자인 화면에서 `ImageView`를 선택 합니다. **속성** 창에서 검색 상자에 `min`을 입력 합니다. 최소 **높이** 및 `50dp`에 대 한 `70dp`를 입력 **합니다.**
...
**속성** 창에서 검색 상자에 `padding`를 입력 하 고 **패딩**에 대 한 `10dp`을 입력 합니다. 이러한 `minHeight``minWidth` 및 `padding` 설정은 `ImageView`의 모든 측면 주위에 안쪽 여백을 추가 하 고 세로로 elongate. 이러한 값을 입력 하면 레이아웃 XML이 변경 됩니다.
...
예를 들어 **왼쪽 안쪽 여백** 필드를 `5dp` 및 안쪽 여백 **아래쪽**안쪽 여백, **오른쪽 안쪽**여백 및 **안쪽** 여백 필드를 `10dp`로 설정 합니다.
...
다음으로 두 개의 `TextView` 위젯을 포함 하는 `LinearLayout` 위젯의 위치를 조정 합니다. **문서 개요**에서 `linearLayout1`를 선택 합니다. **속성** 창에서 검색 상자에 `margin`을 입력 합니다. **레이아웃 여백 아래쪽**, **레이아웃 여백 왼쪽**및 **레이아웃 여백 위쪽** 을 `5dp`로 설정 합니다. `0dp`**레이아웃을 오른쪽** 으로 설정 합니다.
...
`ImageView`는 이미지 대신 색을 표시 하는 데 사용 되기 때문에 다음 단계는 템플릿에 의해 추가 된 기본 이미지 원본을 제거 하는 것입니다.

1. **디자이너 화면**에서 `ImageView`를 선택 합니다.

2. **속성**의 검색 상자에 *src* 를 입력 합니다.

3. **Src** 속성 설정 오른쪽의 작은 사각형을 클릭 하 고 **다시 설정**을 선택 합니다.

[![ImageView src 설정의 선택을 취소 합니다.](designer-walkthrough-images/vs/22-clear-img-src-w158-sml.png)](designer-walkthrough-images/vs/22-clear-img-src-w158.png#lightbox)

그러면 해당 `ImageView`의 원본 XML에서 `android:src="@android:drawable/ic_menu_gallery"` 제거 됩니다.

### <a name="adding-a-listview-container"></a>ListView 컨테이너 추가

이제 **list_item** 레이아웃이 정의 되었으므로 다음 단계는 주 레이아웃에 `ListView`를 추가 하는 것입니다. 이 `ListView`에는 **list_item**목록이 포함 됩니다. 

**솔루션 탐색기**에서 **Resources/layout/activity_main**을 엽니다. **도구 상자**에서 `ListView` 위젯을 찾아 **Design Surface**끌어 옵니다. 테두리를 선택할 때 테두리를 윤곽선으로 하는 파란색 선을 제외 하 고는 디자이너의 `ListView` 비어 있게 됩니다. **문서 개요** 를 보고 **ListView** 가 올바르게 추가 되었는지 확인할 수 있습니다.
...
기본적으로 `ListView`에는 `@+id/listView1`의 `Id` 값이 지정 됩니다.
**문서 개요**에서 `listView1` 계속 선택 된 상태에서 **속성** 창을 열고 **정렬 기준**을 클릭 한 다음 **범주**를 선택 합니다.
**Main**을 열고 **Id** 속성을 찾아 값을 `@+id/myListView`변경 합니다.

[myListView에 id 이름 바꾸기![](designer-walkthrough-images/vs/24-change-id-w158-sml.png)](designer-walkthrough-images/vs/24-change-id-w158.png#lightbox)
...
이 코드는 사용자 지정 `ListView` 어댑터를 사용 하 여 색 정보를 로드 하 고 방금 만든 UI에이 데이터를 표시 합니다. 이 예를 짧게 유지 하기 위해 색 정보는 목록에 하드 코딩 되지만 어댑터는 데이터 원본에서 색 정보를 추출 하거나 즉석에서 계산 하도록 수정할 수 있습니다. `ListView` 어댑터에 대 한 자세한 내용은 [ListView](~/android/user-interface/layouts/list-view/index.md)를 참조 하세요.
...
[![이름 앱](designer-walkthrough-images/xs/02-designer-walkthrough-m75-sml.png)](designer-walkthrough-images/xs/02-designer-walkthrough-m75.png#lightbox)
...
**디자이너 화면**으로 돌아가려면 화면 아래쪽의 **디자이너** 탭을 클릭 합니다. 오른쪽의 **도구 상자** 창에서 **이미지 & 미디어** 섹션까지 아래로 스크롤하고 `ImageView`를 찾습니다.
...
이 `ImageView`를 **Design Surface** 으로 끌어 놓습니다 (이 `ImageView` 색 브라우저 앱에서 색 견본을 표시 하는 데 사용 됨).
...
그런 다음 **도구 상자** 에서 **Design Surface**`LinearLayout (Vertical)` 위젯을 끌어 옵니다. 파란색 윤곽선은 추가 된 `LinearLayout`의 경계를 나타냅니다. **문서 개요** 는 `imageView1 (ImageView)`아래에 있는 `LinearLayout`의 자식 임을 보여 줍니다.
...
그런 다음 **도구 상자** 에서 새로 추가 된 `LinearLayout``Text (Large)` 위젯을 끌어 옵니다. 마우스를 **Design Surface**위로 끌면 새 위젯이 삽입 되는 위치가 강조 표시 됩니다.
`Text (Large)` 위젯은 다음과 같이 `linearLayout1` 내에 위치 해야 합니다.
...
다음으로 `Text (Large)` 위젯 아래에 `Text (Small)` 위젯을 추가 합니다. 이 시점에서 **Design Surface** 는 다음 스크린샷에서와 비슷해야 합니다.
...
두 `textView` 위젯이 `linearLayout1`내에 없는 경우 **문서 개요** 에서 `linearLayout1`으로 끌어 오고 앞의 스크린샷에 표시 된 대로 나타나도록 지정할 수 있습니다 (`linearLayout1`아래에 들여쓰기).

### <a name="arranging-the-user-interface"></a>사용자 인터페이스 정렬

다음 단계는 왼쪽에 `ImageView`을 표시 하도록 UI를 수정 하 고 두 개의 `TextView` 위젯을 `ImageView`오른쪽에 누적 하는 것입니다.

1. `ImageView` 선택 하 고 **속성** 탭을 클릭 합니다.

2. **속성** 탭 바로 아래에서 **레이아웃**을 클릭 합니다.

3. **ViewGroup** 으로 스크롤하고 `Width` 설정을 `wrap_content`로 변경 합니다.

[콘텐츠 줄 바꿈![설정](designer-walkthrough-images/xs/15-wrap-content-m75-sml.png)](designer-walkthrough-images/xs/15-wrap-content-m75.png#lightbox)

`Width` 설정을 변경 하는 또 다른 방법은 위젯의 오른쪽에 있는 삼각형을 클릭 하 여 너비 설정을 `wrap_content`설정/해제 하는 것입니다.

[너비를 설정 하기 위해 끌어서![](designer-walkthrough-images/xs/16-width-arrow-m75-sml.png)](designer-walkthrough-images/xs/16-width-arrow-m75.png#lightbox)

삼각형을 다시 클릭 하면 `match_parent``Width` 설정이 반환 됩니다. 그런 다음 **문서 개요** 창으로 이동 하 여 루트 `LinearLayout`를 선택 합니다.

[![루트 LinearLayout 선택](designer-walkthrough-images/xs/17-root-linearlayout-m75-sml.png)](designer-walkthrough-images/xs/17-root-linearlayout-m75.png#lightbox)

Root `LinearLayout`가 선택 된 상태에서 **속성** 탭으로 돌아가서 **위젯을**클릭 합니다. 아래와 같이 `Orientation` 설정을 `horizontal`로 변경 합니다. 이 시점에서 **Design Surface** 는 다음 스크린샷에서와 비슷해야 합니다. `TextView` 위젯은 `ImageView`오른쪽으로 이동 된 것을 볼 수 있습니다.
...
다음 단계는 위젯 사이에 더 많은 공간을 제공 하도록 UI의 안쪽 여백 및 여백 설정을 수정 하는 것입니다. `ImageView`를 선택 하 고 **속성**아래에서 **레이아웃** 탭을 클릭 합니다. `Min Width`을 `50dp`, `70dp``Min Height`, `Padding`으로 변경 합니다.
...
위쪽, 오른쪽, 아래쪽 및 왼쪽 안쪽 여백 설정은 각각 `Top`, `Right`, `Bottom`및 `Left` 패딩 필드에 값을 입력 하 여 개별적으로 설정할 수 있습니다. 예를 들어 `Left` 패딩 값을 `5dp`로 설정 하 고 `Top`, `Right`및 `Bottom` 패딩 값을 `10dp`로 설정 합니다. `Padding` 설정은 다음 값의 쉼표로 구분 된 목록으로 변경 됩니다.
...
다음으로 두 개의 `TextView` 위젯을 포함 하는 `LinearLayout` 위젯의 위치를 조정 합니다. **문서 개요**에서 `linearLayout1`를 선택 합니다. **속성** 창에서 **레이아웃** 탭을 선택 합니다. **ViewGroup** 섹션까지 아래로 스크롤하고 `Left`, `Top`, `Right`및 `Bottom` 여백을 각각 `5dp`, `5dp`, `0dp`및 `5dp`로 설정 합니다. :
...
3. `Src` 설정이 비어 있도록 확인란의 선택을 취소 합니다.

[![ImageView src 설정의 선택을 취소 합니다.](designer-walkthrough-images/xs/23-clear-src-m75-sml.png)](designer-walkthrough-images/xs/23-clear-src-m75.png#lightbox)
...
테두리를 선택할 때 테두리를 윤곽선으로 하는 파란색 선을 제외 하 고는 디자이너의 `ListView` 비어 있게 됩니다. **문서 개요** 를 보고 **ListView** 가 올바르게 추가 되었는지 확인할 수 있습니다.
...
[myListView에 id 이름 바꾸기![](designer-walkthrough-images/xs/25-change-id-m75-sml.png)](designer-walkthrough-images/xs/25-change-id-m75.png#lightbox)
