...
description: 이 문서에서는 컴퓨터의 하드웨어 가속 기능을 사용하여 Google Android Emulator 성능을 최대화하는 방법을 설명합니다.
ms.prod: xamarin
...
ms.locfileid: "34806818"
---
# <a name="hardware-acceleration-for-emulator-performance"></a>에뮬레이터 성능에 대한 하드웨어 가속

_이 문서에서는 컴퓨터의 하드웨어 가속 기능을 사용하여 Google Android Emulator 성능을 최대화하는 방법을 설명합니다._

## <a name="overview"></a>개요

Visual Studio를 통해 Android 장치가 지원되지 않거나 비현실적인 경우 Google Android Emulator를 사용하여 개발자가 Xamarin.Android 응용 프로그램을 쉽게 테스트하고 디버그할 수 있습니다.
...
1. **Microsoft의 Hyper-V 및 하이퍼바이저 플랫폼**. Hyper-V는 물리적 호스트 컴퓨터에서 가상화된 컴퓨터 시스템을 실행할 수 있게 해 주는 Windows의 가상화 기능입니다. Google Android Emulator를 가속화하는 데 사용하는 권장된 가상화 기술입니다. Hyper-V에 대해 자세히 알아보려면 [Windows 10에서 Hyper-V](https://docs.microsoft.com/en-us/virtualization/hyper-v-on-windows/)를 참조하세요.
...
Google Android Emulator는 다음 기준이 충족되는 경우 자동으로 하드웨어 가속을 사용합니다.

-   개발 컴퓨터에서 하드웨어 가속을 사용하고 활성화할 수 있습니다.

-   에뮬레이터는 **x86** 기반 가상 장치를 위해 특별히 만들어진 에뮬레이터 이미지를 실행 중입니다.

Android Emulator 시작 및 디버깅에 대한 자세한 내용은 [Google Android Emulator를 사용하여 디버깅](~/android/deploy-test/debugging/android-sdk-emulator/index.md)을 참조하세요.
...
Windows 10을 사용하는 개발자(2018년 4월 업데이트 이상)는 Microsoft의 Hyper-V를 사용하여 Google Android Emulator를 가속화하는 것이 좋습니다. Hyper-V에서 Google Android Emulator를 사용하려면:
...
3. **[Visual Studio 15.8 미리 보기 1 이상](https://www.visualstudio.com/vs/preview/)을 설치합니다**.
   이 버전의 Visual Studio에서는 Hyper-V에서 Google Android Emulator를 실행하기 위해 IDE 지원을 제공합니다.
 
4. **Google Android 에뮬레이터 패키지 27.2.7 이상을 설치합니다**. 이 패키지를 설치하려면 Visual Studio에서 **도구 > Android > Android SDK Manager**로 이동합니다. **도구** 탭을 선택하고 Android Emulator 버전이 적어도 27.2.7인지 확인합니다. 또한 Android SDK Tools 버전이 26.1.1 이상인지 확인합니다.
...
VT 기능이 있는 Intel CPU가 탑재된 컴퓨터에서 개발 중인 경우 HAXM을 활용하여 Google Android Emulator의 속도를 크게 높일 수 있습니다(사용 중인 CPU가 VT를 지원하는지 확인하려면 [프로세서가 Intel 가상화 기술을 지원하나요?](https://www.intel.com/content/www/us/en/support/processors/000005486.html) 참조).
...
Google Android Emulator를 처음으로 사용하기 전에 HAXM이 설치되어 있고 사용할 Google Android Emulator에서 사용할 수 있는지 확인하는 것이 좋습니다.
