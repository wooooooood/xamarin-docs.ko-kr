...
최신 모바일 장치에는 카메라, 음악 플레이어, 비디오 레코더 등의 전용 장비 &ndash; 장치를 필요로 하는 기능이 도입 되었습니다. 이로 인해 멀티미디어 프레임 워크는 모바일 Api의 최고 수준의 기능이 됩니다.

Android는 멀티미디어를 광범위 하 게 지원 합니다. 이 문서에서는 Android에서 오디오 작업을 검토 하 고 다음 항목에 대해 설명 합니다.

1. **MediaPlayer로 오디오 재생** &ndash; 기본 제공 `MediaPlayer` 클래스를 사용 하 여 `AudioTrack` 클래스를 사용 하 여 로컬 오디오 파일 및 스트리밍된 오디오 파일을 포함 하 여 오디오를 재생 합니다.

2. 오디오 녹음 &ndash; 기본 제공 `MediaRecorder` 클래스를 사용 하 여 **오디오를 기록** 합니다.

3. 오디오 **알림을** 사용 &ndash; 오디오 알림을 사용 하 여 오디오 출력을 일시 중단 하거나 취소 하 여 이벤트 (예: 수신 전화 통화)에 올바르게 응답 하는 잘 작동 하는 응용 프로그램을 만듭니다.

4. 메모리 버퍼에 직접 기록 하 여 `AudioTrack` 클래스를 사용 하 여 **하위 수준 오디오** &ndash; 오디오 재생 `AudioRecord` 클래스를 사용 하 여 오디오를 기록 하 고 메모리 버퍼에서 직접 읽습니다.

## <a name="requirements"></a>요구 사항

이 가이드를 사용 하려면 Android 2.0 (API 레벨 5) 이상이 필요 합니다. Android에서 오디오 디버깅은 장치에서 수행 해야 합니다.

**Androidmanifest**에서 `RECORD_AUDIO` 권한을 요청 해야 합니다.

![녹화\_오디오를 사용 하도록 설정 된 Android 매니페스트의 필수 사용 권한 섹션](android-audio-images/image01.png)
...
`MediaPlayer`는 파일 경로를 전달 하 여 로컬 또는 원격 파일을 재생할 수 있습니다. 그러나 `MediaPlayer`는 상태를 구분 하 고 잘못 된 상태에서 해당 메서드 중 하나를 호출 하면 예외가 throw 됩니다. 오류를 방지 하려면 아래에 설명 된 순서 대로 `MediaPlayer`와 상호 작용 하는 것이 중요 합니다.

### <a name="initializing-and-playing"></a>초기화 및 재생

`MediaPlayer`를 사용 하 여 오디오를 재생 하려면 다음 시퀀스가 필요 합니다.
...
Android에서 오디오를 기록 하는 `MediaPlayer` 필연적인 결과로는 [Mediarecorder](xref:Android.Media.MediaRecorder) 클래스입니다. `MediaPlayer`와 마찬가지로 상태를 구분 하 고 여러 상태를 전환 하 여 기록을 시작할 수 있는 지점으로 이동 합니다. 오디오를 기록 하려면 `RECORD_AUDIO` 권한을 설정 해야 합니다. 응용 프로그램 사용 권한을 설정 하는 방법에 대 한 지침은 [AndroidManifest 작업](~/android/platform/android-manifest.md)을 참조 하세요.

### <a name="initializing-and-recording"></a>초기화 및 기록

`MediaRecorder`를 사용 하 여 오디오를 기록 하려면 다음 단계를 수행 해야 합니다.
...
기록을 중지 하려면 `MediaRecorder`에서 `Stop` 메서드를 호출 합니다.
...
`MediaRecorder` 중지 된 후 [Reset](xref:Android.Media.MediaRecorder.Reset) 메서드를 호출 하 여 다시 유휴 상태로 전환 합니다.

```csharp
recorder.Reset();
```

`MediaRecorder` 더 이상 필요 하지 않은 경우 [릴리스](xref:Android.Media.MediaRecorder.Release) 메서드를 호출 하 여 해당 리소스를 해제 해야 합니다.
...
오디오 [관리자](xref:Android.Media.AudioManager) 클래스는 오디오 이벤트가 발생 하는 경우 응용 프로그램에서 알 수 있도록 오디오 알림에 대 한 액세스를 제공 합니다. 이 서비스는 볼륨 및 벨소리 모드 제어와 같은 다른 오디오 기능에도 액세스할 수 있도록 합니다. `AudioManager`를 사용 하면 응용 프로그램에서 오디오 재생을 제어 하는 오디오 알림을 처리할 수 있습니다.
...
오디오 포커스는 비슷한 개념으로, 둘 이상의 응용 프로그램이 동시에 오디오를 재생 하거나 기록 하는 것을 방지 합니다. 이는 자발적 &ndash;이는 자발적 이기 때문에 응용 프로그램에서 현재 오디오 포커스가 없고 &ndash;에 관계 없이 재생 되 고, 요청 될 수 있는 오디오 포커스가 서로 다른 형식 이기 때문에 응용 프로그램에서이를 무시할 수 있다는 사실을 무시 합니다. 예를 들어 요청자는 매우 짧은 시간 동안만 오디오를 재생 하는 것으로 예상 되는 경우 일시적인 포커스를 요청할 수 있습니다.
...
`IOnAudioChangeListener`에서 `FocusChangeListener` 콜백을 등록 하는 것은 오디오 포커스를 가져오고 해제 하는 것이 중요 한 부분입니다. 그 이유는 오디오 포커스가 이후 시간까지 지연 될 수 있기 때문입니다. 예를 들어, 응용 프로그램에서 전화 통화를 진행 하는 동안 음악 재생을 요청할 수 있습니다. 전화 호출이 완료 될 때까지 오디오 포커스가 부여 되지 않습니다.

이러한 이유로 콜백 개체는 `AudioManager`의 `GetAudioFocus` 메서드에 매개 변수로 전달 되며이 호출은 콜백을 등록 합니다. 처음에 오디오 포커스가 거부 되었지만 나중에 부여 된 경우 콜백에 대 한 `OnAudioFocusChange`를 호출 하 여 응용 프로그램에 알립니다. 응용 프로그램에 오디오 포커스를 사용 하 고 있음을 알리기 위해 동일한 메서드가 사용 됩니다.

응용 프로그램이 오디오 리소스를 사용 하 여 완료 되 면 `AudioManager`의 `AbandonFocus` 메서드를 호출 하 고 콜백을 다시 전달 합니다. 이렇게 하면 콜백이 등록 취소 오디오 리소스가 해제 되어 다른 응용 프로그램이 오디오 포커스를 받을 수 있습니다.
...
3. `AudioManager`에서 `RequestAudioFocus` 메서드를 호출 하 여 장치의 오디오 리소스를 요청 합니다. 매개 변수는 콜백 개체, 스트림 형식 (음악, 음성 통화, 링 등) 및 요청 된 액세스 권한 유형 (예: 일시적으로 또는 무기한으로 오디오 리소스를 요청할 수 있음)입니다.

4. 요청이 부여 되 면 `playMusic` 메서드가 즉시 호출 되며 오디오가 재생 되기 시작 합니다.
...
트랙 재생이 완료 되 면 `AudioManager`에서 `AbandonFocus` 메서드가 호출 됩니다. 이렇게 하면 다른 응용 프로그램에서 장치의 오디오 리소스를 얻을 수 있습니다. 다른 응용 프로그램은 자체 수신기를 등록 한 경우이 오디오 포커스 변경에 대 한 알림을 받게 됩니다.
...
오디오 [트랙](xref:Android.Media.AudioTrack) 클래스는 기록에 하위 수준 오디오 api를 사용 하며 `MediaPlayer` 클래스의 하위 수준에 해당 합니다.

#### <a name="initializing-and-playing"></a>초기화 및 재생

오디오를 재생 하려면 `AudioTrack`의 새 인스턴스를 인스턴스화해야 합니다. [생성자](xref:Android.Media.AudioTrack) 에 전달 된 인수 목록은 버퍼에 포함 된 오디오 샘플을 재생 하는 방법을 지정 합니다. 인수는 다음과 같습니다.

1. 스트림 유형 &ndash; 음성, 벨 소리, 음악, 시스템 또는 경보입니다.

2. 빈도 &ndash; Hz로 표시 되는 샘플링 률입니다.

3. 채널 구성 &ndash; Mono 또는 스테레오입니다.

4. 오디오 형식 &ndash; 8 비트 또는 16 비트 인코딩입니다.

5. 버퍼 크기가 &ndash; 바이트입니다.

6. 버퍼 모드 &ndash; 스트리밍 또는 정적입니다.

생성 후 재생을 시작 하도록 설정 하려면 `AudioTrack`의 [Play](xref:Android.Media.AudioTrack.Play) 메서드를 호출 합니다. 오디오 버퍼를 `AudioTrack`에 쓰면 재생이 시작 됩니다.
...
`AudioTrack` 더 이상 필요 하지 않은 경우 [릴리스](xref:Android.Media.AudioTrack.Release)를 호출 하 여 해당 리소스를 해제 해야 합니다.
...
[AudioRecord](xref:Android.Media.AudioRecord) 클래스는 기록 측의 `AudioTrack`와 동일 합니다. `AudioTrack`와 마찬가지로 메모리 버퍼는 파일 및 Uri 대신 직접 사용 됩니다. 매니페스트에 `RECORD_AUDIO` 권한을 설정 해야 합니다.

#### <a name="initializing-and-recording"></a>초기화 및 기록

첫 번째 단계는 새 [AudioRecord](xref:Android.Media.AudioRecord) 개체를 생성 하는 것입니다. [생성자](xref:Android.Media.AudioRecord) 에 전달 되는 인수 목록은 기록에 필요한 모든 정보를 제공 합니다. 인수가 주로 열거 되는 `AudioTrack`와 달리 `AudioRecord`의 해당 인수는 정수입니다. 여기에는 다음이 포함됩니다.

1. 마이크와 같은 하드웨어 오디오 입력 원본입니다.

2. 스트림 유형 &ndash; 음성, 벨 소리, 음악, 시스템 또는 경보입니다.

3. 빈도 &ndash; Hz로 표시 되는 샘플링 률입니다.

4. 채널 구성 &ndash; Mono 또는 스테레오입니다.

5. 오디오 형식 &ndash; 8 비트 또는 16 비트 인코딩입니다.

6. 버퍼 크기 (바이트)

`AudioRecord` 생성 되 면 해당 [Startrecording](xref:Android.Media.AudioRecord.StartRecording) 메서드가 호출 됩니다. 이제 기록을 시작할 준비가 되었습니다. `AudioRecord`는 입력에 대 한 오디오 버퍼를 지속적으로 읽고 오디오 파일에이 입력을 씁니다.
...
Android OS는 오디오를 재생, 기록 및 관리 하기 위한 강력한 프레임 워크를 제공 합니다. 이 문서에서는 상위 수준 `MediaPlayer` 및 `MediaRecorder` 클래스를 사용 하 여 오디오를 재생 하 고 기록 하는 방법에 대해 설명 했습니다. 다음으로 오디오 알림을 사용 하 여 서로 다른 응용 프로그램 간에 장치의 오디오 리소스를 공유 하는 방법을 살펴보았습니다. 마지막으로 메모리 버퍼와 직접 상호 작용 하는 하위 수준 Api를 사용 하 여 오디오를 재생 하 고 기록 하는 방법을 처리 했습니다.
