...
---
# <a name="broadcast-receiver-in-xamarinandroid"></a>Xamarin.Android에 브로드캐스트 수신기
...
Android에는 두 가지 범주의 브로드캐스트 식별합니다.

* **일반 브로드캐스트** &ndash; 일반 브로드캐스트는 정해진 순서 없이에 등록 된 모든 브로드캐스트 수신기로 라우트 되도록 합니다. 각 수신자 순서 의도 받게 됩니다. 
* **브로드캐스트 정렬** &ndash; 순서가 지정 된 브로드캐스트 한 번에 하나씩 등록 된 수신기에 전달 됩니다. 의도 받으면 브로드캐스트 수신기 의도 수정 하거나 브로드캐스트 종료할 수 있습니다.

브로드캐스트 수신기가의 서브 클래스는 `BroadcastReceiver` 클래스에서 재정의 해야 합니다는 [ `OnReceive` ](https://developer.xamarin.com/api/member/Android.Content.BroadcastReceiver.OnReceive/p/Android.Content.Context/Android.Content.Intent/) 메서드. Android에서 실행 될 `OnReceive` 주 스레드에서 하므로이 메서드를 디자인 해야 신속 하 게 실행 합니다. 스레드를 생성할 때 주의 해야 `OnReceive` 하므로 Android 메서드가 종료 되는 프로세스를 종료할 수 있습니다. 브로드캐스트 수신기 장기 실행 작업을 수행 해야 하는 경우 다음을 예약 하는 것이 좋습니다.는 _작업_ 를 사용 하는 `JobScheduler` 또는 _Firebase 작업 발송자_합니다. 별도 지침에는 작업과 작업 일정을 설명 합니다.

_의도 필터_ Android 메시지를 올바르게 라우팅할 수 있도록 브로드캐스트 수신기를 등록 하는 데 사용 됩니다. 런타임 시 의도 필터를 지정할 수 있습니다 (이 경우에 따라 라고는 _컨텍스트 등록 수신기_ 또는 as _동적 등록_) Android 매니페스트 (한 에정적으로정의할수있습니다_수신기 매니페스트에 등록_). Xamarin.Android 제공 C# 특성 `IntentFilterAttribute`, (이 살펴봅니다이 가이드의 뒷부분에 보다 자세히) 의도 필터 등록 하는 정적으로 합니다. 
...
브로드캐스트를 캡슐화 하 여 게시 된는 _동작_ 프로그램 의도 및 디스패치 사용 하 여 두 개의 Api 중 하나: 

1. **[`LocalBroadcastManager`](https://developer.android.com/reference/android/support/v4/content/LocalBroadcastManager.html#sendBroadcast(android.content.Intent))** &ndash; 과 함께 게시 의도 `LocalBroadcastManager` 응용 프로그램에서 수신만 됩니다 다른 응용 프로그램으로 라우팅 하지 것입니다. 이 현재 응용 프로그램 내에서 의도 유지 하 여 추가 수준의 보안을 제공에 연결 된 프로세스 간 호출을 수행 하는 오버 헤드 없음이 프로세스에서 모든 항목 이므로 있으므로 기본 이어야 합니다. 이 코드 조각은 표시 방법을 사용 하는 의도 활동을 발송할 수 있습니다는 `LocalBroadcastManager`:
...
2. **[`Context.SendBroadcast`](https://developer.xamarin.com/api/member/Android.Content.Context.SendBroadcast/p/Android.Content.Intent/) 메서드** &ndash; 이 방법의 몇 가지 구현이 있습니다.
   이러한 메서드는 전체 시스템에 의도 브로드캐스트 됩니다. 이 상당한 수준의 유연성을 제공 하지만 다른 응용 프로그램 응용 프로그램을 수신 하도록 등록할 수를 의미 합니다. 이 잠재적 보안 위험이 발생할 수 있습니다. 응용 프로그램 의도에 대 한 무단된 액세스를 보장 하도록 추가 보안을 구현 해야 합니다. 이 코드 조각 중 하나를 사용 하 여 의도 디스패치 하는 방법의 한 가지 예는는 `SendBroadcast` 메서드:
...
> [!NOTE]
> 통해 제공 되는 LocalBroadcastManager는 [Xamarin 지원 라이브러리 v4](https://www.nuget.org/packages/Xamarin.Android.Support.v4/) NuGet 패키지 합니다. 
