...
만들려는 글꼴 패밀리에 있는 모든 글꼴을 먼저 추가 **리소스/글꼴** 폴더. 그런 다음 글꼴 패밀리에 대 한 글꼴 폴더에 새 XML 파일을 만듭니다. 이 XML 파일에는 루트 갖습니다 `font-family` 하나 이상 포함 된 요소 `font` 요소입니다. 각 `font` 요소 글꼴의 특성을 선언 합니다. 
