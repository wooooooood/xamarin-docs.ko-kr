...
다음 예에서는 패키지 이름이 `com.xamarin.fcmexample`로 설정 됩니다.

[패키지 이름을 설정 하는![](remote-notifications-with-fcm-images/01-package-name-vs-sml.png)](remote-notifications-with-fcm-images/01-package-name-vs.png#lightbox)

**Android 매니페스트**를 업데이트 하는 동안 `Internet` 권한이 설정 되었는지 확인 합니다.
...
[패키지 이름을 설정 하는![](remote-notifications-with-fcm-images/01-package-name-xs-sml.png)](remote-notifications-with-fcm-images/01-package-name-xs.png#lightbox)

**Android 매니페스트**를 업데이트 하는 동안 `INTERNET` 권한이 설정 되어 있는지 확인 합니다 ( **필수 권한**아래).
...
이 문은 **xamarin.googleplayservices.base** 의 `GoogleApiAvailability` 클래스를 **fcmclient** 코드에서 사용할 수 있도록 만듭니다.
`GoogleApiAvailability`은 Google Play 서비스 있는지 확인 하는 데 사용 됩니다.
...
4. **속성** 창에서 **빌드 작업** 을 **GoogleServicesJson**로 설정 합니다.
    ...
    [빌드 작업을 GoogleServicesJson 설정 하는![](remote-notifications-with-fcm-images/04-google-services-json-vs-sml.png)](remote-notifications-with-fcm-images/04-google-services-json-vs.png#lightbox)
...
4. **빌드 작업** 을 **GoogleServicesJson**로 설정 합니다.
    ...
    [빌드 작업을 GoogleServicesJson 설정 하는![](remote-notifications-with-fcm-images/04-google-services-json-xs-sml.png)](remote-notifications-with-fcm-images/04-google-services-json-xs.png#lightbox)
...
이 `TextView`는 Google Play 서비스 설치 여부를 나타내는 메시지를 표시 하는 데 사용 됩니다. **주. axml**에 변경 내용을 저장 합니다.
...
`CHANNEL_ID` 및 `NOTIFICATION_ID` 변수는이 연습의 뒷부분에서 `MainActivity`에 추가 되는 메서드 [`CreateNotificationChannel`](#create-notification-channel-code) 에 사용 됩니다.

다음 예제에서 `OnCreate` 메서드는 앱이 FCM Services를 사용 하기 전에 Google Play 서비스를 사용할 수 있는지 확인 합니다.
...
이 코드는 장치에서 Google Play 서비스 APK가 설치 되어 있는지 확인 합니다. 설치 되지 않은 경우 사용자가 Google Play 스토어에서 APK를 다운로드 하거나 장치의 시스템 설정에서 사용 하도록 지시 하는 메시지가 `TextBox`에 표시 됩니다.

<a name="create-notification-channel-code"></a>Android 8.0 (API 레벨 26) 이상에서 실행 되는 앱은 알림을 게시 하기 위한 [_알림 채널_](~/android/app-fundamentals/notifications/local-notifications.md) 을 만들어야 합니다.  `MainActivity` 클래스에 다음 메서드를 추가 하 여 알림 채널을 만듭니다 (필요한 경우).
...
앱이 시작 될 때마다 Google Play 서비스 확인이 실행 되도록 `OnCreate` 끝에 `IsPlayServicesAvailable`가 호출 됩니다. `CreateNotificationChannel` 메서드는 Android 8 이상을 실행 하는 장치에 대 한 알림 채널이 존재 하도록 호출 됩니다. 앱에 `OnResume` 메서드가 있는 경우에도 `OnResume`에서 `IsPlayServicesAvailable`를 호출 해야 합니다. 앱을 완전히 다시 빌드하고 실행 합니다. 모든 항목이 올바르게 구성 된 경우 다음과 같은 화면이 표시 됩니다.
...
다음 단계는 [Firebase 등록 토큰](~/android/data-cloud/google-messaging/firebase-cloud-messaging.md#fcm-in-action-registration-token)의 생성, 회전 및 업데이트를 처리 하도록 `FirebaseInstanceIdService`를 확장 하는 서비스를 추가 하는 것입니다. `FirebaseInstanceIdService` 서비스는 FCM가 장치에 메시지를 보낼 수 있도록 하는 데 필요 합니다. `FirebaseInstanceIdService` 서비스가 클라이언트 앱에 추가 되 면 앱은 자동으로 FCM 메시지를 수신 하 고 앱이 backgrounded 될 때마다 알림으로 표시 합니다.

### <a name="declare-the-receiver-in-the-android-manifest"></a>Android 매니페스트에서 수신기를 선언 합니다.

**Androidmanifest .xml** 을 편집 하 고 다음 `<receiver>` 요소를 `<application>` 섹션에 삽입 합니다.
...
- 각 응용 프로그램 인스턴스에 대 한 [고유 식별자](https://developers.google.com/instance-id/) 를 제공 하는 `FirebaseInstanceIdReceiver` 구현을 선언 합니다. 이 수신기는 또한 작업을 인증 하 고 권한을 부여 합니다.

- 서비스를 안전 하 게 시작 하는 데 사용 되는 내부 `FirebaseInstanceIdInternalReceiver` 구현을 선언 합니다.

- [앱 ID](./firebase-cloud-messaging.md#fcm-in-action-app-id) 는 [프로젝트에 추가](#add-googleplayservices-json)된 **google 서비스의 json** 파일에 저장 됩니다. Firebase 바인딩은 토큰 `${applicationId}`를 앱 ID로 바꿉니다. 클라이언트 앱에서 앱 ID를 제공 하는 데 필요한 추가 코드는 없습니다.

`FirebaseInstanceIdReceiver`은 `FirebaseInstanceId` 및 `FirebaseMessaging` 이벤트를 수신 하 고 `FirebaseInstanceIdService`에서 파생 된 클래스에 전달 하는 `WakefulBroadcastReceiver`입니다.

### <a name="implement-the-firebase-instance-id-service"></a>Firebase Instance ID 서비스 구현

FCM를 사용 하 여 응용 프로그램을 등록 하는 작업은 사용자가 제공 하는 사용자 지정 `FirebaseInstanceIdService` 서비스에 의해 처리 됩니다.
...
이 서비스는 등록 토큰을 처음 생성 하거나 변경할 때 호출 되는 `OnTokenRefresh` 메서드를 구현 합니다. `OnTokenRefresh` 실행 되는 경우 `FirebaseInstanceId.Instance.Token` 속성에서 최신 토큰을 검색 합니다 .이 토큰은 FCM에서 비동기적으로 업데이트 됩니다. 이 예제에서는 출력 창에서 볼 수 있도록 새로 고쳐진 토큰이 기록 됩니다.
...
또한 `OnTokenRefresh` `SendRegistrationToAppServer`를 호출 하 여 사용자의 등록 토큰을 응용 프로그램에서 유지 관리 되는 서버 쪽 계정 (있는 경우)에 연결 합니다.
...
이 구현은 앱 서버 디자인에 따라 달라 지므로이 예제에서는 빈 메서드 본문을 제공 합니다. 앱 서버에서 FCM 등록 정보를 요구 하는 경우 `SendRegistrationToAppServer`를 수정 하 여 사용자의 FCM 인스턴스 ID 토큰을 앱에서 유지 관리 되는 서버 쪽 계정에 연결 합니다. 토큰은 클라이언트 앱에 불투명 합니다.

토큰이 앱 서버에 전송 되 면 토큰이 서버에 전송 되었는지 여부를 나타내는 부울을 유지 해야 `SendRegistrationToAppServer`. 이 부울이 false 인 경우 `SendRegistrationToAppServer`는 앱 &ndash; 서버에 토큰을 보내고, 그렇지 않으면 이전 호출에서 토큰을 이미 앱 서버에 보냈습니다. 경우에 따라 (예:이 `FCMClient` 예제) 앱 서버에는 토큰이 필요 하지 않습니다. 따라서이 예제에서는이 메서드가 필요 하지 않습니다.

## <a name="implement-client-app-code"></a>클라이언트 앱 코드 구현

이제 수신기 서비스가 준비 되었으므로 이러한 서비스를 활용 하도록 클라이언트 앱 코드를 작성할 수 있습니다. 다음 섹션에서는 등록 토큰 ( *인스턴스 ID 토큰*이 라고도 함)을 기록 하는 단추를 UI에 추가 하 고, 알림에서 앱이 시작 될 때 `Intent` 정보를 볼 수 있도록 `MainActivity`에 추가 코드를 추가 합니다.
...
이 단계에서 추가 된 코드는 프로덕션 클라이언트 앱이 등록 토큰을 기록할 필요가 없는 &ndash; 데모용 으로만 사용 됩니다. **리소스/레이아웃/기본. axml** 을 편집 하 고 `TextView` 요소 바로 뒤에 다음 `Button` 선언을 추가 합니다.
...
사용자가 **Fcmclient**에서 발급 된 알림을 탭 하면 해당 알림 메시지와 함께 제공 되는 모든 데이터를 `Intent`에서 사용할 수 있습니다. **MainActivity.cs** 를 편집 하 고 `IsPlayServicesAvailable`를 호출 하기 전에 `OnCreate` 메서드의 맨 위에 다음 코드를 추가 합니다.
...
사용자가 알림 메시지를 탭 하면 응용 프로그램의 시작 관리자 `Intent` 발생 하므로,이 코드는 `Intent`의 해당 데이터를 출력 창에 기록 합니다. 다른 `Intent`를 실행 해야 하는 경우 알림 메시지의 `click_action` 필드를 해당 `Intent`으로 설정 해야 합니다. `click_action` 지정 되지 않은 경우 시작 관리자 `Intent` 사용 됩니다.
...
[![인스턴스 ID 토큰이 출력 창에 표시 됩니다.](remote-notifications-with-fcm-images/07-token-received-sml.png)](remote-notifications-with-fcm-images/07-token-received.png#lightbox)

**토큰** 으로 레이블이 지정 된 긴 문자열은 Firebase 콘솔에 붙여 넣을 인스턴스 ID 토큰으로,이 문자열을 선택 하 고 클립보드에 복사 &ndash; 합니다. 인스턴스 ID 토큰이 표시 되지 않는 경우 `OnCreate` 메서드의 맨 위에 다음 줄을 추가 하 여 **google-json** 이 올바르게 구문 분석 되었는지 확인 합니다.

```csharp
Log.Debug(TAG, "google app id: " + GetString(Resource.String.google_app_id));
```

출력 창에 기록 되는 `google_app_id` 값은 **google-service. json**에 기록 된 `mobilesdk_app_id` 값과 일치 해야 합니다.
...
[메시지 작성 대화![대화 상자](remote-notifications-with-fcm-images/09-compose-message-sml.png)](remote-notifications-with-fcm-images/09-compose-message.png#lightbox)
...
알림 아이콘을 탭 하 여 **Fcmclient** 앱을 시작 합니다. **Fcmclient** 로 전송 되는 `Intent`는 IDE 출력 창에 나열 됩니다.

[키, 메시지 ID 및 축소 키에서![의도 추가 목록](remote-notifications-with-fcm-images/13-intent-extras-sml.png)](remote-notifications-with-fcm-images/13-intent-extras.png#lightbox)

이 예제에서 **from** 키는 앱의 Firebase 프로젝트 번호 (이 예제에서는 `41590732`)로 설정 되 고 collapse_key는 패키지 이름 ( )으로 설정**됩니다.**
메시지가 수신 되지 않으면 장치 (또는 에뮬레이터)에서 **Fcmclient** 앱을 삭제 하 고 위의 단계를 반복 합니다.

> [!NOTE]
> 앱을 강제로 닫으면 FCM에서 알림 배달이 중지 됩니다. Android는 백그라운드 서비스 브로드캐스트가 실수로 중지 된 응용 프로그램의 구성 요소를 시작 하거나 불필요 하 게 시작 하지 않도록 합니다. 이 동작에 대 한 자세한 내용은 중지 된 [응용 프로그램에서 컨트롤 시작](https://developer.android.com/about/versions/android-3.1.html#launchcontrols)을 참조 하세요. 이러한 이유로 응용 프로그램을 실행할 때마다 수동으로 제거 하 고 디버그 &ndash; 세션에서 중지 해야 합니다. 이렇게 하면 FCM가 메시지를 계속 받을 수 있도록 새 토큰을 생성 합니다.
...
사용자 지정 기본 알림 아이콘을 추가 하려면 **리소스/그릴** 수 있는 디렉터리에 아이콘을 추가 하 고 **Androidmanifest .xml**을 편집 하 고 `<application>` 섹션에 다음 `<meta-data>` 요소를 삽입 합니다.
...
이 예제에서는 **리소스/그릴 수 있는/ic\_stat\_ic\_알림에** 있는 알림 아이콘을 사용자 지정 기본 알림 아이콘으로 사용 합니다. 사용자 지정 기본 아이콘이 **Androidmanifest** 에 구성 되어 있지 않고 알림 페이로드에 설정 된 아이콘이 없는 경우 Android는 위의 알림 아이콘 스크린샷에 표시 된 것 처럼 응용 프로그램 아이콘을 알림 아이콘으로 사용 합니다.
...
**리소스/레이아웃/기본. axml** 을 편집 하 고 이전 `Button` 요소 바로 뒤에 다음 `Button` 선언을 추가 합니다.
...
이 코드는 레이아웃에서 **알림 구독** 단추를 찾고 등록 된 항목인 _news_를 전달 하 여 `FirebaseMessaging.Instance.SubscribeToTopic`를 호출 하는 코드에 클릭 처리기를 할당 합니다. 사용자가 **구독** 단추를 누르면 앱이 _news_ 항목을 구독 합니다. 다음 섹션에서는 Firebase Console Notification GUI에서 _news_ 토픽 메시지가 전송 됩니다.
...
3. **항목** 풀 다운 메뉴에서 기본 제공 항목인 **news**:를 선택 합니다.
    ...
    [뉴스 항목을 선택![](remote-notifications-with-fcm-images/16-topic-message-sml.png)](remote-notifications-with-fcm-images/16-topic-message.png#lightbox)
...
[항목 메시지가 알림으로 표시![](remote-notifications-with-fcm-images/18-other-news-sml.png)](remote-notifications-with-fcm-images/18-other-news.png#lightbox)

메시지가 수신 되지 않으면 장치 (또는 에뮬레이터)에서 **Fcmclient** 앱을 삭제 하 고 위의 단계를 반복 합니다.

## <a name="foreground-notifications"></a>포그라운드 알림

포그라운드 apps에서 알림을 받으려면 `FirebaseMessagingService`를 구현 해야 합니다. 이 서비스는 데이터 페이로드를 받고 업스트림 메시지를 전송 하는 데에도 필요 합니다. 다음 예제에서는 `FirebaseMessagingService` 확장 하는 서비스를 구현 하는 방법을 보여 줍니다. 그러면 결과 앱이 포그라운드로 실행 되는 동안 원격 알림을 처리할 수 &ndash;.

### <a name="implement-firebasemessagingservice"></a>FirebaseMessagingService 구현

`FirebaseMessagingService` 서비스는 Firebase에서 메시지를 수신 하 고 처리 하는 일을 담당 합니다. 각 앱은이 형식을 서브클래싱하 고 `OnMessageReceived`를 재정의 하 여 들어오는 메시지를 처리 해야 합니다. 응용 프로그램이 전경에 있으면 `OnMessageReceived` 콜백에서 항상 메시지를 처리 합니다.
...
새 FCM 메시지가 `MyFirebaseMessagingService`으로 전달 되도록 `MESSAGING_EVENT` 의도 필터를 선언 해야 합니다.

```csharp
[IntentFilter(new[] { "com.google.firebase.MESSAGING_EVENT" })]
```

클라이언트 앱은 FCM에서 메시지를 받으면 `GetNotification` 메서드 `OnMessageReceived`를 호출 하 여 전달 된 `RemoteMessage` 개체에서 메시지 콘텐츠를 추출 합니다. 그런 다음 IDE 출력 창에서 볼 수 있도록 메시지 콘텐츠를 기록 합니다.
...
> [!NOTE]
> `FirebaseMessagingService`에서 중단점을 설정 하는 경우 FCM에서 메시지를 배달 하는 방법 때문에 디버깅 세션이 이러한 중단점에 도달 하거나 적중 하지 않을 수 있습니다.
...
4. 응용 프로그램이 포그라운드에서 실행 중인지 확인 하 고 Firebase 콘솔에서 **메시지 보내기** 를 클릭 합니다.
    ...
    [콘솔에서 다른 메시지를 보낼![](remote-notifications-with-fcm-images/19-hello-again-sml.png)](remote-notifications-with-fcm-images/19-hello-again.png#lightbox)
...
이 알림을 백그라운드 알림과 구분 하기 위해이 코드는 응용 프로그램 아이콘과 다른 아이콘을 사용 하 여 알림을 표시 합니다. Ic [\_stat\_ic\_notification. png](remote-notifications-with-fcm-images/ic-stat-ic-notification.png) 를 **리소스/그릴** 수 있는 파일에 추가 하 고 **fcmclient** 프로젝트에 포함 합니다.

`SendNotification` 메서드는 `NotificationCompat.Builder`를 사용 하 여 알림을 만들고 `NotificationManagerCompat` 알림을 시작 하는 데 사용 됩니다. 이 알림은 사용자가 앱을 열고 `messageBody`에 전달 된 문자열의 내용을 볼 수 있도록 하는 `PendingIntent`을 포함 합니다. `NotificationCompat.Builder`에 대 한 자세한 내용은 [로컬 알림](~/android/app-fundamentals/notifications/local-notifications.md)을 참조 하세요.

`OnMessageReceived` 메서드의 끝에 `SendNotification` 메서드를 호출 합니다.
...
이러한 변경의 결과로 앱이 전경에 있는 동안 알림이 수신 될 때마다 `SendNotification` 실행 되 고 알림 영역에 알림이 표시 됩니다.
...
- **데이터** &ndash; 메시지는 `OnMessageReceived`에서 처리 됩니다.
- 알림과 데이터 페이로드가 모두 있는 &ndash; 메시지는 **모두** 시스템 트레이에 배달 됩니다. 앱이 시작 되 면 응용 프로그램을 시작 하는 데 사용 된 `Intent`의 `Extras`에 데이터 페이로드가 표시 됩니다.

이 예제에서 앱이 backgrounded 인 경우 메시지에 데이터 페이로드가 있으면 `SendNotification` 실행 됩니다. 그렇지 않으면이 연습의 앞부분에서 설명한 백그라운드 알림이 시작 됩니다.
...
4. 응용 프로그램이 포그라운드에서 실행 중인지 확인 하 고 Firebase 콘솔에서 **메시지 보내기** 를 클릭 합니다.
    ...
    [포그라운드 메시지를 보내는![](remote-notifications-with-fcm-images/21-console-fg-msg-sml.png)](remote-notifications-with-fcm-images/21-console-fg-msg.png#lightbox)

이번에는 출력 창에 기록 된 메시지도 새 알림에서 패키지 됩니다 &ndash; 응용 프로그램이 포그라운드에서 실행 되는 동안 알림 트레이에 알림 아이콘이 표시 됩니다.
...
[전경 아이콘으로 표시 된![포그라운드 알림](remote-notifications-with-fcm-images/23-foreground-msg-sml.png)](remote-notifications-with-fcm-images/23-foreground-msg.png#lightbox)
