...
**빈 데이터베이스 만들기** &ndash; 호출을 `CreateFile` 유효한 메서드 (ie. 쓰기 가능한) 파일 경로입니다. 여부를이 메서드를 호출 하기 전에 파일이 이미, 그렇지 않으면 (새) 데이터베이스를 이전 버전의 맨 위에 만들어지고 이전 파일에 데이터가 손실 됩니다 확인 해야 합니다.
