...
Xamarin에서 권장 하는 SQLite.NET 라이브러리는 Android 장치의 로컬 SQLite 데이터베이스에서 개체를 쉽게 저장 하 고 검색할 수 있도록 하는 매우 기본적인 ORM입니다. ORM은 SQL 문을 작성 하지 않고도 데이터베이스에서 "개체"를 저장 하 고 검색할 수 있는 API &ndash; 개체 관계형 매핑을 나타냅니다.
...
1. **Using 문을 추가** &ndash; 데이터 액세스가 필요한 C# 파일에 다음 문을 추가 합니다.

    ```csharp
    using SQLite;
    ```

2. **빈 데이터베이스 만들기** &ndash; SQLiteConnection 클래스 생성자의 파일 경로를 전달 하 여 데이터베이스 참조를 만들 수 있습니다. 파일이 이미 있는지 여부를 확인 하지 않아도 됩니다 &ndash; 필요한 경우 자동으로 생성 되 고 그렇지 않으면 기존 데이터베이스 파일이 열립니다. `dbPath` 변수는이 문서의 앞부분에서 설명한 규칙에 따라 결정 해야 합니다.

    ```csharp
    var db = new SQLiteConnection (dbPath);
    ```

3. **데이터 &ndash; 저장** SQLiteConnection 개체를 만든 후 CreateTable와 같이 Insert와 같이 메서드를 호출 하 여 데이터베이스 명령을 실행 합니다.
...
4. **데이터 &ndash; 검색** 하 여 개체 (또는 개체 목록)를 검색 하려면 다음 구문을 사용 합니다.
...
마지막 항목을 사용 하려면 프로젝트에 SQLite를 추가 해야 합니다. SQLite 데이터베이스 테이블은 CREATE TABLE 명령이 아닌 클래스 (`Stock` 클래스)에 특성을 추가 하 여 정의 됩니다.
...
- **[PrimaryKey]** &ndash;이 특성을 정수 속성에 적용 하 여 기본 테이블의 기본 키가 되도록 할 수 있습니다. 복합 기본 키는 지원 되지 않습니다.
...
- **[Ignore]** &ndash; SQLite.NET이 속성을 무시 합니다.
...
- **Insert** &ndash; 데이터베이스에 새 개체를 추가 합니다.

- 기본 키를 사용 하 여 개체를 검색 하려고 시도 하는 **&lt;t&gt;&ndash; 가져옵니다** .

- 테이블 **&lt;t&gt;** &ndash; 테이블의 모든 개체를 반환 합니다.

- **Delete** &ndash;은 해당 기본 키를 사용 하 여 개체를 삭제 합니다.

- **쿼리&lt;t&gt;** &ndash; 여러 행을 개체로 반환 하는 SQL 쿼리를 수행 합니다.

- SQL에서 행을 다시 표시 하지 않을 경우 (예: INSERT, UPDATE 및 DELETE 명령)이 메서드를 사용 하 &ndash;를 **실행** 합니다 (`Query`는 아님).
...
스레딩 모드를 변경 하려면 `SqliteConnection.SetConfig`를 호출 합니다. 예를 들어 다음 코드 줄은 **serialize** 된 모드에 대해 SQLite를 구성 합니다.
...
SQLite의 Android 버전에는 몇 가지 추가 단계가 필요한 제한 사항이 있습니다. `SqliteConnection.SetConfig`에 대 한 호출이 `library used incorrectly`와 같은 SQLite 예외를 생성 하는 경우 다음 해결 방법을 사용 해야 합니다.

1. `sqlite3_shutdown` 및 `sqlite3_initialize` Api를 앱에서 사용할 수 있도록 네이티브 **libsqlite.so** 라이브러리에 연결 합니다.
