...
- Xamarin **android** &ndash; 9.0.0.17 이상 버전을 Visual Studio와 함께 설치 해야 합니다. xamarin은 **.net 작업을 사용 하 여 모바일 개발** 의 일부로 자동으로 설치 됩니다.

- Xamarin Android 9.0을 개발 &ndash;는 **Java 개발자 키트** 는 [JDK 8](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html) 을 요구 합니다 (또는 Microsoft의 [openjdk](~/android/get-started/installation/openjdk.md)배포 미리 보기를 사용해 볼 수 있음). JDK8는 .NET 워크 로드 **를 사용 하 여 모바일 개발** 의 일부로 자동으로 설치 됩니다.

- **Android SDK** &ndash; Android SDK API 28 이상을 Android SDK Manager를 통해 설치 해야 합니다.
...
3. **플랫폼 탭에** **Android SDK Platform 28** 로 나열 된 **Android 원형** SDK 패키지를 설치 합니다 (SDK Manager를 사용 하는 방법에 대 한 자세한 내용은 [Android SDK 설치](~/android/get-started/installation/android-sdk.md)참조).
    ...
    [Android 원형 패키지를 설치 하는![](pie-images/vs/sdk-manager-sml.png)](pie-images/vs/sdk-manager.png#lightbox)
...
- 최신 Android 장치에서 화면 위쪽에 있는 _잘라낸_ 위치와 모양을 찾을 수 있는 Api를 제공 하는 &ndash; **컷아웃 지원 표시**

- **알림** 메시지 &ndash; 알림 메시지는 이제 이미지를 표시할 수 있으며, 대화 참가자를 간소화 하기 위해 새 `Person` 클래스가 사용 됩니다.

- **실내 위치 지정** &ndash; Wifi 왕복 시간 프로토콜에 대 한 플랫폼 지원을 통해 앱이 실내 설정에서 탐색 하는 데 wifi 장치를 사용할 수 있습니다.

- **다중 카메라 지원** &ndash;는 여러 실제 카메라 (예: 이중 전면 및 이중 후면 카메라)에서 동시에 스트림에 액세스 하는 기능을 제공 합니다.
...
[![Android emulator 컷아웃 시뮬레이션](pie-images/02-example-cutout-sml.png)](pie-images/02-example-cutout.png#lightbox)
...
- [LayoutInDisplayCutoutModeNever](https://developer.android.com/reference/android/view/WindowManager.LayoutParams.html#LAYOUT_IN_DISPLAY_CUTOUT_MODE_NEVER) &ndash; 창이 잘라낸 영역과 겹칠 수 없습니다.

- [LayoutInDisplayCutoutModeShortEdges](https://developer.android.com/reference/android/view/WindowManager.LayoutParams.html#LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES) &ndash; 창이 오려내기 영역으로 확장 될 수 있지만 화면의 짧은 가장자리 에서만 가능 합니다. 

- [LayoutInDisplayCutoutModeDefault](https://developer.android.com/reference/android/view/WindowManager.LayoutParams.html#LAYOUT_IN_DISPLAY_CUTOUT_MODE_DEFAULT) 는 시스템 표시줄 내에 잘라낸 부분을 포함 하는 경우 창을 오려내기 영역으로 확장할 수 &ndash;.
...
다음 예에서는 이러한 컷아웃 모드의 예제를 제공 합니다. 왼쪽의 첫 번째 스크린샷은 비 전체 화면 모드의 앱입니다. 가운데 스크린샷에서 앱은 `LayoutInDisplayCutoutMode` `LayoutInDisplayCutoutModeShortEdges`으로 설정 된 전체 화면으로 이동 합니다. 앱의 흰색 배경은 표시 컷아웃 영역으로 확장 됩니다.

[![예제 에뮬레이터에서 컷아웃 모드 표시](pie-images/03-cutout-modes-sml.png)](pie-images/03-cutout-modes.png#lightbox)

마지막 스크린샷 (오른쪽 위의)에서 `LayoutInDisplayCutoutMode`은 전체 화면으로 이동 하기 전에 `LayoutInDisplayCutoutModeShortNever`로 설정 됩니다.
앱의 흰색 배경은 표시 컷아웃 영역으로 확장할 수 없습니다.

장치의 오려내기 영역에 대 한 자세한 정보가 필요한 경우 새 [Displaycutout](https://developer.android.com/reference/android/view/DisplayCutout.html) 클래스를 사용할 수 있습니다. `DisplayCutout`는 콘텐츠를 표시 하는 데 사용할 수 없는 디스플레이 영역을 나타냅니다. 이 정보를 사용 하 여 앱이 기능을 사용 하지 않는 영역에 콘텐츠를 표시 하지 않도록 잘라낸 위치와 셰이프를 검색할 수 있습니다.
...
1. 보낸 사람을 나타내는 `Person` 개체를 만듭니다. 예를 들어 발신자의 이름과 아이콘이 `fromPerson`에 포함 되어 있습니다.
...
2. 보낼 이미지를 포함 하는 `Notification.MessagingStyle.Message`를 만들어 이미지를 새 [Notification. MessagingStyle. Message. SetData](https://developer.android.com/reference/android/app/Notification.MessagingStyle.Message.html#setData%28java.lang.String,%20android.net.Uri) 메서드로 전달 합니다.
   예를 들면,
...
3. `Notification.MessagingStyle` 개체에 메시지를 추가 합니다. 예를 들면,
...
- `BitmapFactory`를 대체 하는 새 [ImageDecoder](https://developer.android.com/reference/android/graphics/ImageDecoder.html) 클래스입니다. `ImageDecoder`를 사용 하 여 `AnimatedImageDrawable`를 디코딩할 수 있습니다.
