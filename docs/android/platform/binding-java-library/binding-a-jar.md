...
이 가이드에서는 단일에 대 한 바인딩 라이브러리를 만드는 기본 사항을 단계별로 설명 합니다. JAR 파일. 사용자 지정 또는 바인딩 디버깅이 필요 하지 않은 모든 항목을 오른쪽 &ndash;로 이동 하는 예를 보여 드리겠습니다. 
...
다음 연습에서는 인기 있는 Android 인 [Picasso](https://square.github.io/picasso/)에 대 한 바인딩 라이브러리를 만듭니다. 이미지 로드 및 캐싱 기능을 제공 하는 JAR입니다. **Picasso-2** 를 바인딩하려면 다음 단계를 사용 하 여 xamarin.ios 프로젝트에서 사용할 수 있는 새 .net 어셈블리를 만듭니다. 
...
**Picasso-2**에 대 한 바인딩 라이브러리를 생성 한 후에서 C#이 메서드를 호출할 수 있습니다. 예를 들면,
...
[jar 파일![선택 하 고 추가를 클릭 합니다.](binding-a-jar-images/03-select-jar-file-sml.png)](binding-a-jar-images/03-select-jar-file.png#lightbox)
...
일반적으로 **EmbeddedJar** 빌드 작업을 사용 하 여를 만듭니다. JAR은 자동으로 바인딩 라이브러리에 패키지 됩니다. 이는에서 Java 바이트 코드 &ndash; 가장 간단한 옵션입니다. JAR은 Dex 바이트 코드로 변환 되며 관리 되는 호출 가능 래퍼와 함께 APK에 포함 됩니다. 을 유지 하려는 경우 JAR-바인딩 라이브러리와는 별도로 **inputjar** 옵션을 사용할 수 있습니다. 그러나를 확인 해야 합니다. 앱을 실행 하는 장치에서 JAR 파일을 사용할 수 있습니다. 
...
[API 19로 설정![대상 API 수준](binding-a-jar-images/06-set-target-framework-sml.png)](binding-a-jar-images/06-set-target-framework.png#lightbox)

마지막으로 바인딩 라이브러리를 빌드합니다. 일부 경고 메시지가 표시 될 수 있지만 바인딩 라이브러리 프로젝트를 성공적으로 빌드하고 출력을 생성 해야 합니다. 다음 위치에 있는 DLL: **JarBinding/bin/Debug/JarBinding**
...
다음 단계에서는 바인딩 라이브러리를 사용 하 여 `ImageView`에서 이미지를 다운로드 하 고 표시 하는 최소 앱을 만듭니다. "과도 한"는에 있는 코드에 의해 수행 됩니다. JAR 파일. 
...
[![오른쪽 추가 참조](binding-a-jar-images/08-add-reference.png)](binding-a-jar-images/08-add-reference.png#lightbox)
...
**MainActivity.cs** &ndash;에 다음 `using` 문을 추가 합니다. 이렇게 하면 바인딩 라이브러리에 있는 Java 기반 `Picasso` 클래스의 메서드에 쉽게 액세스할 수 있습니다.

```csharp
using Com.Squareup.Picasso;
```

`Picasso` 클래스를 사용 하 여 URL에서 이미지를 로드 하 고 `ImageView`에 표시 하도록 `OnCreate` 메서드를 수정 합니다. 
...
지금까지 Java 라이브러리를 성공적으로 바인딩 했습니다. JAR 및 Xamarin Android 앱에서 사용 합니다.
