...
> [!NOTE]
>참고: 설정은 `ACCESS_FINE_LOCATION` 거칠게 및 파인 위치 데이터 모두에 대 한 액세스를 의미 합니다. 앱이 작동 하는 데 필요한 *최소* 권한만 두 권한을 모두 설정할 필요는 없습니다.

이 코드 조각은 앱 `ACCESS_FINE_LOCATION` 에 사용 권한이 있는지 확인 하는 방법의 예입니다.
...
Xamarin Android 응용 프로그램에서 퓨즈 위치 공급자를 사용 하려면 먼저 프로젝트에 **xamarin.googleplayservices.base** 패키지를 추가 해야 합니다. 또한 아래에 설명 된 `using` 클래스를 참조 하는 모든 소스 파일에 다음 문을 추가 해야 합니다.
...
퓨즈 위치 공급자와 상호 작용 하려면 Xamarin Android 응용 프로그램에의 `FusedLocationProviderClient`인스턴스가 있어야 합니다. 이 클래스는 위치 업데이트를 구독 하 고 장치의 마지막으로 알려진 위치를 검색 하는 데 필요한 메서드를 제공 합니다.

활동의 `FusedLocationProviderClient`메서드는 다음 코드 조각에 나와 있는 것 처럼에 대 한 참조를 가져올 수 있는 적절 한 위치입니다. `OnCreate`
...
메서드 `FusedLocationProviderClient.GetLastLocationAsync()` 는 Xamarin Android 응용 프로그램에서 최소한의 코딩 오버 헤드로 장치를 마지막으로 알려진 위치를 신속 하 게 가져올 수 있는 간단한 차단을 제공 합니다.

이 코드 조각에서는 메서드를 `GetLastLocationAsync` 사용 하 여 장치의 위치를 검색 하는 방법을 보여 줍니다.
...
Xamarin Android 응용 프로그램은 다음 코드 조각에 표시 된 `FusedLocationProviderClient.RequestLocationUpdatesAsync` 것 처럼 메서드를 사용 하 여 퓨즈 위치 공급자의 위치 업데이트를 구독할 수도 있습니다.
...
퓨즈 위치 공급자가 위치 데이터의 가용성에 대 한 변경을 감지한 경우 `LocationProvider.OnLocationAvailability(LocationAvailability
locationAvailability)` 메서드를 호출 합니다. 속성이을 `LocationAvailability.IsLocationAvailable` 반환 `true`하는 경우에서 보고 `OnLocationResult` 하는 장치 위치 결과가 정확 하 고에서 요구 `LocationRequest`하는 대로 최신으로 간주 될 수 있습니다. 가 `IsLocationAvailable` false 이면에 의해 `OnLocationResult`위치 결과가 반환 되지 않습니다.
...
Android Location Service는 Android에서 위치 정보를 사용 하기 위한 이전 API입니다. 위치 데이터는 하드웨어 센서에 의해 수집 되 고 시스템 서비스에서 수집 됩니다 `LocationManager` `ILocationListener`.이 서비스는 클래스 및를 사용 하 여 응용 프로그램에서 액세스 됩니다.

Location Service는 Google Play 서비스 설치 되지 않은 장치에서 실행 해야 하는 응용 프로그램에 가장 적합 합니다.

Location Service는 시스템에서 관리 하는 특수 한 유형의 [서비스](https://developer.android.com/guide/components/services.html) 입니다. 시스템 서비스는 장치 하드웨어와 상호 작용 하며 항상 실행 되 고 있습니다. 응용 프로그램에서 위치 업데이트를 탭 하면 `LocationManager` `RequestLocationUpdates` 및 호출을 사용 하 여 시스템 위치 서비스에서 위치 업데이트를 구독 합니다.
...
`LocationManager` 클래스의 인스턴스를 사용 하 여 시스템 위치 서비스에 액세스할 수 있습니다. `LocationManager`는 시스템 위치 서비스와 상호 작용 하 고 메서드를 호출할 수 있는 특수 클래스입니다. 응용 프로그램은 아래와 같이를 호출 `LocationManager` `GetSystemService` 하 고 서비스 형식으로 전달 하 여에 대 한 참조를 가져올 수 있습니다.

```csharp
LocationManager locationManager = (LocationManager) GetSystemService(Context.LocationService);
```

`OnCreate`는에 대 `LocationManager`한 참조를 가져올 수 있는 좋은 장소입니다.
를 `LocationManager` 클래스 변수로 유지 하 여 활동 수명 주기의 다양 한 지점에서 호출할 수 있도록 하는 것이 좋습니다.

### <a name="request-location-updates-from-the-locationmanager"></a>LocationManager에서 위치 업데이트 요청

응용 프로그램에에 대 `LocationManager`한 참조가 있는 경우 필요한 위치 정보 유형 및 정보를 업데이트할 빈도를 `LocationManager` 알려 주어 야 합니다. 이렇게 하려면 `LocationManager` 개체에 `RequestLocationUpdates` 대해를 호출 하 고 업데이트에 대 한 기준을 전달 하 고 위치 업데이트를 수신 하는 콜백을 전달 합니다. 이 콜백은 인터페이스를 `ILocationListener` 구현 해야 하는 형식입니다 (이 가이드의 뒷부분에서 자세히 설명).

메서드 `RequestLocationUpdates` 는 응용 프로그램이 위치 업데이트 수신을 시작 하도록 시스템 위치 서비스에 지시 합니다. 이 메서드를 사용 하면 공급자를 지정 하 고 시간 및 거리 임계값을 지정 하 여 업데이트 빈도를 제어할 수 있습니다. 예를 들어 아래 메서드는 2000 밀리초 마다 GPS 위치 공급자의 위치 업데이트를 요청 하며 위치가 미터 이상으로 변경 되는 경우에만 해당 합니다.
...
응용 프로그램이에서 `LocationManager`업데이트를 요청한 후에는 인터페이스를 [`ILocationListener`](xref:Android.Locations.ILocationListener) 구현 하 여 서비스에서 정보를 받을 수 있습니다. 이 인터페이스는 위치 서비스 및 위치 공급자를 수신 하는 네 가지 메서드를 `OnLocationChanged`제공 합니다. 위치 업데이트를 요청할 `OnLocationChanged` 때 설정 된 조건에 따라 위치가 변경 될 때까지 사용자의 위치가 변경 되 면 시스템에서를 호출 합니다. 

다음 코드에서는 `ILocationListener` 인터페이스의 메서드를 보여 줍니다.
...
시스템 리소스를 절약 하기 위해 응용 프로그램은 가능한 한 빨리 위치 업데이트를 구독 취소 해야 합니다. 메서드 `RemoveUpdates` 는 응용 프로그램 `LocationManager` 에 대 한 업데이트 전송을 중지 하도록에 지시 합니다.  예를 들어 작업이 화면에 없는 상태 `RemoveUpdates` 에서 응용 `OnPause` 프로그램의 위치 업데이트가 필요 하지 않은 경우 작업은 메서드를 호출 하 여 전원을 절약할 수 있습니다.
...
위의 응용 프로그램은 위치 공급자로 GPS를 설정 합니다. 그러나 장치가 실내에 있거나 GPS 수신기가 없는 경우와 같이 일부 경우에는 GPS를 사용 하지 못할 수 있습니다. 이 경우에는 공급자에 대 한 결과가 `null` 반환 됩니다.

GPS를 사용할 수 없을 때 앱이 작동 하려면 응용 프로그램 시작 시 사용 `GetBestProvider` 가능한 최상의 (장치 지원 및 사용자 지원) 위치 공급자를 요청 하는 방법을 사용 합니다. 특정 공급자를 전달 하는 대신 [ `Criteria` 개체](xref:Android.Locations.Criteria)를 사용 하 `GetBestProvider` 여 정확도 및 기능과 같은 공급자에 대 한 요구 사항을 알려줄 수 있습니다. `GetBestProvider`지정 된 조건에 가장 적합 한 공급자를 반환 합니다.
...
- [Location (샘플)](https://developer.xamarin.com/samples/monodroid/Location/)
