...
기본 클래스는 유지 되 고 관리 코드 내에서 재정의 되는 각 메서드에 대 한 기본 메서드 선언을 제공 되는 알 수 있습니다.
...
하지만,이 경우 완벽 하 게 결정 하지 않습니다. 특히 관리 되는 형식과 같은 Java 형식 간의 몇 가지 고급 매핑에 그렇습니다.
...
메서드 핸들을 정적 필드에 저장 된 참고 `id_getRuntime`합니다. 이 성능 최적화 되므로 메서드 핸들을 호출할 때마다 조회할 필요가 없습니다. 이러한 방식으로 메서드 핸들을 캐시 하는 데 필요한 것입니다. 메서드 핸들을 가져오면 [JNIEnv.CallStaticObjectMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticObjectMethod/) 메서드를 호출 하는 데 사용 됩니다. `JNIEnv.CallStaticObjectMethod` 반환 된 `IntPtr` 반환 된 Java 인스턴스 핸들을 포함 하는.
...
메서드 바인딩 구현은 Java 메서드의 런타임 호출을 담당 합니다. 또한 포함을 `[Register]` 메서드 등록의 일부 이며 메서드 등록 섹션에서 다룰는 사용자 지정 특성 선언:
...
마지막으로 `n_Add` 메서드는 해당 관리 되는 유형으로 JNI 매개 변수 마샬링 위임 메서드를 호출 합니다.
...
`Invoker` 형식은 JNI Java 만든 인스턴스에 대 한 참조를 가져오는 경우에 필요한입니다.
...
1.  하는 경우 `T` 추상 클래스 또는 인터페이스에 있는 *해야* 수는 *호출자* 에 사용할 수 있는 `T` 합니다. 호출자는 상속 된 추상이 아닌 형식 `T` 되거나 구현 `T` 와 동일한 이름을 가진 `T` 호출자 접미사를 사용 하 여 합니다. 예를 들어, T는 인터페이스 `Java.Lang.IRunnable` , 형식은 `Java.Lang.IRunnableInvoker` 존재 해야 하며 필요한 있어야 `(IntPtr,
...
인스턴스 메서드를 통해 호출 되 *메서드 Id*합니다. 통해 메서드 Id를 가져오는 [JNIEnv.GetMethodID](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.GetMethodID/), 형식을 요구 하는 메서드의 이름에는 메서드가 정의 되어 있는 하며 [JNI 형식 시그니처](#JNI_Type_Signatures) 메서드.

메서드 Id를 해제할 필요가 없습니다 및 해당 Java 형식을 로드 하는으로 유효 합니다. (Android 지원 하지 않습니다 현재 클래스 언로드.)

메서드를 호출 하기 위한 메서드 집합: 거의 메서드 호출 및 비가상 메서드를 호출 합니다. 두 가지 메서드는 메서드를 호출 하는 메서드 ID 하며 비가상 호출도 지정 해야 하는 하는 클래스 구현을 호출 해야 합니다.

인터페이스 메서드만을 선언 형식 내에서 조회할 수 확장 및 상속 된 인터페이스에서 제공 하는 메서드를 조회할 수 없습니다. 나머지 바인딩 인터페이스를 참조 하세요. 자세한 세부 정보에 대 한 호출자 구현 섹션 /입니다.

클래스에서 메서드 선언 또는 기본 클래스 또는 구현 된 인터페이스 조회할 수 있습니다.


### <a name="virtual-method-invocation"></a>가상 메서드 호출

메서드를 호출 하기 위한 메서드 집합을 거의 명명 패턴을 따릅니다.
...
-   [JNIEnv.CallObjectMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallObjectMethod/) &ndash; 와 같은 비-기본 제공 형식을 반환 하는 메서드를 호출 `java.lang.Object` , 배열 및 인터페이스입니다. 반환 값이는 jni 논리 참조 합니다.
...
비가상 메서드를 호출 하기 위한 메서드 집합을 명명 패턴을 따릅니다.

```csharp
* JNIEnv.CallNonvirtual*Method( IntPtr instance, IntPtr class, IntPtr methodID, params JValue[] args );
```

여기서 `*` 메서드의 반환 형식입니다. 비가상 메서드 호출 가상 메서드의 기본 메서드를 호출 하려면 일반적으로 사용 됩니다.

-   [JNIEnv.CallNonvirtualObjectMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualObjectMethod/) &ndash; 와 같은 비-기본 제공 형식을 반환 하는 메서드를 호출 하는 비-거의 `java.lang.Object` , 배열 및 인터페이스입니다. 반환 값이는 jni 논리 참조 합니다.

-   [JNIEnv.CallNonvirtualBooleanMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualBooleanMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `bool` 값입니다.

-   [JNIEnv.CallNonvirtualByteMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualByteMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `sbyte` 값입니다.

-   [JNIEnv.CallNonvirtualCharMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualCharMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `char` 값입니다.

-   [JNIEnv.CallNonvirtualShortMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualShortMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `short` 값입니다.

-   [JNIEnv.CallNonvirtualLongMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualLongMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `long` 값입니다.

-   [JNIEnv.CallNonvirtualFloatMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualFloatMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `float` 값입니다.

-   [JNIEnv.CallNonvirtualDoubleMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallNonvirtualDoubleMethod/) &ndash; 비 거의 반환 하는 메서드 호출을 `double` 값입니다.
...
정적 메서드를 통해 호출 되 *메서드 Id*합니다. 통해 메서드 Id를 가져오는 [JNIEnv.GetStaticMethodID](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.GetStaticMethodID/), 형식을 요구 하는 메서드의 이름에는 메서드가 정의 되어 있는 하며 [JNI 형식 시그니처](#JNI_Type_Signatures) 메서드.
...
-   [JNIEnv.CallStaticObjectMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticObjectMethod/) &ndash; 와 같은 비-기본 제공 형식을 반환 하는 정적 메서드를 호출 `java.lang.Object` , 배열 및 인터페이스입니다. 반환 값이는 jni 논리 참조 합니다.

-   [JNIEnv.CallStaticBooleanMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticBooleanMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `bool` 값입니다.

-   [JNIEnv.CallStaticByteMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticByteMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `sbyte` 값입니다.

-   [JNIEnv.CallStaticCharMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticCharMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `char` 값입니다.

-   [JNIEnv.CallStaticShortMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticShortMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `short` 값입니다.

-   [JNIEnv.CallStaticLongMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallLongMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `long` 값입니다.

-   [JNIEnv.CallStaticFloatMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticFloatMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `float` 값입니다.

-   [JNIEnv.CallStaticDoubleMethod](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallStaticDoubleMethod/) &ndash; 반환 하는 정적 메서드를 호출을 `double` 값입니다.
...
[JNI 형식 시그니처](http://docs.oracle.com/javase/1.5.0/docs/guide/jni/spec/types.html#wp16432) 됩니다 [JNI 형식 참조](#_JNI_Type_References) (하지만 하지 간소화 된 유형 참조) 메서드는 제외 합니다. 메서드를 사용 하 여 JNI 형식 시그니처는 여는 괄호 `'('`뒤에 닫는 괄호 뒤에 (또는 사용 하 여 구분 쉼표를 사용 하지 명제)를 함께 연결 하는 형식 매개 변수 모두에 대 한 형식 참조, `')'`, 다음 메서드 반환 형식의 JNI 형식 참조입니다.

예를 들어 다음 Java 메서드의 경우:
...
JNI 형식 참조는 Java 형식 참조와 다릅니다. 와 같은 정규화 된 Java 형식 이름을 사용할 수 없습니다 `java.lang.String` jni를 대신 사용 해야 JNI 변형 `"java/lang/String"` 또는 `"Ljava/lang/String;"`, 컨텍스트에 따라 자세한 내용은 아래 참조 합니다.
네 가지 유형 JNI 형식 참조입니다.
...
기본 제공 형식 참조는 기본 제공 값 형식을 참조 하는 데 사용 하는 단일 문자입니다. 매핑을 아래와 같습니다.
...
간소화 된 형식 참조만 사용할 수 있습니다 [JNIEnv.FindClass(string)](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.FindClass/(System.String))합니다.
두 가지 방법으로 간소화 된 형식 참조를 가져오려고 합니다.

1.  Java 정규화 된 이름으로 대체 마다 `'.'` 패키지 이름 및 형식 이름을 사용 하 여 이전 `'/'` , 매 `'.'` 내 형식 이름을 `'$'` .
...
형식 참조를 얻으려고 하는 또 다른 방법은의 출력을 읽는 `'javap -s -classpath android.jar fully.qualified.Java.Name'`입니다.
유형에 따라 관련 된 사용할 수는 생성자 선언 또는 메서드 반환 형식을 JNI 이름을 확인 합니다. 예를 들어:
...
`Thread.State` 시그니처를 사용할 수 있도록이 Java 열거형 형식이 `valueOf` 형식 참조를 Ljava/l a n g/스레드$ 상태 인지 확인 하는 방법입니다.



### <a name="array-type-references"></a>배열 형식 참조

배열 형식 참조가 `'['` JNI 형식 참조에 접두사로 지정 됩니다.
배열을 지정 하는 경우 간소화 된 형식 참조를 사용할 수 없습니다.

예를 들어 `int[]` 됩니다 `"[I"`, `int[][]` 는 `"[[I"`, 및 `java.lang.Object[]` 는 `"[Ljava/lang/Object;"`합니다.



## <a name="java-generics-and-type-erasure"></a>Java 제네릭과 형식 지우기
...
[Android.Runtime.JNIEnv](https://developer.xamarin.com/api/type/Android.Runtime.JNIEnv/) 는 관리 되는 래퍼에 대 한는 Jave 네이티브 인터페이스 (JNI). JNI 함수 내에서 선언 된 합니다 [Java 기본 인터페이스 사양](http://download.oracle.com/javase/1.5.0/docs/guide/jni/spec/functions.html)되지만 메서드는 명시적 제거 하도록 변경 되었습니다 `JNIEnv*` 매개 변수 및 `IntPtr` 대신 사용 됩니다 `jobject`를 `jclass`, `jmethodID`등입니다. 예를 들어 합니다 [JNI NewObject 함수](http://download.oracle.com/javase/1.5.0/docs/guide/jni/spec/functions.html#wp4517):

```csharp
jobject NewObjectA(JNIEnv *env, jclass clazz, jmethodID methodID, jvalue *args);
```

이 기능은으로 표시 합니다 [JNIEnv.NewObject](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.NewObject/p/System.IntPtr/System.IntPtr/Android.Runtime.JValue[]/) 메서드:

```csharp
public static IntPtr NewObject(IntPtr clazz, IntPtr jmethod, params JValue[] parms);
```

두 호출 간에 변환 하는 것은 비교적 간단 합니다. C에서 해야 합니다.
...
IntPtr에 보관 하는 Java 개체 인스턴스를 만든 후 원할 것에 작업을 수행 해야 합니다. 와 같은 JNIEnv 메서드를 사용할 수 있습니다 [ <span class="external">JNIEnv.CallVoidMethod()</span> ](https://developer.xamarin.com/api/member/Android.Runtime.JNIEnv.CallVoidMethod/p/System.IntPtr/System.IntPtr/Android.Runtime.JValue[]/) 가 이미와 유사 하지만를 위해 C# 래퍼 있다면 JNI 참조를 통해 래퍼를 생성 하려고 합니다. 통해 수행할 수는 [Extensions.JavaCast <t>()</t> ](https://developer.xamarin.com/api/member/Android.Runtime.Extensions.JavaCast%7BTResult%7D/p/Android.Runtime.IJavaObject/) 확장 메서드:
...
JNI 직접 처리는 피해 야 하는 비용이 끔찍한 환경. 그러나는 것은 피할 수 있습니다. 다행히이 가이드 Android 용 Mono 사용 하 여 바인딩되지 않은 Java 사례를 누르면 일부 지원을 제공 합니다.
