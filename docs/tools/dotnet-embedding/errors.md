...
이 도구는 옵션 `X`을 지원 하지 않습니다. 다른 버전의 도구에서이를 지원 하거나 `X` 옵션이이 환경에 적용 되지 않을 수 있습니다.

<a name="EM0003" />

### <a name="em0003-the-platform-x-is-not-valid"></a>EM0003: 플랫폼이 `X` 잘못 되었습니다.

이 도구는 플랫폼 `X`을 지원 하지 않습니다. 다른 버전의 도구에서이를 지원 하거나 `X` 플랫폼이이 환경에 적용 되지 않을 수 있습니다.

<a name="EM0004" />

### <a name="em0004-the-target-x-is-not-valid"></a>EM0004: 대상이 `X` 잘못 되었습니다.

이 도구는 대상을 `X`지원 하지 않습니다. 다른 버전의 도구에서이를 지원 하거나 `X` 대상이이 환경에 적용 되지 않을 수 있습니다.

<a name="EM0005" />

### <a name="em0005-the-compilation-target-x-is-not-valid"></a>EM0005: 컴파일 대상이 `X` 잘못 되었습니다.

이 도구는 컴파일 대상을 `X`지원 하지 않습니다. 이 환경에서는 다른 버전의 도구가이를 지원 하거나 `X` 컴파일 대상이 적용 되지 않을 수 있습니다.

<a name="EM0006" />

### <a name="em0006-could-not-find-the-xcode-location"></a>EM0006: Xcode 위치를 찾을 수 없습니다.

도구에서 `xcode-select -p` 명령을 사용 하 여 현재 선택한 Xcode 위치를 찾을 수 없습니다. 이 명령을 성공적으로 실행할 수 있는지 확인 하 고 올바른 Xcode 위치를 반환 하십시오.

<a name="EM0007" />

### <a name="em0007-could-not-get-the-sdk-version-for-sdk"></a>EM0007: ' {sdk} '에 대 한 sdk 버전을 가져올 수 없습니다.

도구에서 `xcrun --show-sdk-version --sdk {sdk}` 명령을 사용 하 여 SDK 버전을 가져올 수 없습니다. 이 명령을 성공적으로 실행할 수 있는지 확인 하 고 SDK 버전을 반환 합니다.
...
오류 메시지의 아키텍처가 대상 플랫폼에 적합 하지 않습니다. `--abi` 옵션에 대해 유효한 아키텍처가 전달 되었는지 확인 하세요.

<a name="EM0009" />

### <a name="em0009-the-feature-x-is-not-currently-implemented-by-the-generator"></a>EM0009: 현재 생성기 `X` 에서이 기능을 구현 하지 않습니다.
...
명령 줄 옵션 `A` 에 지정 된 구문을 도구에서 구문 분석할 수 없습니다. 설명서에서 올바른 구문을 확인 하세요.
...
### <a name="em1010-type-t-is-not-generated-because-x-are-not-supported"></a>EM1010:은 `T` 지원 되지 않으므로 `X` 형식이 생성 되지 않습니다.

이는 지원 **warning** 되지 않는 기능을 `T` 사용 `X`하기 때문에 해당 형식이 무시 됨을 나타내는 경고입니다 (즉, 아무 것도 생성 되지 않음).

참고: 지원 되는 기능은 도구의 새 버전에서 개선 될 예정입니다.

<a name="EM1011" />

### <a name="em1011-type-t-is-not-generated-because-it-lacks-marshaling-code-with-a-native-counterpart"></a>EM1011: 형식이 `T` 네이티브에 해당 하는 마샬링 코드가 없으므로 형식이 생성 되지 않습니다.

이는 추가 **warning** 마샬링이 필요한 .net framework `T` 의 항목을 노출 하므로 형식이 무시 됨을 나타내는 경고입니다. 즉, 아무 것도 생성 되지 않습니다.
...
이는 형식의 `T` 매개 변수가 지원 되지 `C` 않기 때문에 생성자가 무시 됨을 나타내는 **경고** 입니다. 즉, 아무 것도 생성 되지 않습니다.
...
이는 생성자 **warning** `C` 의 기본 매개 변수가 추가 코드를 생성 하지 않는 경고입니다. 가장 일반적인 원인은 기존 메서드에 이미 동일한 시그니처가 있는 경우입니다. 예를 들어 .NET에서는 다음을 사용할 수 있습니다.
...
이 경우 두 개의 생성 된 `init` 선택기만 Mono를 호출 하는 두 개의 생성 된 선택기를 만들지만 후자에 대 한 래퍼가 존재 하지 않습니다.
...
반환 형식이 `T` 지원 되지 않기 **때문에 메서드** `M` 는 무시 됩니다. 즉, 반환 되는 것은 아닙니다.
...
이는 형식의 `T` 매개 변수가 지원 되지 `M` 않기 때문에 메서드가 무시 됨을 나타내는 **경고** 입니다. 즉, 아무 것도 생성 되지 않습니다.
...
이는 메서드의 **warning** `M` 기본 매개 변수가 추가 코드를 생성 하지 않는 경고입니다. 가장 일반적인 원인은 기존 메서드에 이미 동일한 시그니처가 있는 경우입니다. 예를 들어 .NET에서는 다음을 사용할 수 있습니다.
...
이 경우 두 개의 생성 된 `Increment` 선택기만 Mono를 호출 하는 두 개의 생성 된 선택기를 만들지만 후자에 대 한 래퍼가 존재 하지 않습니다.

<a name="EM1033" />

### <a name="em1033-method-m-is-not-generated-because-another-method-exposes-the-operator-with-a-friendly-name"></a>EM1033: 다른 `M` 메서드에서는 이름으로 연산자를 노출 하므로 메서드가 생성 되지 않습니다.

이는 다른 **warning** 메서드가 이름으로 연산자 `M` 를 노출 하기 때문에 메서드가 생성 되지 않는 경고입니다. (https://msdn.microsoft.com/library/ms229032(v=vs.110).aspx)
...
이는 인스턴스 **warning** 형식 (예: `System.Int32`)에 대 한 확장 메서드를 찾을 수 있다는 경고입니다. 목적-C에서는 기본 형식에 대 한 범주를 만들 수 없습니다. 대신 생성기에서 일반적인 정적 메서드를 생성 합니다.
...
### <a name="em1041-indexed-properties-on-t-is-not-generated-because-multiple-indexed-properties-are-not-supported"></a>EM1041: 여러 인덱싱된 속성이 `T` 지원 되지 않으므로의 인덱싱된 속성이 생성 되지 않습니다.

이는 인덱싱된 **warning** 속성이 여러 개 지원 되지 않기 `T` 때문에의 인덱싱된 속성이 무시 됨을 나타내는 경고입니다 (즉, 아무 것도 생성 되지 않음).
...
### <a name="em1051-element-e-is-generated-instead-as-f-because-its-name-conflicts-with-an-important-objective-c-selector"></a>EM1051: 해당 `E` 이름이 중요 한 목표 `F` -c 선택기와 충돌 하기 때문에 요소가 대신 생성 됩니다.

이는 이름이 **warning** 중요 한 목표- `E` c 선택기와 충돌 `F` 하기 때문에 요소가 생성 된다는 경고입니다.
...
이는 이름이 **warning** 동일한 클래스의 `E` 다른 요소와 충돌 하므로 요소가 생성 되지 않는 경고입니다.
...
이는 Xamarin.ios **warning** 및 xamarin.ios 사용 `E` 사례에서 대상이 지원 되지 않는 것으로 간주 된다는 경고입니다. 
