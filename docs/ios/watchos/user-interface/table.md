...
[![](table-images/add-row-controller-sml.png "속성 패드에 클래스 이름 입력")](table-images/add-row-controller.png#lightbox)
...
- 인터페이스 컨트롤러 `DidSelectRow` 에서 메서드를 구현 하거나
- 스토리 보드에 대해 segue을 만들고 행 `GetContextForSegue` 을 선택 하 여 다른 장면을 열지 않으려면을 구현 합니다.

### <a name="didselectrow"></a>DidSelectRow

프로그래밍 방식으로 행 선택을 처리 하려면 `DidSelectRow` 메서드를 구현 합니다. 새 장면을 열려면를 사용 `PushController` 하 고 장면 식별자와 사용할 데이터 컨텍스트를 전달 합니다.
...
인터페이스 컨트롤러에서 메서드를 `GetContextForSegue` 구현 하 고 segue에서 제공 하는 장면에 제공 되어야 하는 데이터 컨텍스트를 반환 합니다.
...
이 데이터는 해당 `Awake` 메서드의 대상 스토리 보드 장면에 전달 됩니다.
...
![](table-images/prototype-rows1.png "프로토타입 행 수 설정")

**Rows** 속성을 **3** 으로 설정 하면 컨트롤을 끌어 놓을 수 있는 추가 행 자리 표시 자가 만들어집니다. 각 행에 대해 **속성** 패드에서 **클래스** 이름을 설정 하 여 행 컨트롤러 클래스가 생성 되도록 합니다.

![](table-images/prototype-rows2.png "디자이너의 프로토타입 행")

다른 행 형식으로 테이블을 채우려면 `SetRowTypes` 메서드를 사용 하 여 테이블의 각 행에 사용할 행 컨트롤러 유형을 지정 합니다. 행의 식별자를 사용 하 여 각 행에 사용할 행 컨트롤러를 지정 합니다.
...
이 기능을 사용 하려면 디자인 화면 `WKInterfaceTable` 에서을 선택 하 고 **세로 세부 정보 페이징** 옵션을 선택 합니다.

![](table-images/vertical-detail-paging-sml.png "세로 세부 정보 페이징 옵션 선택")
...
**Designer.cs** 파일은 디자이너 화면에 생성 된 작업 (예: 컨트롤 한 개 `WKInterfaceLabel` 를 사용 하는 예제)을 포함 하는 부분 클래스 정의입니다.
