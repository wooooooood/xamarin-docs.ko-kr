...
시스템은 이러한 작업 새 호출 하 여 `HandleBackgroundTasks` 의 메서드는 `WKExtensionDelegate` 위임 합니다. 예를 들어:
...
> [!IMPORTANT]
> 시간은 항상 해야는 ` WKSnapshotRefreshBackgroundTask` 응용 프로그램에서 새 데이터 받고 해당 사용자 인터페이스를 업데이트 하거나 사용자 수정된 내용은 표시 되지 것입니다.
...
> [!IMPORTANT]
> 앱 홈 화면에만 추가 된 일반 예약 지정 되지 않습니다. 일반 일정 예약 및 백그라운드 업데이트, 응용 프로그램 _해야_ 도킹 스테이션에 추가할 수 있습니다.
