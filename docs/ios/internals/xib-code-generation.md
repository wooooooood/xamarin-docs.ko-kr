...
description: '이 문서에서는 xib 파일을 c #에 매핑하여 파일에 프로그래밍 방식으로 액세스할 수 있게 만드는 코드를 생성 하는 방법을 설명 합니다.'
ms.prod: xamarin
...
Cocoa Touch에서 기존 유형을 사용 하는 것 외에도 **xib** 파일에 사용자 지정 형식을 정의할 수 있습니다. **Xib** 파일에 정의 되어 있거나 c # 코드에만 정의 된 형식을 사용할 수도 있습니다. 현재는 Interface Builder **xib** 파일 외부에서 정의 된 형식의 세부 정보를 인식 하지 못하므로 사용자 지정 콘센트 및 작업을 표시 하거나 표시 하지 않습니다. 이러한 제한 제거는 나중에 계획 됩니다.
...
** {0} Xib** 파일의 경우 *페이지*의 빌드 작업을 포함 하는 ** {0} xib.designer.cs** 파일이 Mac용 Visual Studio 프로젝트에도 있는 경우에는 모든 동작에 대 한 속성을 사용 하 여 **xib** 파일에서 찾을 수 있는 모든 사용자 클래스에 대해 디자이너 파일에 부분 클래스가 생성 됩니다. 코드 생성은이 파일이 있는 경우에만 사용할 수 있습니다.
...
클래스를 목표 C 런타임에서 검색할 수 있도록 하려면 클래스에 특성을 적용 Mac용 Visual Studio `[Register (name)]` 합니다. Xamarin.ios는 파생 클래스를 자동으로 등록 하지만 정규화 `NSObject` 된 .net 이름을 사용 합니다. Mac용 Visual Studio에서 적용 하는 특성은이를 재정의 하 여 각 클래스가 **xib** 파일에 사용 된 이름으로 등록 되도록 합니다. Mac용 Visual Studio를 사용 하 여 디자이너 파일을 생성 하지 않고 IB에서 사용자 지정 클래스를 사용 하는 경우 관리 되는 클래스가 예상 목표-C 클래스 이름과 일치 하도록 수동으로이를 적용 해야 할 수 있습니다.
...
** {0} Xib.designer.cs** 디자이너 파일 옆에 있는 ** {0} xib.cs** 파일에 이러한 파일을 배치 하는 것이 일반적입니다.
...
에 해당 하는 형식으로 콘센트 속성을 정의 하는 경우 `id` `NSObject` 디자이너 코드 생성기는 편의를 위해 현재 해당 콘센트에 연결 된 개체를 기반으로 가능한 가장 강력한 형식을 결정 합니다.
...
1. `partial`비 디자이너 클래스 파트의 클래스 본문에를 입력 하는 경우에는에서 구현 되지 않은 모든 부분 메서드 (partial method)의 시그니처를 자동 완성 하는 Mac용 Visual Studio 제공 합니다.
...
작업이의 발신자 형식을 갖도록 정의 된 경우 `id` (에 해당 `NSObject` ) 디자이너 코드 생성기는 현재 해당 작업에 연결 된 개체를 기반으로 가능한 가장 강력한 형식을 결정 합니다. 그러나이는 이후 버전에서 지원 되지 않을 수 있으므로 사용자 지정 클래스를 정의할 때 명시적으로 작업을 강력한 형식으로 지정 하는 것이 좋습니다.

이러한 부분 메서드 (partial method)는 c #의 경우에만 만들어지므로 CodeDOM은 부분 메서드를 지원 하지 않으므로 다른 언어에 대해서는 생성 되지 않습니다.
