...
예를 들어 xamarin.ios 응용 프로그램 `Main` 에서 일반적으로 관리 되는 메서드를 살펴보겠습니다.

```csharp
UIApplication.Main (args, null, "AppDelegate");
```

이 코드는 목표-C 런타임에 응용 프로그램의 대리자 클래스로 호출 `AppDelegate` 되는 형식을 사용 하도록 지시 합니다. 목적-C 런타임이 C# `AppDelegate` 클래스의 인스턴스를 만들 수 있도록 하려면 해당 클래스를 등록 해야 합니다.
...
범주를 만들려면 `[Category]` 특성을 사용 하 고 확장할 유형을 지정 합니다. 예를 들어 다음 코드는를 `NSString`확장 합니다.

```csharp
[Category (typeof (NSString))]
```

각 범주의 메서드는 `[Export]` 특성을 가지 며, 목표-C 런타임에 사용할 수 있도록 합니다.
...
이 예제에서는 네이티브 `toUpper` 인스턴스 메서드를 `NSString` 클래스에 추가 합니다. 이 메서드는 목표-C에서 호출 될 수 있습니다.
...
Xamarin.ios 8.10부터 `[Protocol]` 특성을 사용 하는 인터페이스는 목표 C를 프로토콜로 내보냅니다.
...
이 코드는 `IMyProtocol` 호출 `MyProtocol` 된 프로토콜 및 프로토콜을 구현 하는 라는 `MyClass` 클래스를 목표로 하는를 내보냅니다.
...
> [!NOTE]
> Xamarin의 Classic API `--registrar:legacystatic` 및 등의 다른 옵션을 `--registrar:legacydynamic`지원 합니다. 그러나 이러한 옵션은 Unified API에서 지원 되지 않습니다.

## <a name="shortcomings-in-the-old-registration-system"></a>이전 등록 시스템의 단점

이전 등록 시스템의 단점은 다음과 같습니다.

- 타사 네이티브 라이브러리의 목적-C 클래스 및 메서드에 대 한 (네이티브) 정적 참조가 없습니다. 즉, 모든 항목이 제거 되기 때문에 실제로는 사용 되지 않는 타사 네이티브 코드를 제거 하도록 네이티브 링커에 요청할 수 없습니다. 이는 모든 타사 바인딩이 수행 `-force_load libNative.a` 해야 하는 또는 `ForceLoad=true` `[LinkWith]` 특성에 해당 하는에 대 한 이유입니다.
- 동일한 객관적인 C 이름으로 두 개의 관리 되는 형식을 내보낼 수 있습니다 (경고 없음). 드문 시나리오는 다른 네임 스페이스에 있는 두 `AppDelegate` 클래스를 사용 하는 것 이었습니다. 런타임에는 완전히 무작위로 선택 된 것입니다. 즉, 매우 난해해 하 고 뛰어난 디버깅 환경을 위해 다시 작성 되지 않은 앱의 실행 사이에서 차이가 있습니다.
...
Variadic 함수는 `IsVariadic` `[Export]` 특성에 대 한 인수를 사용 해야 합니다.
