...
- **Info.plist에 컨테이너 식별자를 추가** 합니다. 컨테이너 식별자 형식이 `TeamID.BundleID`. 자세한 내용은 [자격 사용](~/ios/deploy-test/provisioning/entitlements.md) 가이드를 참조 하세요.
...
응용 프로그램은 `NSNotificationCenter.DefaultCenter`에 관찰자를 추가 하 여 iCloud에 의해 값이 변경 될 때 알림을 받을 수도 있습니다.
**KeyValueViewController.cs** `ViewWillAppear` 메서드의 다음 코드는 이러한 알림을 수신 대기 하 고 변경 된 키 목록을 만드는 방법을 보여 줍니다.
...
ICloudUIDoc 예제는 단일 텍스트 필드를 포함 하는 간단한 `UIDocument` 서브 클래스를 구현 합니다. 텍스트는 `UITextView`에서 렌더링 되 고 편집 내용은 iCloud에서 빨간색으로 표시 된 알림 메시지와 함께 다른 장치로 전파 됩니다. 예제 코드는 충돌 확인과 같은 고급 iCloud 기능을 처리 하지 않습니다.
...
1. **ICloud 문서 표시** -UI 컨트롤과 상호 작용할 수 있도록 `UIDocument`의 속성을 노출 합니다.
 ...
 **스레드** -`GetUrlForUbiquityContainer`에 대 한 초기 호출이 주 스레드의 차단을 방지 하기 위해 다른 스레드에서 수행 `AppDelegate.FinishedLaunching`.

 **Notificationcenter** -`NSMetadataQuery.StartQuery`와 같은 비동기 작업이 완료 되 면 알림에 등록 합니다.

 **완료 처리기** -`UIDocument.Open`와 같은 비동기 작업이 완료 될 때 실행할 메서드를 전달 합니다.
...
이 코드는 샘플의 `AppDelegate.FinishedLaunching` 메서드에 있습니다.
...
샘플 앱은 단일 파일-test.txt를 처리 합니다. 따라서 **AppDelegate.cs** 의 코드는 해당 파일 이름에 대 한 `NSPredicate`를 만들고 `NSMetadataQuery` 합니다. `NSMetadataQuery`은 비동기적으로 실행 되 고 완료 되 면 알림을 보냅니다. `DidFinishGathering` 알림 관찰자에 의해 호출 되 고, 쿼리를 중지 하 고, 완료 처리기와 함께 `UIDocument.Open` 메서드를 사용 하 여 파일을 로드 하 고 `MonkeyDocumentViewController`에 표시 하는 LoadDocument를 호출 합니다.
...
예제에서 **iCloudUIDoc\MonkeyDocumentViewController.cs** 는 `UITextView`에 MonkeyDocument 텍스트를 표시 합니다. `ViewDidLoad`은 `MonkeyDocument.LoadFromContents` 메서드에서 보내는 알림을 수신 대기 합니다. iCloud가 파일에 대 한 새 데이터를 포함 하는 경우 `LoadFromContents` 호출 되어 문서가 업데이트 되었다는 알림이 표시 됩니다.
...
ICloud에 UIDocument를 추가 하려면 `UIDocument.Save`를 직접 호출 하거나 (새 문서에만 해당) `NSFileManager.DefaultManager.SetUbiquitious`를 사용 하 여 기존 파일을 이동할 수 있습니다. 예제 코드는이 코드를 사용 하 여 어디 컨테이너에 직접 새 문서를 만듭니다. 여기에는 두 가지 완료 처리기가 있습니다. 하나는 `Save` 작업에 대 한 것이 고 다른 하나는 열려 있습니다.
...
문서에 대 한 후속 변경 내용은 직접 "저장" 되지 않으며, 대신 `UpdateChangeCount`를 사용 하 여 변경 되었음을 `UIDocument`에 게 알리고 디스크에 저장 작업을 자동으로 예약 합니다.
...
이 처럼 많은 양의 데이터를 저장 하는 응용 프로그램은 백업 되지 않은 사용자 디렉터리 중 하나에 저장 해야 합니다 (예: 캐시 또는 tmp)를 사용 하거나 `NSFileManager.SetSkipBackupAttribute` 사용 하 여 백업 작업 중에 iCloud가 무시 하도록 해당 파일에 플래그를 적용 합니다.
