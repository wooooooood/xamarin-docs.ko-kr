...
> [!NOTE]
> 이 CloudKit는 중요 한 _전송 기술_합니다. 모든 지 속성; 제공 되지 않습니다. 만 응용을 프로그램을 보내고 서버에서 정보를 효율적으로 받을 수 있습니다.
...
> [!NOTE]
> 현 수 포인터 CloudKit를 사용 하는 경우 가능성이입니다. 예를 들어 응용 프로그램 레코드 포인터 목록을 인출, 레코드 선택 있으며 레코드에 대 한 다음 요청 될 때 레코드 더 이상 존재 데이터베이스에 없습니다. 응용 프로그램이이 상황을 정상적으로 처리 하도록 코드를 작성 해야 합니다.
...
> [!NOTE]
> 모바일 네트워크 통신, 연결 손실 되는 지속적으로 또는 중단 되 면 개발자 CloudKit 작업은 오류 처리 하는 경우 수행 해야 하는 첫 번째 고려 사항 중 하나입니다 "손실" 이기 때문에.
...
> [!NOTE]
> CloudKit 원인을 발생에 대 한 요구와 같은 특정 정보를 포함 하는 페이로드를 포함 하는 대로 약간 CloudKit 푸시 확대 됩니다.
...
> [!NOTE]
> 다른 CloudKit 컨테이너에 연결 되기 때문에 동일한 iCloud 사용자에 대해 동일한 장치에서 실행 중인 다른 응용 프로그램에서 다른 사용자 Id를 해야 합니다.
...
> [!NOTE]
> 시뮬레이터는 에서만 작동 하는 iOS는 **개발 환경**합니다. 개발자는에서 응용 프로그램을 테스트할 준비가 때는 **프로덕션 환경**, 실제 iOS 장치는 필요 합니다.
