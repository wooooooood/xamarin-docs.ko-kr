...
ms.locfileid: "73021906"
---
# <a name="editing-tables-with-xamarinios"></a>Xamarin.ios를 사용 하 여 테이블 편집

`UITableViewSource` 하위 클래스의 메서드를 재정의 하 여 테이블 편집 기능을 사용할 수 있습니다. 가장 간단한 편집 동작은 단일 메서드 재정의를 사용 하 여 구현할 수 있는 살짝 밀기-삭제 제스처입니다.
...
- **CommitEditingStyle** –이 메서드가 재정의 되는 경우 테이블 소스에서 검색 하 고 자동으로 살짝 밀기-삭제 제스처를 사용 하도록 설정 합니다. 메서드의 구현은 `UITableView`에서 `DeleteRows`를 호출 하 여 셀이 사라지게 하 고 모델에서 기본 데이터 (예: 배열, 사전 또는 데이터베이스)도 제거 해야 합니다. 
...
이러한 메서드는 다음과 같은 `TableSource` 클래스에서 구현 됩니다.
...
이 예제에서 `UITableViewSource`는 컬렉션에서 항목 추가 및 삭제를 지원 하기 때문에 문자열 배열 대신 `List<TableItem>`를 데이터 소스로 사용 하도록 업데이트 되었습니다.
...
`UITableViewSource`에는 테이블의 편집 모드 동작에 영향을 주는 다양 한 메서드가 있습니다.
...
- **EditingStyleForRow** – 테이블이 편집 모드에 있는 경우이 메서드의 반환 값은 셀에 빨간색 삭제 아이콘이 표시 되는지 아니면 녹색 추가 아이콘이 표시 되는지를 결정 합니다. 행을 편집할 수 없는 경우 `UITableViewCellEditingStyle.None`을 반환 합니다. 
- **MoveRow** – 테이블에 표시 되는 데이터와 일치 하도록 기본 데이터 구조를 수정할 수 있도록 행이 이동 될 때 호출 됩니다. 

`indexPath`를 사용 하 여 특정 행의 동작을 변경 하려는 경우를 제외 하 고 전체 테이블에 대 한 반환 값을 하드 코딩 하려는 경우를 제외 하 고 처음 세 가지 메서드에 대 한 구현은 비교적 수직으로 전달 됩니다.
...
`MoveRow` 구현은 새 순서와 일치 하도록 기본 데이터 구조를 변경 해야 하기 때문에 좀 더 복잡 합니다. 데이터는 `List`으로 구현 되기 때문에 아래 코드는 이전 위치에 있는 데이터 항목을 삭제 하 고 새 위치에 삽입 합니다. 데이터가 ' order ' 열 (예:)을 사용 하 여 SQLite 데이터베이스 테이블에 저장 된 경우이 메서드는 대신 해당 열의 숫자를 다시 정렬 하기 위해 일부 SQL 작업을 수행 해야 합니다.
...
마지막으로, 테이블을 편집 모드로 전환 하려면 다음과 같이 **편집** 단추가 `SetEditing`를 호출 해야 합니다.
...
`UITableViewSource`에는 테이블의 편집 모드 동작에 영향을 주는 다양 한 메서드가 있습니다. 이러한 메서드는 예제 코드에서 다음과 같이 구현 되었습니다.

- **EditingStyleForRow** – 데이터를 포함 하는 행에 대 한 `UITableViewCellEditingStyle.Delete`를 반환 하 고 마지막 행에 대 한 `UITableViewCellEditingStyle.Insert` (삽입 단추로 동작 하도록 특별히 추가 됨)을 반환 합니다. 
...
- **WillBeginTableEditing** – **편집** 단추가 작업 하는 경우 `SetEditing`를 호출 하 여 테이블을 편집 모드로 전환 합니다. 그러면 테이블의 끝에 있는 **(새 추가)** 행을 ' 삽입 단추 ' 역할을 수행 하는 WillBeginTableEditing 메서드가 트리거됩니다. 
- **DidFinishTableEditing** – Done 단추가 `SetEditing` 되 면 편집 모드를 해제 하기 위해가 다시 호출 됩니다. 예제 코드는 편집이 더 이상 필요 하지 않을 때 테이블에서 **(새 추가)** 행을 제거 합니다. 
...
이 행 삽입 UI 패턴은 자주 사용 되지 않지만 `UITableView.BeginUpdates` 및 `EndUpdates` 메서드를 사용 하 여 모든 테이블의 셀 삽입 또는 제거에 애니메이션 효과를 줄 수 있습니다. 이러한 메서드를 사용 하는 규칙은 `BeginUpdates`와 `EndUpdates` 호출 사이에 `RowsInSection`에서 반환 되는 값의 차이가 `InsertRows` 및 `DeleteRows` 메서드와 함께 추가/삭제 된 셀의 순 수와 일치 해야 한다는 것입니다. 기본 데이터 원본이 테이블 뷰의 삽입/삭제와 일치 하도록 변경 되지 않은 경우 오류가 발생 합니다.
