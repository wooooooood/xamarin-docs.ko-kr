...
`AVCaptureDeviceInput`에서 `AVCaptureConnection`를 사용 하 여 `AVCaptureSession`으로 입력을 가져옵니다. 결과는 스틸 이미지 또는 비디오 스트림으로 출력 됩니다. 전체 프로세스는 `AVCaptureDevice`에 의해 제어 됩니다.
...
IOS 장치에 비디오를 기록 하는 경우 항상 필요한 몇 가지 일반적인 설정 코드가 있습니다. 이 섹션에서는 iOS 장치의 카메라에서 비디오를 기록 하는 데 필요한 최소 설정에 대해 설명 하 고 `UIImageView`에 실시간으로 비디오를 표시 합니다.

### <a name="output-sample-buffer-delegate"></a>출력 샘플 버퍼 대리자

필요한 첫 번째 작업 중 하나는 샘플 출력 버퍼를 모니터링 하 고 grabbed 이미지를 버퍼에서 응용 프로그램 UI의 `UIImageView` 표시 하는 대리자입니다.
...
이 루틴을 사용 하면 `AppDelegate`을 수정 하 여 라이브 비디오 피드를 기록 하는 AV 캡처 세션을 열 수 있습니다.

### <a name="creating-an-av-capture-session"></a>AV 캡처 세션 만들기

AV 캡처 세션은 iOS 장치의 카메라에서 라이브 비디오 기록을 제어 하는 데 사용 되며 iOS 응용 프로그램에 비디오를 가져오는 데 필요 합니다. 예제 `ManualCameraControl` 샘플 응용 프로그램은 여러 위치에서 캡처 세션을 사용 하 고 있으므로 `AppDelegate`에서 구성 되 고 전체 응용 프로그램에서 사용할 수 있게 됩니다.
...
iOS 7 및 이전 버전에서는 `FocusMode`속성을 통해 기존 포커스 컨트롤을 다음과 같이 제공 했습니다.
...
또한 기존 컨트롤은 사용자가 특정 영역에 초점을 맞출 수 있도록`FocusPointOfInterest` 속성을 통해 설정 가능한 관심 지점을 제공 합니다. 또한 응용 프로그램은 `IsAdjustingFocus` 속성을 모니터링 하 여 렌즈 이동을 추적할 수 있습니다.
...
마지막으로 자동 포커스 알고리즘의 속도를 높이는 `SmoothAutoFocus` 속성을 사용 하 여 비디오를 녹화할 때 아티팩트 이동을 방지 하기 위해 더 작은 증가값으로 작업을 수행 합니다.
...
일반적인 AV 캡처 설정 코드가 준비 되 면 응용 프로그램의 스토리 보드에 `UIViewController`을 추가 하 고 다음과 같이 구성할 수 있습니다.
...
- 비디오 피드를 표시 하는 `UIImageView`입니다.
- 포커스 모드를 자동에서 잠김으로 변경 하는 `UISegmentedControl`입니다.
- 현재 렌즈 위치를 표시 하 고 업데이트 하는 `UISlider`입니다.
...
1. `ViewDidLoad` 메서드를 재정의 하 고 다음 코드를 추가 합니다.
...
1. `ViewDidAppear` 메서드를 재정의 하 고 다음을 추가 하 여 뷰가 로드 될 때 기록을 시작 합니다.
...
`ExposurePointOfInterest`는 표시할 대상 개체를 선택 하 여 장면을 노출 하는 데 사용할 수 있으며, 응용 프로그램은 `AdjustingExposure` 속성을 모니터링 하 여 노출이 조정 될 때를 확인할 수 있습니다.
...
- 비디오 피드를 표시 하는 `UIImageView`입니다.
- 포커스 모드를 자동에서 잠김으로 변경 하는 `UISegmentedControl`입니다.
- 오프셋, 기간, ISO 및 바이어스를 표시 하 고 업데이트 하는 네 가지 `UISlider` 컨트롤
...
iOS 7 이상에서는 `WhiteBalanceMode` 속성을 통해 다음과 같은 기존 흰색 잔액 컨트롤을 제공 했습니다.

- `AVCapture WhiteBalance ModeLocked` – 장면을 한 번 샘플링 하 고 장면 전체에 해당 값을 사용 합니다.
- `AVCapture WhiteBalance ModeContinuousAutoExposure` – 적절 하 게 균형을 유지 하기 위해 장면을 지속적으로 샘플링 합니다.
...
위의 기능을 구현 하기 위해 다음 멤버와 함께 `AVCaptureWhiteBalanceGain` 구조가 추가 되었습니다.

- `RedGain`
- `GreenGain`
- `BlueGain`

최대 잔액 이득은 현재 4 (4) 이며 `MaxWhiteBalanceGain` 속성에서 준비할 수 있습니다. 따라서 올바른 범위는 1에서 `MaxWhiteBalanceGain` (4) 사이입니다.

`DeviceWhiteBalanceGains` 속성을 사용 하 여 현재 값을 관찰할 수 있습니다. 카메라의 잠금 상태를 유지 하는 경우 `SetWhiteBalanceModeLockedWithDeviceWhiteBalanceGains`를 사용 하 여 균형을 조정 합니다.

#### <a name="conversion-routines"></a>변환 루틴

변환 루틴은 장치 독립적 색 공간으로 변환 하는 데 도움이 되도록 iOS 8에 추가 되었습니다. 변환 루틴을 구현 하기 위해 다음 멤버와 함께 `AVCaptureWhiteBalanceChromaticityValues` 구조가 추가 되었습니다.

- `X`-0에서 1 사이의 값입니다.
- `Y`-0에서 1 사이의 값입니다.

다음 멤버를 사용 하 여 `AVCaptureWhiteBalanceTemperatureAndTintValues` 구조도 추가 됩니다.

- `Temperature`-절대 온도 단위로 된 부동 소수점 값입니다.
- `Tint`-0에서 150 사이의 오프셋으로, 양수 값은 녹색 방향으로, 자홍에서는 음수입니다.

`CaptureDevice.GetTemperatureAndTintValues`및 `CaptureDevice.GetDeviceWhiteBalanceGains`메서드를 사용 하 여 온도와 색조, chromaticity 및 RGB 게인 색 공간 간을 변환 합니다.
...
이는 사용자에 게 카메라 앞에 실제 회색 카드를 배치할지를 묻는 메시지를 표시 하 고, `GrayWorldDeviceWhiteBalanceGains` 속성을 모니터링 하 고, 값이 세분화 될 때까지 기다리는 방법으로 응용 프로그램에서 구현 될 수 있습니다.

그런 다음 응용 프로그램은 `GrayWorldDeviceWhiteBalanceGains` 속성의 값을 사용 하 여 `SetWhiteBalanceModeLockedWithDeviceWhiteBalanceGains` 메서드에 대 한 균형 향상을 잠그고 변경 내용을 적용 합니다.
...
- 비디오 피드를 표시 하는 `UIImageView`입니다.
- 포커스 모드를 자동에서 잠김으로 변경 하는 `UISegmentedControl`입니다.
- 온도와 색조를 표시 하 고 업데이트 하는 두 개의 `UISlider` 컨트롤
- 회색 카드 (회색 세계) 공간을 샘플링 하 고 해당 값을 사용 하 여 백색 잔액을 설정 하는 데 사용 되는 `UIButton`입니다.
...
대괄호로 묶인 모든 캡처 명령은 `AVCaptureStillImageOutput` 클래스에서 구현 됩니다. 지정 된 설정 배열을 사용 하 여 일련의 이미지를 가져오려면 `CaptureStillImageBracket`메서드를 사용 합니다.
...
- `PrepareToCaptureStillImageBracket` 메서드를 호출 하 여 최악의 캡처 상황을 대비해 앱을 준비 합니다.
- 샘플 버퍼가 동일한 공유 풀에서 제공 되는 것으로 가정 합니다.
- 이전 준비 호출에 의해 할당 된 메모리를 해제 하려면 `PrepareToCaptureStillImageBracket`을 다시 호출 하 고 하나의 개체 배열을 보냅니다.
...
- 단일 캡처에 `MaxBracketedCaptureStillImageCount` 개 이상의 이미지를 요청 하지 마세요.
...
- 비디오 피드를 표시 하는 `UIImageView`입니다.
- 캡처 결과를 표시 하는 세 개의 `UIImageViews`입니다.
- 비디오 피드 및 결과 보기를 저장할 `UIScrollView`입니다.
- 일부 기본 설정으로 대괄호로 묶인 캡처를 수행 하는 데 사용 되는 `UIButton`입니다.
...
1. `ViewDidAppear` 메서드를 재정의 하 고 다음 코드를 추가 합니다.
