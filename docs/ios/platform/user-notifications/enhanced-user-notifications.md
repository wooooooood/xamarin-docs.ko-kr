...
새 ios 10, 앱 알림을 처리할 수 다르게 포그라운드에서 되 고 알림이 트리거됩니다. 제공 하 여는 `UNUserNotificationCenterDelegate` 구현 및는 `WillPresentNotification` 메서드, 앱 알림을 표시 하는 것에 대 한 책임 걸릴 수 있습니다. 예를 들어:
