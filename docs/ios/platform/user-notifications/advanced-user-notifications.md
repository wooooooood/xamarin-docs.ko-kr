...
서비스 확장에서 원격 알림을 수정 하려면 해당 페이로드를 변경 가능으로 표시 해야 합니다. 예를 들면,
...
원격 알림이 장치 (APNs를 통해)에 전달 되 면 서비스 확장에서 원하는 모든 수단 (예: `NSURLSession`)을 통해 필요한 이미지를 다운로드 하 고 이미지를 받은 후에 알림 콘텐츠를 수정 하 여에 표시할 수 있습니다. 사용자입니다.
...
APNs에서 알림을 받으면 이미지의 사용자 지정 주소를 콘텐츠에서 읽고 파일이 서버에서 다운로드 됩니다. 그런 다음 고유한 ID 및 이미지의 로컬 위치 (`NSUrl`)를 사용 하 여 `UNNotificationAttachement`를 만듭니다. 변경 가능한 알림 콘텐츠의 복사본이 만들어지고 미디어 첨부 파일이 추가 됩니다. 마지막으로 `contentHandler`를 호출 하 여 사용자에 게 알림을 표시 합니다.

알림에 첨부 파일이 추가 되 면 시스템은 파일의 이동 및 관리를 수행 합니다.

위에 나와 있는 원격 알림 외에도, 미디어 첨부 파일은 로컬 알림에서 지원 되며, `UNNotificationAttachement` 만들어지고 해당 콘텐츠와 함께 알림에 첨부 됩니다.
...
`DidReceiveNotification` 메서드는 알림 콘텐츠 확장이 사용자 지정 UI를 `UNNotification`콘텐츠로 채울 수 있도록 사용자가 알림을 확장할 때 호출 됩니다. 위의 예에서는 레이블이 `label`으로 코드에 노출 되 고 알림의 본문을 표시 하는 데 사용 되는 레이블이 뷰에 추가 되었습니다.
...
1. **Solution Pad** 에서 확장의 `Info.plist` 파일을 두 번 클릭 하 여 편집용으로 엽니다.
2. **원본** 뷰로 전환 합니다.
3. `NSExtension` 키를 확장 합니다.
4. 확장명이 속하는 범주의 값을 사용 하 여 `UNNotificationExtensionCategory` 키를 형식 **문자열로** 추가 합니다 (이 예제에서는 ' 이벤트-초대 '). 
...
1. **솔루션 탐색기** 에서 확장의 `Info.plist` 파일을 두 번 클릭 하 여 편집용으로 엽니다.
2. `NSExtension` 키를 확장 합니다.
3. 확장명이 속하는 범주의 값을 사용 하 여 `UNNotificationExtensionCategory` 키를 형식 **문자열로** 추가 합니다 (이 예제에서는 ' 이벤트-초대 '). 
...
알림 콘텐츠 확장 범주 (`UNNotificationExtensionCategory`)는 알림 작업을 등록 하는 데 사용 되는 것과 동일한 범주 값을 사용 합니다. 앱에서 여러 범주에 대해 동일한 UI를 사용 하는 경우 `UNNotificationExtensionCategory`를 형식 **배열로** 전환 하 고 필요한 모든 범주를 제공 합니다. 예를 들면,
...
사용자 지정 알림 UI가 기본 알림과 동일한 콘텐츠를 표시 하는 경우 (제목, 부제목 및 본문은 알림 UI의 맨 아래에 자동으로 표시 됨)이 기본 정보는를 추가 하 여 숨길 수 있습니다 `UNNotificationExtensionDefaultContentHidden` 확장의 `Info.plist` 파일에서 값이 `YES` 인 **부울** 형식의 `NSExtensionAttributes` 키에 대 한 키입니다.
...
알림 콘텐츠 확장의 사용자 지정 사용자 인터페이스를 디자인 하려면 `MainInterface.storyboard` 파일을 두 번 클릭 하 여 편집용으로 엽니다. iOS 디자이너에서 편집 하려면 원하는 인터페이스를 작성 하는 데 필요한 요소 (예: `UILabels` 및 `UIImageViews`)를 끌어 놓습니다.
...
UI가 배치 되 고 필요한 컨트롤이 코드에 C# 노출 되 면 편집을 위해`NotificationViewController.cs`를 열고`DidReceiveNotification`메서드를 수정 하 여 사용자가 알림을 확장할 때 ui를 채웁니다. 예를 들면,
...
사용자에 게 표시 되는 콘텐츠 영역의 크기를 조정 하려면 아래 코드에서 `ViewDidLoad` 메서드의 `PreferredContentSize` 속성을 원하는 크기로 설정 합니다. IOS 디자이너에서 뷰에 제약 조건을 적용 하 여이 크기를 조정할 수도 있습니다 .이 크기는 개발자에 게 가장 적합 한 방법을 선택 하는 것입니다.

알림 콘텐츠 확장을 호출 하기 전에 알림 시스템이 이미 실행 되 고 있기 때문에 콘텐츠 영역은 전체 크기를 시작 하 고 사용자에 게 표시 될 때 요청 된 크기에 애니메이션을 적용 합니다.

이 효과를 제거 하려면 확장에 대 한 `Info.plist` 파일을 편집 하 고 `NSExtensionAttributes` 키의 `UNNotificationExtensionInitialContentSizeRatio` 키를 원하는 비율을 나타내는 값으로 형식 **Number** 로 설정 합니다. 예를 들면,
...
예를 들어 위의 사용자 지정 UI에 코드에 C# 노출 된 `UIImageView` 포함 되어 있는 경우 다음 코드를 사용 하 여 미디어 첨부 파일에서로 채울 수 있습니다.
...
미디어 첨부 파일은 시스템에 의해 관리 되기 때문에 앱의 샌드박스 외부에 있습니다. 확장은 `StartAccessingSecurityScopedResource` 메서드를 호출 하 여 파일에 대 한 액세스를 필요로 함을 시스템에 알려야 합니다. 확장명이 파일을 사용 하 여 수행 되는 경우 연결을 해제 하려면 `StopAccessingSecurityScopedResource`를 호출 해야 합니다.
...
완료 처리기를 포함 하는 `DidReceiveNotification` 메서드의 두 번째 버전을 구현 하 여이 작업을 수행 합니다. 예를 들면,
...
알림 콘텐츠 확장의 `DidReceiveNotification` 메서드에 `Server.PostEventResponse` 처리기를 추가 하 여 확장은 모든 사용자 지정 작업을 처리 *해야* 합니다. 확장은 `UNNotificationContentExtensionResponseOption`를 변경 하 여 포함 하는 앱에 대 한 사용자 지정 작업을 전달할 수도 있습니다. 예를 들면,
