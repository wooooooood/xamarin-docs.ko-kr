...
앱은 `NSUserActivity`, 웹 마크업, 핵심 스포트라이트, MapKit, Media Player 및 UIKit와 같은 기술 컬렉션을 사용 하 여이 기능을 시스템에 노출 합니다. 또한 앱에 대 한 사전 제안 지원을 제공 하 여 더 심층적인 Siri 통합을 무료로 제공 합니다.

## <a name="location-based-suggestions"></a>위치 기반 제안

IOS 10의 새로운 기능으로, `NSUserActivity` 클래스에는 개발자가 다른 컨텍스트에서 사용할 수 있는 위치 정보를 제공할 수 있도록 하는 `MapItem` 속성이 포함 되어 있습니다. 예를 들어 앱에서 식당 리뷰를 표시 하는 경우 개발자는 사용자가 앱에서 보고 있는 식당의 위치로 `MapItem` 속성을 설정할 수 있습니다. 사용자가 Maps 앱으로 전환 하는 경우 식당의 위치를 자동으로 사용할 수 있습니다.
...
예를 들어 사용자가 `UITextContentType.FullStreetAddress`표시 된 텍스트 필드에 텍스트를 입력 하는 경우 시스템은 사용자가 최근에 보고 있던 위치로 필드를 자동으로 채우는 것을 제안할 수 있습니다.
...
[MKDirectionsRequest](xref:MapKit.MKDirectionsRequest) API를 사용 하는 사용자가 공유 하는 앱이 있는 경우, iOS 10은 사용자가 원하는 경우에 앱 전환기에서 옵션으로 표시 합니다. 또한 `Info.plist` 파일에서 [MKDirectionsApplicationSupportedModes](https://developer.apple.com/library/content/documentation/General/Reference/InfoPlistKeyReference/Articles/iPhoneOSKeys.html) 키에 대 한 `MKDirectionsModeRideShare`를 지정 하 여 해당 앱을 타는 공유 앱으로 등록 해야 합니다.
...
- **`NSUserActivity` 및 Schema.org** - `NSUserActivity`를 사용 하면 시스템에서 사용자가 현재 화면에서 작업 하 고 있는 정보를 이해할 수 있습니다. Schema.org는 웹 페이지에 비슷한 기능을 추가 합니다.
...
- 전달 기능을 지원 하기 **위해 - `NSUserActivity`** 를 iOS 8에 추가 했습니다 .이를 통해 개발자는 한 장치에서 작업을 시작 하 고 다른 장치에서 작업을 계속할 수 있습니다 ( [핸드 오프 소개](~/ios/platform/handoff.md)참조).
- **스포트라이트 검색** -iOS 9 `NSUserActivity`를 사용 하 여 스포트라이트 검색 결과 내에서 앱 콘텐츠를 승격 하는 기능을 추가 했습니다 ( [Core 스포트라이트를 사용 하 여 검색](~/ios/platform/search/corespotlight.md)참조).
- **상황별 Siri 미리 알림** -iOS 10에서는 siri가 나중에 앱에서 현재 보고 있는 콘텐츠를 확인 하는 미리 알림을 신속 하 게 만들 수 있도록 `NSUserActivity` 확장 되었습니다.
...
- **연락처 상호 작용** -iOS 10의 새로운 기능으로, `NSUserActivity` 통신 앱을 연락처 카드 (연락처 앱)에서 대체 통신 방법으로 승격할 수 있습니다.
...
개발자는 위에서 만든 활동과 동일한 활동 유형 식별자 (`com.xamarin.platform`) 인지 확인 해야 합니다. 앱은 `NSUserActivity`에 저장 된 정보를 사용 하 여 사용자가 남은 위치로 상태를 다시 복원 합니다.
...
5. 이는 타사 앱 (`NSUserActivity`지원) 에서도 작동 하므로 사용자는이를 통해 사용자가 타는 공유 앱으로 전환할 수 있으며 식당의 주소도 자동으로 대상으로 제안 됩니다.
...
연락처 상호 작용은 iOS 10에 도입 된 `NSUserActivity` 및 새로운 의도 프레임 워크를 사용 하 여 앱에서 구현 됩니다. 의도로 작업 하는 방법에 대 한 자세한 내용은 [Sirikit 개념 이해](~/ios/platform/sirikit/understanding-sirikit.md) 및 [Sirikit 구현](~/ios/platform/sirikit/implementing-sirikit.md) 가이드를 참조 하세요.
...
앱은 **의도** (`INIntent`), **참가자** 및 **메타 데이터**를 포함 하는 `INInteraction` 개체를 만듭니다. **의도** 는 비디오 통화 또는 문자 메시지 전송과 같은 사용자 작업을 나타냅니다. **참가자** 에 게는 통신을 받는 사람이 포함 됩니다. **메타 데이터** 는 메시지를 성공적으로 보낸 등의 추가 정보를 정의 합니다.

개발자는 `INIntent` 또는 `INIntentResponse`의 인스턴스를 직접 만들지 않습니다. 이러한 부모 클래스에서 상속 되는 특정 자식 클래스 (사용자를 대신 하 여 앱에서 수행 하는 작업에 따라) 중 하나를 사용 합니다. 예를 들어 문자 메시지를 보내기 위한 `INSendMessageIntent` 및 `INSendMessageIntentResponse`. 

상호 작용이 완전히 채워지면 `DonateInteraction` 메서드를 호출 하 여 상호 작용을 사용할 수 있다는 사실을 시스템에 알립니다.

사용자가 연락처 카드에서 앱과 상호 작용할 때 상호 작용은 `NSUserActivity`함께 번들로 묶여 앱을 시작 하는 데 사용 됩니다.
...
이 코드를 자세히 살펴보면 위의 [작업 만들기](#creating-an-activity) 섹션에 표시 된 것 처럼 `NSUserActivity`의 인스턴스를 만들고 채웁니다. 그런 다음 `INIntent`에서 상속 되는 `INSendMessageIntent`의 인스턴스를 만들고 전송 되는 메시지의 세부 정보로 채웁니다.

```csharp
var intent = new INSendMessageIntent (to, text, "", "MonkeyChat", from);
```

`INSendMessageIntentResponse` 만들어지고 위에서 만든 `NSUserActivity` 전달 됩니다.

```csharp
var response = new INSendMessageIntentResponse (INSendMessageIntentResponseCode.Success, activity);
```

`INInteraction`은 전송 메시지 의도 (`INSendMessageIntent`) 및 응답 (`INSendMessageIntentResponse`)을 기반으로 작성 됩니다.
...
위와 같이 `NSUserActivity`는 시스템에서 사용자가 현재 화면에서 작업 하 고 있는 정보를 이해 하는 데 도움이 됩니다. Schema.org는 웹 페이지에 비슷한 기능을 추가 합니다.
...
- 개발자는 웹 사이트에서 구현 하 여 네이티브 앱에서 `NSUserActivity`를 사용 하는 몇 가지 이점을 얻을 수 있습니다.
...
IOS 10에 있는 텍스트 필드 컨트롤의 새 `TextContentType` 속성을 사용 하면 개발자는 지정 된 필드에 사용자가 입력 하는 값에 대 한 의미 체계 의도를 정의할 수 있습니다. 예를 들면,
...
개발자는 iOS 10에 간단한 API 호출을 포함 하 여 미디어 앱의 사용자에 게 더욱 매력적인 잠금 화면 환경을 만들 수 있습니다. `MPPlayableContentManager` 클래스를 사용 하 여 미디어 재생을 관리 하면, 전체 미디어 컨트롤 (음악 앱에서 제공 하는 것과 같은)이 앱의 잠금 화면에 표시 됩니다.
