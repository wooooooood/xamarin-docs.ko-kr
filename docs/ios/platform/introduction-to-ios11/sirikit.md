...
예를 들어 `IINCreateTaskListIntent` 인터페이스의 경우 필요한 메서드가 `HandleCreateTaskList`됩니다. Siri 상호 작용에 대 한 제어를 강화 하는 네 가지 선택적 메서드가 있습니다.
...
- `ConfirmCreateTaskList` – 코드에서 요청 된 작업을 수행할 수 있지만이를 수행 하지 않는지 확인 합니다 (`Handle*` 메서드만 데이터를 수정 해야 함).
...
각 메서드에는 전달 되는 특정 의도 형식이 있습니다. 여기에는 Siri가 사용자 요청에서 구문 분석 한 모든 정보 (`Resolve*` 및 `Confirm*` 메서드에서 업데이트 될 수 있음)가 포함 됩니다.
앱은 제공 된 데이터를 구문 분석 한 다음 데이터를 저장 하거나 처리 하는 몇 가지 작업을 수행 하 고 Siri가 사용자에 게 표시 하는 결과를 반환 해야 합니다.

### <a name="response-codes"></a>응답 코드

필요한 `Handle*` 및 선택적 `Confirm*` 메서드는 완료 처리기에 전달 되는 개체의 값을 설정 하 여 응답 코드를 표시 합니다. 응답은 `INCreateTaskListIntentResponseCode` 열거에서 제공 됩니다.

- `Ready` – 확인 단계를 수행 하는 동안 반환 됩니다 (`Confirm*` 메서드에서는 `Handle*` 메서드에서는 그렇지 않음).
- `InProgress`-장기 실행 작업 (예: 네트워크/서버 작업)에 사용 됩니다.
- `Success` – 성공한 작업에 대 한 세부 정보를 사용 하 여 응답 합니다 (`Handle*` 메서드에서만).
- `Failure` – 오류가 발생 하 여 작업을 완료할 수 없음을 의미 합니다.
- `RequiringAppLaunch` – 의도에 따라 처리할 수 없지만 응용 프로그램에서 작업을 수행할 수 있습니다.
- `Unspecified` – 사용 하지 마세요. 사용자에 게 오류 메시지가 표시 됩니다.
...
3. 응용 프로그램에서 `INPreferences.RequestSiriAuthorization` 메서드를 호출 하 여 사용자에 게 Siri 상호 작용을 허용할지 묻는 메시지를 표시 합니다.
...
`IntentHandler` 클래스는 Siri 요청에 대 한 진입점입니다. 모든 의도는 요청을 처리할 수 있는 개체를 반환 하는 `GetHandler` 메서드에 전달 됩니다.
...
이 클래스는 `INExtension`에서 상속 해야 하며,이 샘플에서는 목록 및 메모 의도를 처리 하기 때문에 `IINNotebookDomainHandling`도 구현 합니다.

> [!NOTE]
>
> - .NET에는 인터페이스에 대문자 `I`접두사가 추가 되는 규칙이 있습니다 .이는 Xamarin에서 iOS SDK의 프로토콜을 바인딩하는 경우에 준수 합니다.
> - 또한 Xamarin은 iOS에서 형식 이름을 유지 하며, Apple은 형식이 속한 프레임 워크를 반영 하기 위해 형식 이름에 처음 두 문자를 사용 합니다.
> - `Intents` 프레임 워크의 경우 형식에는 `IN*` (예: `INExtension`) 하지만 인터페이스가 _아닙니다_ .
> - 또한이 프로토콜 (의 C#인터페이스가 됨)은`IINAddTasksIntentHandling`와 같이 두 개의`I`를 사용 하 여 종료 됩니다.

#### <a name="handling-intents"></a>처리 의도

각 의도 (작업 추가, 작업 특성 설정 등)는 아래에 표시 된 것과 유사한 단일 메서드로 구현 됩니다. 메서드는 세 가지 주요 함수를 수행 해야 합니다.

1. **의도를 처리** 합니다. siri에서 구문 분석 한 데이터는 의도 유형과 관련 된 `intent` 개체에서 사용할 수 있습니다. 앱이 선택적 `Resolve*` 메서드를 사용 하 여 해당 데이터의 유효성을 검사 했을 수 있습니다.
...
`null`는 응답에 두 번째 매개 변수로 전달 됩니다. 이것은 사용자 활동 매개 변수이 고, 제공 되지 않으면 기본값이 사용 됩니다.
IOS 앱이 **info.plist**의 `NSUserActivityTypes` 키를 통해 지 원하는 경우 사용자 지정 활동 형식을 설정할 수 있습니다. 그러면 앱이 열릴 때이 경우를 처리 하 고 특정 작업 (예: 관련 보기 컨트롤러에 열고 Siri 작업에서 데이터를 로드 하는 등)을 수행할 수 있습니다.
