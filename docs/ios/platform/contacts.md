...
클래스 `CNContact` 는 이름, 주소 또는 전화 번호와 같은 연락처의 속성에 대 한 읽기 전용 액세스를 스레드로부터 안전 하 게 제공 합니다. `CNContact`및와 같은 `NSDictionary` 함수는 여러 개의 읽기 전용 속성 컬렉션 (예: 주소 또는 전화 번호)을 포함 합니다.

[![](contacts-images/contactobjects.png "연락처 개체 개요")](contacts-images/contactobjects.png#lightbox)

전자 메일 주소 또는 전화 번호와 같이 여러 값을 가질 수 있는 속성의 경우에는 `NSLabeledValue` 개체 배열로 표시 됩니다. `NSLabeledValue`레이블이 사용자에 게 값을 정의 하는 읽기 전용 레이블 및 값 (예: Home 또는 Work email)으로 구성 된 스레드로부터 안전한 튜플입니다. 연락처 프레임 워크는 응용 프로그램에서 사용할 수 있는 미리 정의 `CNLabelKey` 된 `CNLabelPhoneNumberKey` 레이블 (및 정적 클래스를 통해)을 선택 하거나 사용자 요구에 맞게 사용자 지정 레이블을 정의할 수 있습니다.

기존 연락처의 값을 조정 해야 하는 xamarin.ios 앱 (또는 새로 만들기)의 경우 클래스 및 해당 하위 클래스의 `NSMutableContact` 버전 ( `CNMutablePostalAddress`예:)을 사용 합니다.
...
`CNContactStore` 클래스의 인스턴스를 사용 하 여 사용자의 연락처 데이터베이스에서 연락처 정보를 가져올 수 있습니다. 에 `CNContactStore` 는 데이터베이스에서 연락처와 그룹을 페치 하거나 업데이트 하는 데 필요한 모든 메서드가 포함 되어 있습니다. 이러한 메서드는 동기적 이므로 UI를 차단 하지 않도록 백그라운드 스레드에서 실행 하는 것이 좋습니다.

`CNContact` 클래스에서 빌드된 조건자를 사용 하 여 데이터베이스에서 연락처를 가져올 때 반환 되는 결과를 필터링 할 수 있습니다. 문자열 `Appleseed`을 포함 하는 연락처만 페치 하려면 다음 코드를 사용 합니다.
...
최종 사용자는 응용 프로그램 별로 연락처 정보에 대 한 액세스 권한을 부여 하거나 거부할 수 있으므로를 처음으로 `CNContactStore`호출 하면 앱에 대 한 액세스를 허용 하도록 요청 하는 대화 상자가 표시 됩니다.

권한 요청은 앱을 처음 실행할 때 한 번만 표시 되 고 후속 실행 또는에 대 `CNContactStore` 한 호출은 사용자가 해당 시간에 선택한 사용 권한을 사용 하 게 됩니다.
...
> [!IMPORTANT]
> 클래스의 `GetUnifiedContacts` `GetUnifiedContact` 및 메서드는 제공 된 fetch 키에서 요청 된 속성으로 제한 된 부분 연결만 반환 합니다. `CNContactStore`
...
[![](contacts-images/unified01.png "통합 연락처 개요")](contacts-images/unified01.png#lightbox)
...
위의 [Contact Objects](#Contact_Objects) 섹션에서 살펴본 것 처럼, `CNContactStore` 및 인스턴스 `CNMutableContact` 를 사용 하 여 새 연락처를 만든 다음을 `CNSaveRequest`사용 하 여 사용자의 연락처 데이터베이스에 기록 합니다.
...
연락처가 수정 될 때마다 연락처 저장소는를 `CNContactStoreDidChangeNotification` 기본 알림 센터에 게시 합니다. 캐시 되었거나 현재 연락처를 표시 하 고 있는 경우 연락처 저장소 (`CNContactStore`)에서 해당 개체를 새로 고쳐야 합니다.
...
[![](contacts-images/containers02.png "컨테이너 및 그룹 내에서 겹치기")](contacts-images/containers02.png#lightbox)
...
연락처 선택기 보기 컨트롤러 (`CNContactPickerViewController`)는 사용자가 사용자의 연락처 데이터베이스에서 연락처 또는 연락처 속성을 선택할 수 있는 표준 연락처 선택기 보기를 관리 합니다. 사용자는 해당 사용량에 따라 하나 이상의 연락처를 선택할 수 있으며, 연락처 선택기 보기 컨트롤러는 선택 항목을 표시 하기 전에 사용 권한을 묻는 메시지를 표시 하지 않습니다.
...
Contact view Controller (`CNContactViewController`) 클래스는 최종 사용자에 게 표준 연락처 보기를 제공 하는 컨트롤러를 제공 합니다. 연락처 보기는 새로운 새, 알 수 없음 또는 기존 연락처를 표시할 수 있으며, 올바른 정적 생성자 (`FromNewContact`, `FromUnknownContact`, `FromContact`)를 호출 하 여 뷰가 표시 되기 전에 형식을 지정 해야 합니다. 예를 들면 다음과 같습니다.
