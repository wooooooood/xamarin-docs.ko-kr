...
> [!IMPORTANT]
> IOS 8 (및 이전 버전)에서 사용 하는 기존 `AddressBook` 및 `AddressBookUI` 프레임 워크는 iOS 9에서 더 이상 사용 되지 않으며, 기존 Xamarin.ios 앱의 경우 가능한 한 빨리 새로운 `Contacts` 및 `ContactsUI` 프레임 워크로 바꾸어야 합니다. 새 프레임 워크에 대해 새 앱을 작성 해야 합니다.
...
`CNContact` 클래스는 이름, 주소 또는 전화 번호와 같은 연락처의 속성에 대 한 스레드로부터 안전 하 고 읽기 전용 액세스를 제공 합니다. `CNContact`은 `NSDictionary`와 같이 함수를 수행 하 고, 여러 개의 읽기 전용 속성 컬렉션 (예: 주소 또는 전화 번호)을 포함 합니다.

[![](contacts-images/contactobjects.png "Contact Object overview")](contacts-images/contactobjects.png#lightbox)

전자 메일 주소 또는 전화 번호와 같이 여러 값을 가질 수 있는 모든 속성의 경우 `NSLabeledValue` 개체의 배열로 표시 됩니다. `NSLabeledValue`은 레이블이 사용자에 게 값을 정의 하는 읽기 전용 레이블 및 값 집합으로 구성 된 스레드로부터 안전한 튜플입니다 (예: Home 또는 Work email). 연락처 프레임 워크는 응용 프로그램에서 사용할 수 있는 미리 정의 된 레이블 (`CNLabelKey` 및 `CNLabelPhoneNumberKey` 정적 클래스를 통해)을 선택 하거나 요구 사항에 맞게 사용자 지정 레이블을 정의할 수 있습니다.

기존 연락처의 값을 조정 해야 하는 Xamarin.ios 앱 (또는 새로 만들기)의 경우 클래스 및 해당 하위 클래스의 `NSMutableContact` 버전을 사용 합니다 (예: `CNMutablePostalAddress`).
...
이 코드가 iOS 9 장치에서 실행 되는 경우 새 연락처가 사용자의 컬렉션에 추가 됩니다. 예를 들면,
...
응용 프로그램의 UI에 표시 되는 속성 레이블의 경우 Contact framework에도 이러한 문자열을 지역화 하는 메서드가 있습니다. 이는 앱이 실행 되는 iOS 장치의 현재 로캘을 기반으로 합니다. 예를 들면,
...
`CNContactStore` 클래스의 인스턴스를 사용 하 여 사용자의 연락처 데이터베이스에서 연락처 정보를 가져올 수 있습니다. `CNContactStore`는 데이터베이스에서 연락처와 그룹을 가져오거나 업데이트 하는 데 필요한 모든 메서드를 포함 합니다. 이러한 메서드는 동기적 이므로 UI를 차단 하지 않도록 백그라운드 스레드에서 실행 하는 것이 좋습니다.

`CNContact` 클래스에서 빌드된 조건자를 사용 하 여 데이터베이스에서 연락처를 가져올 때 반환 되는 결과를 필터링 할 수 있습니다. `Appleseed`문자열을 포함 하는 연락처만 페치 하려면 다음 코드를 사용 합니다.
...
최종 사용자는 응용 프로그램 별로 연락처 정보에 대 한 액세스 권한을 부여 하거나 거부할 수 있으므로, 처음으로 `CNContactStore`를 호출 하면 앱에 대 한 액세스를 허용 하도록 요청 하는 대화 상자가 표시 됩니다.

권한 요청은 앱을 처음 실행할 때 한 번만 표시 되 고 후속 실행 또는 `CNContactStore` 호출은 사용자가 해당 시간에 선택한 사용 권한을 사용 하 게 됩니다.
...
`CNContact` 인스턴스의 `IsKeyAvailable` 또는 `AreKeysAvailable` 메서드를 사용 하 여 지정 된 연락처에 원하는 속성이 있는지 쉽게 확인할 수 있습니다. 예를 들면,
...
> [!IMPORTANT]
> `CNContactStore` 클래스의 `GetUnifiedContact` 및 `GetUnifiedContacts` 메서드는 제공 된 fetch 키에서 요청 된 속성으로 제한 된 부분 _연결만_ 반환 합니다.
...
위의 [Contact Objects](#Contact_Objects) 섹션에서 살펴본 것 처럼 `CNContactStore` 및 `CNMutableContact` 인스턴스를 사용 하 여 새 연락처를 만든 다음 `CNSaveRequest`를 사용 하 여 사용자의 연락처 데이터베이스에 기록 합니다.
...
`CNSaveRequest`를 사용 하 여 여러 연락처 및 그룹 변경 내용을 단일 작업으로 캐시 하 고 이러한 수정 내용을 `CNContactStore`로 일괄 처리할 수도 있습니다.

Fetch 작업에서 가져온 변경할 수 없는 연락처를 업데이트 하려면 먼저 변경 가능한 복사본을 요청한 다음 수정 하 고 연락처 저장소에 다시 저장 해야 합니다. 예를 들면,
...
연락처가 수정 될 때마다 연락처 저장소는 `CNContactStoreDidChangeNotification`를 기본 알림 센터에 게시 합니다. 캐시 되었거나 현재 연락처를 표시 하 고 있는 경우 연락처 저장소 (`CNContactStore`)에서 해당 개체를 새로 고쳐야 합니다.
...
연락처 선택기 보기 컨트롤러 (`CNContactPickerViewController`)는 사용자의 연락처 데이터베이스에서 연락처 또는 연락처 속성을 선택할 수 있는 표준 연락처 선택기 보기를 관리 합니다. 사용자는 해당 사용량에 따라 하나 이상의 연락처를 선택할 수 있으며, 연락처 선택기 보기 컨트롤러는 선택 항목을 표시 하기 전에 사용 권한을 묻는 메시지를 표시 하지 않습니다.

`CNContactPickerViewController` 클래스를 호출 하기 전에 사용자가 선택할 수 있는 속성을 정의 하 고 연락처 속성의 표시 및 선택을 제어 하는 조건자를 정의 합니다.

`CNContactPickerDelegate`에서 상속 되는 클래스의 인스턴스를 사용 하 여 선택기와의 사용자 상호 작용에 응답 합니다. 예를 들면,
...
연락처 보기 컨트롤러 (`CNContactViewController`) 클래스는 최종 사용자에 게 표준 연락처 보기를 제공 하는 컨트롤러를 제공 합니다. 연락처 보기는 새로운 새, 알 수 없음 또는 기존 연락처를 표시할 수 있으며 올바른 정적 생성자 (`FromNewContact`, `FromUnknownContact`, `FromContact`)를 호출 하 여 뷰가 표시 되기 전에 유형을 지정 해야 합니다. 예를 들면 다음과 같습니다.
