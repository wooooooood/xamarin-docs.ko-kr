...
1. 메시지 앱 확장의 개체는확장의뷰가사용자에게표시될때호출되는주클래스입니다.`MSMessageAppViewController`
2. 대화는 사용자 `MSConversation` 에 게 개체 인스턴스로 제공 됩니다.
3. 클래스 `MSMessage` 는 대화의 지정 된 메시지 거품형을 나타냅니다.
...
[![](advanced-message-app-extensions-images/interactive04.png "메시지 앱 확장을 활성화 하는 프로세스입니다.")](advanced-message-app-extensions-images/interactive04.png#lightbox)

1. 앱 서랍에서와 같이 확장이 시작 되 면 메시지 앱이 프로세스를 시작 합니다.
2. 메서드를 호출 하 고 메시지 앱 `MSConversation` 확장이 실행 되는 대화를 나타내는를 전달 합니다. `DidBecomeActive`
3. 확장은 `UIViewController` `ViewWillAppear`둘다 를기반으로하기때문에를호출합니다.`ViewDidAppear`

다음으로 비활성화 되는 메시지 앱 확장 프로세스를 살펴보겠습니다.

[![](advanced-message-app-extensions-images/interactive05.png "메시지 앱 확장을 비활성화 하는 프로세스입니다.")](advanced-message-app-extensions-images/interactive05.png#lightbox)

1. 메시지 앱 확장을 비활성화 `ViewWillDisappear` 하는 경우 메서드가 먼저 호출 됩니다.
2. 그런 다음 `ViewDidDisappear` 메서드가 호출 됩니다.
3. 메서드를 호출 하 고 메시지 앱 `MSConversation` 확장이 실행 되는 대화를 나타내는를 전달 합니다. `WillResignActive` 이 시점에서 메시지 앱과 확장의 연결을 해제 하려고 합니다.
...
이 코드는 새 `MSMessage` 를 만들고 여러 속성을 설정 합니다 ( `Url`예:). IOS 에서만 메시지를 만들 수 있지만 표시 되려면 iOS와 macOS 모두에 메시지를 보낼 수 있습니다.
...
속성 `AccessibilityLabel` 은 화면 판독기에서 사용자에 대 한 대화의 기록을 읽는 데 사용 됩니다. 속성 `Layout` 은 메시지가 표시 되는 방법을 지정 합니다. 현재 `MSMessageTemplateLayout` 만 지원 되며 다음과 같습니다.
...
의 속성은 `Image` 화면에 messagebubble의 본문에 대 한 콘텐츠를 제공합니다.`MSMessageTemplateLayout` 또한 `MediaFileUrl` 속성은 메시지 거품형의 본문에 대 한 콘텐츠를 제공 하지만에서 `UIImage` 지원 하지 않는 콘텐츠 (예: 백그라운드에서 반복 되는 비디오 파일)를 허용 합니다. `Image` 및`MediaFileUrl`속성이모두 제공 되는 경우 속성이우선적으로적용됩니다.`Image` 는 `MediaFileUrl` PNG, JPEG, GIF 및 비디오 (Media Player 프레임 워크에서 재생할 수 있는 모든 형식) 미디어 형식을 지원 합니다.
...
`Caption` ,`SubCaption`및 속성은이미지에대해자세히설명하고이미지아래의섹션에서렌더링됩니다.`TrailingSubcaption` `TrailingCaption` 이러한 속성을 모두로 `null` 설정 하면 캡션 영역 없이 메시지 거품형이 생성 됩니다.

[![](advanced-message-app-extensions-images/interactive07.png "캡션 영역이 없는 메시지 거품형")](advanced-message-app-extensions-images/interactive07.png#lightbox)

마지막으로 메시지 앱은 메시지 거품형의 왼쪽 위 모퉁이에 메시지 앱 확장 아이콘을 그립니다.

## <a name="sending-a-message"></a>메시지 보내기

를 `MSMessage` 구성한 후에는 다음 코드를 사용 하 여 메시지를 보낼 수 있습니다.
...
의 속성에는 메시지 앱 확장이 시작 된 현재 대화가 포함 됩니다.`MSMessagesAppViewController` `ActiveConversation`

의를 `InsertMessage` 호출 하여대화에메시지를포함하고발생할수있는오류를처리합니다.`MSConversation` 메시지가 성공적으로 포함 되 면 메시지 버블이 입력 필드에 표시 됩니다.
...
- **가 인 스티커**  - . `ActiveConversation.InsertSticker (sticker, (obj) => {...});` `sticker` `MSSticker`
...
[![](advanced-message-app-extensions-images/interactive08.png "메시지 앱 확장은 다음 두 가지 보기 모드로 표시 됩니다. 압축 & 확장 됨")](advanced-message-app-extensions-images/interactive08.png#lightbox)

- **Compact** -메시지 앱 확장이 메시지 보기의 하위 25%를 차지 하는 기본 모드입니다. 압축 모드에서는 앱에 키보드, 가로 스크롤 또는 살짝 밀기 제스처 인식기에 대 한 액세스 권한이 없습니다. 앱은 입력 필드에 대 한 액세스 권한을 가지 며에 `InsertMessage` 대 한 호출은 사용자에 게 즉시 표시 됩니다.
...
두 가지 서로 다른 보기 모드 간의 전환을 처리 하는 다음 예를 살펴보겠습니다. 각 상태에는 두 개의 서로 다른 뷰 컨트롤러가 필요 합니다. 는 `StickerBrowserViewController` **Compact** 뷰를 처리 하 고 `AddStickerViewController` 는 **확장** 된 뷰를 처리 합니다.
...
필요에 따라 앱은 사용자에 게 `WillTransition` 표시 되기 전에 메서드를 사용 하 여 보기 모드 변경을 처리할 수 있습니다 (위의 icecream 빌더 예제에서 수행 된 것과 같이). 자세한 내용은 [추가 스티커 사용자 지정](~/ios/platform/message-app-integration/intro-to-message-app-extensions.md) 설명서를 참조 하세요.

## <a name="replying-to-a-message"></a>메시지에 회신

메시지에 회신할 때 메시지 앱 확장이 처리 해야 하는 두 가지 경우는 다음과 같습니다.

[![](advanced-message-app-extensions-images/interactive09.png "비활성 모드 및 활성 모드의 메시지 앱 확장")](advanced-message-app-extensions-images/interactive09.png#lightbox)
...
[![](advanced-message-app-extensions-images/interactive10.png "비활성 메시지 거품형 처리")](advanced-message-app-extensions-images/interactive10.png#lightbox)

1. 사용자가 확장의 메시지 거품형을 탭 합니다.
2. 확장이 시작 되 면 메시지 앱이 프로세스를 시작 합니다.
3. 메서드를 호출 하 고 메시지 앱 `MSConversation` 확장이 실행 되는 대화를 나타내는를 전달 합니다. `DidBecomeActive`
4. 확장은 `UIViewController` `ViewWillAppear`둘다 를기반으로하기때문에를호출합니다.`ViewDidAppear`
...
[![](advanced-message-app-extensions-images/interactive11.png "활성 메시지 거품형 처리")](advanced-message-app-extensions-images/interactive11.png#lightbox)

1. 사용자가 확장의 메시지 거품형을 탭 합니다.
2. 메시지 앱 확장이 이미 활성화 `WillTransition` 되어 있으므로 `MSMessagesAppViewController` 의 메서드를 호출 하 여 압축에서 확장 된 보기 모드로 전환 하는 것을 처리 합니다.
3. 의 메서드를 호출 하 고 메시지 거품이 속한 `MSMessage` 및 `MSConversation` 를 전달 합니다. `DidSelectMessage` `MSMessagesAppViewController`
4. 의 메서드는 압축 된 보기 모드로 전환 하는 것을처리하기위해호출됩니다.`MSMessagesAppViewController` `DidTransition`
...
[![](advanced-message-app-extensions-images/interactive12.png "부분적으로 완료 된 메시지 거품은 메시지 기록을 복잡 하 게 만들 수 있습니다.")](advanced-message-app-extensions-images/interactive12.png#lightbox)

대신 메시지 앱 확장은 이전 메시지 거품을 메시지 기록의 간결한 주석으로 축소 해야 합니다.

[![](advanced-message-app-extensions-images/interactive13.png "메시지 기록을 통해 이전 메시지 거품 축소")](advanced-message-app-extensions-images/interactive13.png#lightbox)

이는를 사용 하 `MSSession` 여 모든 기존 단계를 축소 하기 위해 처리 됩니다. 따라서 클래스`MSMessagesAppViewController` 의 메서드를다음과`DidSelectMessage` 같이 수정할 수 있습니다.
...
선택한 메시지가 이미 종료 `MSSession`된 경우에는 새 `MSSession` 가 만들어집니다. 의 속성은 축소 된 이전 단계에 캡션을 추가 하는 데 사용 됩니다.`MSMessage` `SummaryText` `SummaryText` 속성이 로`null`설정 되 면 대화의 이전 단계가 대화 성적에서 완전히 제거 됩니다.
...
[![](advanced-message-app-extensions-images/interactive14.png "3 명의 사용자를 사용 하는 그룹 대화의 상호 작용")](advanced-message-app-extensions-images/interactive14.png#lightbox)
...
두 경우 모두 사용자 1이 사용자 2와 사용자 3의 선택 항목에 모두 액세스할 수 있으므로이 동작은 바람직하지 않습니다. 이러한 상황을 처리 하기 위해 Apple에서는 메시지 앱 확장이 클라우드에 메시지 상태를 저장 하 고, `MSMessage` (사용자 간에 전송 됨)의 URL 속성을 사용 하 여이 상태에 액세스 하는 것을 제안 합니다.
...
[![](advanced-message-app-extensions-images/interactive15.png "그룹 대화 전송 식별자")](advanced-message-app-extensions-images/interactive15.png#lightbox)
...
4. Apple은 사용자 개인 정보 보호와 매우 관련이 있으므로, 메시지 앱 확장은 대화의 각 참가자에 게 할당 `NSUUID`되는 고유 식별자 ()만 알고 있습니다. 로컬 장치에서는 현재 사용자의 식별자만 알려집니다.
5. 에 `MSMessage` 는 확장 `SenderIdentifier` 에 알려진 참가자 목록의 사용자 중 하 나와 일치 하는 속성이 있습니다.
...
메시지 앱이 이러한 형식의 형식으로 메시지 거품형을 표시 하는 경우 메시지를 보낸 대화 `$uuid...` 에서 참가자의 연락처 이름으로를 바꿉니다.
...
세 플랫폼 중에서 iOS 10만 사용자가 대화형 메시지를 생성할 수 있습니다. MacOS Sierra에서 사용자가 대화형 메시지 거품형을 클릭 하면에 연결 `MSMessage` 된 URL이 Safari에서 열리고 메시지 표시가 표시 됩니다.
