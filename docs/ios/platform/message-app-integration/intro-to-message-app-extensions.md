...
> [!IMPORTANT]
> 스티커 이미지는 항상 300 x 300에서 618 x 618 픽셀 범위의 `@3x` 해상도로 제공 되어야 합니다. 시스템은 필요에 따라 런타임에 `@2x` 및 `@1x` 버전을 자동으로 생성 합니다.
...
기본적으로 `MessagesViewController.cs` 파일은 솔루션에 추가 됩니다. 이는 확장에 대 한 주 진입점 이며 `MSMessageAppViewController` 클래스에서 상속 됩니다.

메시지 프레임 워크는 사용자에 게 사용 가능한 스티커를 제공 하기 위한 클래스를 제공 합니다.

- `MSStickerBrowserViewController`-스티커가 표시 될 뷰를 제어 합니다. 또한 지정 된 브라우저 인덱스의 스티커 수와 스티커 수를 반환 하는 `IMSStickerBrowserViewDataSource` 인터페이스를 따릅니다.
- `MSStickerBrowserView`-사용 가능한 스티커가 표시 되는 보기입니다.
- `MSStickerSize`-브라우저 보기에 표시 되는 스티커 표의 개별 셀 크기를 결정 합니다.

### <a name="creating-a-custom-sticker-browser"></a>사용자 지정 스티커 브라우저 만들기

개발자는 메시지 앱 확장에 사용자 지정 스티커 브라우저 (`MSMessageAppBrowserViewController`)를 제공 하 여 사용자의 스티커 경험을 추가로 사용자 지정할 수 있습니다. 사용자 지정 스티커 브라우저는 메시지 스트림에 포함할 스티커를 선택할 때 사용자에 게 스티커가 표시 되는 방식을 변경 합니다.

다음을 수행합니다.

# <a name="visual-studio-for-mactabmacos"></a>[Visual Studio for Mac](#tab/macos)

1. **Solution Pad**에서 확장의 프로젝트 이름을 마우스 오른쪽 단추로 클릭 하 고 **추가** > **새 파일** ... > iOS |를 선택 합니다. **인터페이스 컨트롤러**를 > Apple Watch.
2. **이름** 에 `StickerBrowserViewController`를 입력 하 고 **새로 만들기** 단추를 클릭 합니다.
...
1. **솔루션 탐색기**에서 확장의 프로젝트 이름을 마우스 오른쪽 단추로 클릭 하 고 **추가** > **새 파일** ... > iOS |를 선택 합니다. **인터페이스 컨트롤러**를 > Apple Watch.
2. **이름** 에 `StickerBrowserViewController`를 입력 하 고 **새로 만들기** 단추를 클릭 합니다.
...
`StickerBrowserViewController.cs` 다음과 같이 표시 되도록 합니다.
...
및는 `MSStickerBrowserViewController` 클래스의 두 메서드를 재정의 하 여이 데이터 저장소의 브라우저에 대 한 데이터를 제공 합니다.
...
`CreateSticker` 메서드는 확장의 번들에서 이미지 자산의 경로를 가져온 다음이 자산을 사용 하 여 컬렉션에 추가 되는 `MSSticker`의 새 인스턴스를 만듭니다.
...
`ViewDidLoad`에서 `LoadSticker` 메서드를 호출 하 여 명명 된 이미지 자산의 스티커를 만들어 스티커의 컬렉션에 추가 합니다.
...
`ViewDidLoad` 메서드에서는 새 브라우저를 인스턴스화하고 구성 합니다.
...
두 가지 서로 다른 보기 모드 간의 전환을 처리 하는 다음 예를 살펴보겠습니다. 각 상태에는 두 개의 서로 다른 뷰 컨트롤러가 필요 합니다. `StickerBrowserViewController`은 **간결한** 뷰를 처리 하 고 다음과 같습니다.
...
`AddStickerViewController`는 **확장** 된 스티커 보기를 처리 하 고 다음과 같이 표시 됩니다.
...
`MessageViewController`는 이러한 뷰 컨트롤러를 구현 하 여 요청 된 상태를 구동 합니다.
...
사용자가 사용 가능한 컬렉션에 새 스티커를 추가 하도록 요청 하면 표시 되는 컨트롤러에 새 `AddStickerViewController` 표시 되 고 스티커 보기가 **확장** 된 보기로 전환 됩니다.
