...
- `.a` 확장명을 포함 하는 미리 컴파일된 정적 라이브러리 파일로 서 헤더 (.h 파일)와 함께 사용 됩니다. 예: [Google의 분석 라이브러리](https://developers.google.com/analytics/devguides/collection/ios/v3/sdk-download?hl=es#download_sdk)
- 미리 컴파일된 프레임 워크로. 이는 정적 라이브러리, 헤더 및 때로는 `.framework` 확장을 사용 하는 추가 리소스를 포함 하는 폴더입니다. 예를 들어 [Google의 AdMob 라이브러리](https://developers.google.com/admob/ios/download)입니다.
- 원본 코드 파일입니다. 예를 들어 `.m` 및 `.h` 목적 C 파일을 포함 하는 라이브러리입니다.
...
- **최신 버전의 목표 Sharpie** - [여기](~/cross-platform/macios/binding/objective-sharpie/get-started.md)에서 다운로드 한 목표 Sharpie 도구의 현재 복사본입니다. 이미 Sharpie를 설치한 경우에는 `sharpie update` 사용 하 여 최신 버전으로 업데이트할 수 있습니다.
...
- **Xcode 설치** -Xcode을 설치 하면 모든 명령줄 도구와 함께 제공 됩니다. OS X 10.9 shim (`/usr/bin`에 설치 됨)에서 `/usr/bin`에 포함 된 도구를 Xcode 내의 해당 도구에 매핑할 수 있습니다. 예를 들어 명령줄에서 Xcode 내의 모든 도구를 찾거나 실행할 수 있는 `xcrun` 명령을 사용할 수 있습니다.
- **터미널 응용 프로그램** -터미널 응용 프로그램에서 `xcode-select --install` 명령을 실행 하 여 명령줄 도구를 설치할 수 있습니다.
  - 터미널 응용 프로그램을 시작 합니다.
  - `xcode-select --install`를 입력 하 고 **enter**키를 누릅니다. 예를 들면 다음과 같습니다.
...
[GitHub](https://github.com/InfinitApps/InfColorPicker/archive/master.zip) 에서 InfColorPicker 프로젝트를 다운로드 하 고 선택한 디렉터리에서 압축을 풉니다. `PickerSamplePhone` 프로젝트에 대 한 Xcode 대상을 열면 Xcode 탐색기에 다음과 같은 프로젝트 구조가 표시 됩니다.
...
4. **프로젝트 이름** 에 `InfColorPicker`를 입력 하 고 **다음** 단추를 클릭 합니다.
...
11. 이제 라이브러리에 필요한 프레임 워크가 있는지 확인 해야 합니다. 이 정보는 추가 정보에서 찾거나 제공 된 샘플 프로젝트 중 하나를 열어 볼 수 있습니다. 이 예에서는 `Foundation.framework`, `UIKit.framework`및 `CoreGraphics.framework`를 사용 하 여 추가 해 보겠습니다.
...
Mac에서 터미널 응용 프로그램을 열고 메이크파일의 위치로 이동 합니다. 터미널에 `make`를 입력 하 고 **enter** 키를 누르면 **Makefile** 이 실행 됩니다.

[![](walkthrough-images/lib01.png "Sample makefile output")](walkthrough-images/lib01.png#lightbox)

만들기를 실행 하면로 많은 텍스트 스크롤이 표시 됩니다. 모든 것이 올바르게 작동 하면 **빌드 성공** 이라는 단어가 표시 되 고 `libInfColorPicker-armv7.a``libInfColorPicker-i386.a` 및 `libInfColorPickerSDK.a` 파일이 **메이크파일의**동일한 위치에 복사 됩니다.
...
1. `libInfColorPickerSDK.a`로 이동 하 고 **추가** 단추를 누릅니다.
...
파일이 프로젝트에 추가 되 면 Xamarin.ios는 자동으로 파일의 **빌드 작업** 을 **ObjcBindingNativeLibrary**로 설정 하 고 `libInfColorPickerSDK.linkwith.cs`라는 특수 파일을 **만듭니다.**

이 파일에는 Xamarin.ios에서 방금 추가한 정적 라이브러리를 처리 하는 방법을 설명 하는 `LinkWith` 특성이 포함 되어 있습니다. 이 파일의 내용은 다음 코드 조각에 나와 있습니다.
...
위의 항목에서 `iphoneos9.3` SDK가 컴퓨터에 설치 되어 있는 것을 확인할 수 있습니다. 이 정보를 사용 하 여 infcolorpicker 프로젝트 `.h` 파일을 InfColorPicker 프로젝트의 초기 **ApiDefinition.cs** 및 `StructsAndEnums.cs`으로 구문 분석할 준비가 되었습니다.
...
목표 Sharpie `Delegates`를 변환 하는 데 문제가 있는 경우 `InfColorPickerControllerDelegate` 인터페이스의 정의를 수정 하 고 `[Protocol, Model]` 줄을 다음으로 바꾸어야 합니다.
...
다음으로 `InfColorPicker.enums.cs` 파일의 내용으로 동일한 작업을 수행 하 고 `StructsAndEnums.cs` 파일에 복사 하 여 붙여넣어 `using` 문을 그대로 둡니다.
...
1. **IPhone 사용자 인터페이스 만들기** - **Infcolormainstoryboard.storyboard 샘플** 프로젝트에서 **storyboard** 파일을 두 번 클릭 하 여 iOS 디자이너에서 편집 합니다. 뷰에 **단추** 를 추가 하 고 다음과 같이 `ChangeColorButton`호출 합니다.
...
목표-C에서 프로토콜은 특정 환경에서 사용할 수 있는 메서드 (또는 메시지)를 정의 합니다. 개념적으로는의 C#인터페이스와 매우 비슷합니다. 목적과 인터페이스의 C# 주요 차이점은 프로토콜에는 클래스가 구현 하지 않아도 되는 선택적 메서드 메서드를 사용할 수 있다는 것입니다. 목적-C는 @optional 키워드를 사용 하 여 선택적 메서드를 표시 합니다. 프로토콜에 대 한 자세한 내용은 [이벤트, 프로토콜 및 대리자를](~/ios/app-fundamentals/delegates-protocols-and-events.md)참조 하세요.
...
바인딩 라이브러리가 컴파일될 때 Xamarin.ios는 가상 메서드를 사용 하 여이 인터페이스를 구현 하는 `InfColorPickerControllerDelegate`라는 추상 기본 클래스를 만듭니다.
...
**하위 클래스 InfColorPickerControllerDelegate** -`ColorSelectedDelegate`라는 프로젝트에 새 클래스를 추가 합니다. 다음 코드를 포함 하도록 클래스를 편집 합니다.
...
Xamarin.ios는 `InfColorPickerControllerDelegate`라는 추상 기본 클래스를 만들어 목표-C 대리자를 바인딩합니다. 이 형식의 서브 클래스를 만들고 `ColorPickerControllerDidFinish` 메서드를 재정의 하 여 `InfColorPickerController`의 `ResultColor` 속성 값에 액세스 합니다.
...
**ColorSelectedDelegate 변수를 초기화 합니다** . `selector` 유효한 인스턴스인지 확인 하려면 다음 코드 조각과 일치 하도록 `ViewController`의 `ViewDidLoad` 메서드를 업데이트 합니다.
...
**HandleTouchUpInsideWithStrongDelegate 메서드를 구현** 합니다. 그러면 사용자가 **colorchangebutton 단추**를 사용할 때에 대 한 이벤트 처리기를 구현 합니다. `ViewController`를 편집 하 고 다음 메서드를 추가 합니다.
...
먼저 정적 메서드를 통해 `InfColorPickerController`의 인스턴스를 가져오고 해당 인스턴스가 속성 `InfColorPickerController.Delegate`를 통해 강력한 대리자를 인식 하도록 합니다. 이 속성은 Sharpie 목표에 따라 자동으로 생성 되었습니다. 마지막으로 `PresentModallyOverViewController`를 호출 하 여 사용자가 색을 선택할 수 있도록 보기 `InfColorPickerSampleViewController.xib`를 표시 합니다.

**응용 프로그램을 실행** 합니다 .이 시점에서 모든 코드를 사용 하 여 완료 됩니다. 응용 프로그램을 실행 하는 경우 다음 스크린샷에 표시 된 것 처럼 `InfColorColorPickerSampleView`의 배경색을 변경할 수 있습니다.
...
**ViewDidLoad 업데이트** -방금 만든 이벤트 처리기를 사용 하도록 `ViewDidLoad`를 변경 해야 합니다. `ViewController`를 편집 하 고 `ViewDidLoad` 다음 코드 조각과 유사 하 게 변경 합니다.
