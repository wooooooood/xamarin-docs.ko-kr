...
ms.locfileid: "76940891"
---
# <a name="touch-events-and-gestures-in-xamarinios"></a>Xamarin.ios의 터치 이벤트 및 제스처

IOS 응용 프로그램에서 터치 이벤트 및 터치 Api를 이해 하는 것은 장치에 대 한 모든 물리적 상호 작용의 핵심 요소입니다. 모든 터치 상호 작용은 `UITouch` 개체와 관련이 있습니다. 이 문서에서는 `UITouch` 클래스와 해당 Api를 사용 하 여 터치를 지 원하는 방법에 대해 설명 합니다. 나중에이 정보를 확장 하 여 제스처를 지 원하는 방법을 알아보세요.

## <a name="enabling-touch"></a>터치 사용

`UIKit`의 컨트롤 – UIControl에서 서브클래싱된 컨트롤은 UIKit에 기본 제공 되는 제스처를 사용 하는 사용자 상호 작용에 의존 하므로 터치를 사용 하도록 설정할 필요가 없습니다. 이미 사용 하도록 설정 되어 있습니다.

그러나 `UIKit`의 뷰 대부분은 기본적으로 터치를 사용 하지 않습니다. 컨트롤에서 터치를 사용 하도록 설정 하는 방법에는 두 가지가 있습니다. 첫 번째 방법은 다음 스크린샷에 표시 된 것 처럼 iOS 디자이너의 속성 패드에서 사용자 상호 작용 사용 확인란을 선택 하는 것입니다.
...
또한 `UIView` 클래스에서 컨트롤러를 사용 하 여 `UserInteractionEnabled` 속성을 true로 설정할 수 있습니다. 이는 UI가 코드에서 생성 되는 경우에 필요 합니다.
...
사용자가 화면에 접촉 하거나 손가락을 이동 하거나 손가락을 제거 하는 경우 발생 하는 세 가지 터치 단계가 있습니다. 이러한 메서드는 UIView의 기본 클래스인 `UIResponder`에서 정의 됩니다. iOS는 `UIView` 및 `UIViewController`에 연결 된 메서드를 재정의 하 여 터치를 처리 합니다.

- `TouchesBegan` – 화면이 처음 작업 될 때 호출 됩니다.
- `TouchesMoved` – 사용자가 화면 주위에서 손가락을 이동할 때 터치 위치가 변경 될 때 호출 됩니다.
- `TouchesEnded` 또는 `TouchesCancelled` – `TouchesEnded` 사용자의 손가락이 화면에서 리프트 될 때 호출 됩니다.  iOS가 터치를 취소 하는 경우 호출 됩니다. 예를 들어 사용자가 단추에서 손가락을 벗어나 누름을 취소 하는 경우를 들 수 있습니다. `TouchesCancelled`

터치 이벤트는 UIViews의 스택에서 재귀적으로 이동 하 여 터치 이벤트가 뷰 개체의 범위 내에 있는지 확인 합니다. 이를 종종 _적중 테스트_라고 합니다. 처음에는 맨 위에 있는 `UIView` 또는 `UIViewController`에서 호출 되며, `UIView`에서 호출 되 고 뷰 계층 구조에서 아래 `UIViewControllers` 됩니다.

사용자가 화면에 닿을 때마다 `UITouch` 개체가 생성 됩니다. `UITouch` 개체에는 터치가 발생 한 경우, 터치가 살짝 밀기 인 경우 등 터치에 대 한 데이터가 포함 됩니다. 터치 이벤트는 하나 이상의 터치를 포함 하는 `NSSet` 터치 속성을 전달 받습니다. 이 속성을 사용 하 여 터치에 대 한 참조를 가져오고 응용 프로그램의 응답을 확인할 수 있습니다.

터치 이벤트 중 하나를 재정의 하는 클래스는 먼저 기본 구현을 호출한 다음 이벤트와 연결 된 `UITouch` 개체를 가져와야 합니다. 첫 번째 터치에 대 한 참조를 가져오려면 `AnyObject` 속성을 호출 하 고 다음 예제와 같이 `UITouch`로 캐스팅 합니다.
...
다음 코드 줄에 표시 된 대로 `MultipleTouchEnabled` 속성을 설정 하 여 프로그래밍 방식으로 다중 터치를 설정할 수도 있습니다.

```csharp
imgTouchMe.MultipleTouchEnabled = true;
```

화면에 대 한 손가락의 수를 확인 하려면 `UITouch` 속성에서 `Count` 속성을 사용 합니다.
...
`UITouch.LocationInView` 메서드는 지정 된 뷰 내에서 터치의 좌표를 포함 하는 CGPoint 개체를 반환 합니다. 또한 `Frame.Contains`메서드를 호출 하 여 해당 위치가 컨트롤 내에 있는지 여부를 테스트할 수 있습니다. 다음 코드 조각에서는이에 대 한 예를 보여 줍니다.
...
Xamarin.ios는 다음과 같은 기본 제공 제스처 인식기에 대 한 기본 클래스로 `UIGestureRecognizer` 클래스를 제공 합니다.
...
1. **제스처 설정 구성** – 다음 단계는 제스처 인식기를 구성 하는 것입니다. `UIGestureRecognizer` 인스턴스의 동작을 제어 하기 위해 설정할 수 있는 속성 목록은 `UIGestureRecognizer` 및 해당 서브 클래스에 대 한 Xamarin 설명서를 참조 하세요.
1. 목표 **구성** -C heritage 때문에 xamarin.ios는 제스처 인식기가 제스처와 일치할 때 이벤트를 발생 시 키 지 않습니다.  `UIGestureRecognizer`에는 제스처 인식기가 일치 하는 경우 실행할 코드를 사용 하 여 익명 대리자 또는 목표-C 선택기를 받아들일 수 있는 메서드 (`AddTarget`)가 있습니다.
1. **제스처 인식기 사용** – 터치 이벤트와 마찬가지로 터치 상호 작용을 사용 하는 경우에만 제스처가 인식 됩니다.
1. **뷰에 제스처 인식기 추가** – 최종 단계는 `View.AddGestureRecognizer`를 호출 하 고 제스처 인식기 개체를 전달 하 여 뷰에 제스처를 추가 하는 것입니다.
...
보기에 제스처 인식기를 추가 하 고 나면 뷰 (및 그 아래 뷰)가 터치 이벤트를 수신 하지 않는다는 점을 기억해 야 합니다. 터치 이벤트를 제스처와 동시에 허용 하려면 다음 코드에 나와 있는 것 처럼 `CancelsTouchesInView` 속성을 false로 설정 해야 합니다.
...
Xamarin.ios는 `UIGestureRecognizerState` 열거형에서 이러한 값을 나타냅니다.
...
다음 코드 조각에 나와 있는 것 처럼 강력한 형식의 `UIGestureRecognizerDelegate`, 약한 대리자 또는 이벤트 처리기 구문을 통해 바인딩할 수 있습니다.
