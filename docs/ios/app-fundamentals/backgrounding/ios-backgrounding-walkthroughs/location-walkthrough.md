...
1. 이제 응용 프로그램이 등록 되었으므로 장치에서 위치 데이터를 가져올 수 있습니다. IOS에서 `CLLocationManager` 클래스는 위치 정보에 액세스 하는 데 사용 되며 위치 업데이트를 제공 하는 이벤트를 발생 시킬 수 있습니다.

1. 코드에서 위치 업데이트를 구독 하는 다양 한 화면 및 코드를 위한 단일 위치를 제공 하는 `LocationManager` 이라는 새 클래스를 만듭니다. `LocationManager` 클래스에서 `LocMgr`라는 `CLLocationManager`의 인스턴스를 만듭니다.
    ...
    - `PausesLocationUpdatesAutomatically` – 시스템이 위치 업데이트를 일시 중지할 수 있는지 여부에 따라 설정할 수 있는 부울입니다. 일부 장치에서는 기본적으로 `true`로 설정 되며,이로 인해 장치에서 15 분 후에 백그라운드 위치 업데이트 가져오기를 중지할 수 있습니다.
    - `RequestAlwaysAuthorization`-백그라운드에서 위치에 액세스할 수 있도록 허용 하는 옵션을 앱 사용자에 게 제공 하려면이 메서드를 전달 해야 합니다. 앱이 전경에 있는 경우에만 위치에 액세스할 수 있는 옵션을 사용자에 게 제공 하려는 경우에도 `RequestWhenInUseAuthorization`을 전달할 수 있습니다.
...
1. `LocationManager` 클래스 내에서 다음 코드를 사용 하 여 `StartLocationUpdates` 라는 메서드를 만듭니다. 이 코드는 `CLLocationManager`에서 위치 업데이트 수신을 시작 하는 방법을 보여 줍니다.
    ...
    이 방법에서는 몇 가지 중요 한 사항이 있습니다. 첫째, 응용 프로그램이 장치의 위치 데이터에 액세스할 수 있는지 확인 하는 검사를 수행 합니다. `CLLocationManager`에서 `LocationServicesEnabled`를 호출 하 여이를 확인 합니다. 사용자가 위치 정보에 대 한 응용 프로그램 액세스를 거부 한 경우이 메서드는 **false** 를 반환 합니다.

1. 그런 다음 위치 관리자에 업데이트 빈도를 알려 줍니다. `CLLocationManager`는 업데이트 빈도를 비롯 하 여 위치 데이터를 필터링 및 구성 하기 위한 여러 옵션을 제공 합니다. 이 예제에서는 위치가 측정기에 의해 변경 될 때마다 `DesiredAccuracy`를 업데이트 하도록 설정 합니다. 위치 업데이트 빈도 및 기타 기본 설정 구성에 대 한 자세한 내용은 Apple 설명서의 [Cllocationmanager 클래스 참조](https://developer.apple.com/library/ios/#documentation/CoreLocation/Reference/CLLocationManager_Class/CLLocationManager/CLLocationManager.html) 를 참조 하세요.

1. 마지막으로 `CLLocationManager` 인스턴스에서 `StartUpdatingLocation`를 호출 합니다. 그러면 위치 관리자가 현재 위치에 대 한 초기 수정을 받고 업데이트 보내기를 시작 하도록 지시 합니다.

지금까지 수신 하려는 데이터 종류를 사용 하 여 위치 관리자를 만들고 초기 위치를 결정 했습니다. 이제 코드에서 위치 데이터를 사용자 인터페이스로 렌더링 해야 합니다. `CLLocation`를 인수로 사용 하는 사용자 지정 이벤트를 사용 하 여이 작업을 수행할 수 있습니다.
...
다음 단계는 `CLLocationManager`에서 위치 업데이트를 구독 하 고 새 위치 데이터를 사용할 수 있게 되 면 사용자 지정 `LocationUpdated` 이벤트를 발생 시켜 위치를 인수로 전달 합니다. 이렇게 하려면 **LocationUpdateEventArgs.cs**새 클래스를 만듭니다. 이 코드는 주 응용 프로그램 내에서 액세스할 수 있으며 이벤트가 발생할 때 장치 위치를 반환 합니다.
...
1. Solution Pad에서 `ViewController.cs` 파일을 두 번 클릭 하 고 편집 하 여 LocationManager의 새 인스턴스를 만들고이에 대 한 `StartLocationUpdates`를 호출 합니다.
  다음과 같이 코드를 변경 합니다.
...
여전히 AppDelegate에서 `LocationUpdated` 이벤트를 구독 하 고 새 메서드를 호출 하 여 UI를 업데이트 해야 합니다. `ViewDidLoad,`에서 `StartLocationUpdates`를 호출한 직후에 다음 코드를 추가 합니다.
...
1. 응용 프로그램이 포그라운드에서 활성 상태인 동안 위치 업데이트를 출력 합니다. 앱이 백그라운드에 들어갈 때 발생 하는 상황을 보여 주기 위해 응용 프로그램 상태 변경 내용을 추적 하는 `AppDelegate` 메서드를 재정의 하 여 응용 프로그램이 포그라운드 및 백그라운드 간에 전환 될 때 콘솔에 쓰도록 합니다.
    ...
    업데이트 된 위치 데이터를 응용 프로그램 출력으로 지속적으로 인쇄 하는 `LocationManager`에 다음 코드를 추가 하 여 백그라운드에서 위치 정보를 계속 사용할 수 있는지 확인 합니다.
