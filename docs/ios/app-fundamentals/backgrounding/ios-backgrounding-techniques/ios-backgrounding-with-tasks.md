...
등록 프로세스에서는 작업을 고유 식별자 `taskID`로 쌍으로 지정한 다음 일치 `BeginBackgroundTask` 하는 및 `EndBackgroundTask` 호출로 래핑합니다. 식별자를 생성 하려면 `BeginBackgroundTask` `UIApplication` 개체에 대해 메서드를 호출한 다음 일반적으로 새 스레드에서 장기 실행 작업을 시작 합니다. 작업이 완료 되 면를 호출 `EndBackgroundTask` 하 고 동일한 식별자를 전달 합니다. 이는 `BeginBackgroundTask` 호출에 일치 `EndBackgroundTask`하는가 없는 경우 iOS에서 응용 프로그램을 종료 하기 때문에 중요 합니다.
...
이전 예제에서와 같이 `DidEnterBackground` 를 통해 `BeginBackgroundTask` 작업 `AppDelegate`을 등록 하는에서 메서드를 재정의 하는 것부터 시작 합니다. 다음으로 새 스레드를 생성 하 고 장기 실행 작업을 수행 합니다. `DidEnterBackground` 메서드가 이미를 반환 했으므로 장기 실행 작업 내에서 호출이수행됩니다.`EndBackgroundTask`
...
등록 된 작업이 있는 응용 프로그램을 백그라운드로 이동 하면 등록 된 작업을 실행 하는 데 600 초 정도 걸립니다. 클래스의 정적 `BackgroundTimeRemaining` 속성을 사용 하 여 작업이 완료 되어야 하는 시간을 확인할 수 있습니다. `UIApplication` 다음 코드는 백그라운드 작업에서 남은 시간 (초)을 제공 합니다.
...
IOS는 `BackgroundTimeRemaining` 속성에 대 한 액세스를 제공 하는 것 외에도 **만료 처리기**를 통해 백그라운드 시간 만료를 처리 하는 정상적인 방법을 제공 합니다. 작업에 할당 된 시간이 만료 될 때 실행 되는 선택적 코드 블록입니다. 만료 처리기의 코드는 작업 `EndBackgroundTask` ID를 호출 하 고이를 전달 합니다 .이는 앱이 제대로 작동 하 고 작업에 시간이 오래 걸리는 경우에도 iOS에서 앱을 종료 하지 못하도록 합니다. `EndBackgroundTask`만료 처리기 내에서 및 일반적인 실행 과정에서 호출 해야 합니다. 
...
IOS 7 이전에는 백그라운드에서 실행 되는 작업을 완료 하는 데 600 초가 있었습니다. 이 제한의 한 가지 이유는 백그라운드에서 실행 중인 작업이 작업 기간 동안 장치를 절전 모드에서 해제 하는 것입니다.
 ...
 [![](ios-backgrounding-with-tasks-images/ios6.png "앱의 활성 상태를 유지 하는 작업의 그래프 사전 iOS 7")](ios-backgrounding-with-tasks-images/ios6.png#lightbox)
...
iOS 7 백그라운드 처리는 배터리 수명이 긴 경우에 최적화 되어 있습니다. IOS 7에서 backgrounding는 장치를 활성 상태로 유지 하는 대신 장치를 절전 모드에서 해제 하는 것이 아니라, 장치가 전화 통화, 알림, 수신 메일 등을 처리 하기 위해 절전 모드를 해제 하는 경우에는 청크를 대신 처리 합니다. 일반적인 중단. 다음 다이어그램은 태스크가 분할 되는 방법에 대 한 통찰력을 제공 합니다.
 ...
 [![](ios-backgrounding-with-tasks-images/ios7.png "청크로 분할 된 작업의 그래프-iOS 7")](ios-backgrounding-with-tasks-images/ios7.png#lightbox)
...
API `NSURLSession` 는 하나 이상의 세션을 만들고, 네트워크를 통해 관련 데이터의 블록을 셔틀 하는 작업을 생성 합니다. 작업은 비동기적으로 실행 되어 데이터를 빠르고 안정적으로 전송 합니다. 는 `NSURLSession` 비동기 이기 때문에 모든 세션에는 전송이 완료 될 때 시스템 및 응용 프로그램에서 알 수 있도록 하는 완료 처리기 블록이 필요 합니다.

사전 ios 7 및 사후 ios 7 모두에서 유효한 네트워크 전송을 수행 하려면를 큐에서 전송에 사용할 수 있는지 확인 `NSURLSession` 하 고 일반 백그라운드 작업을 사용 하 여 다음이 아닌 경우 전송을 수행 합니다.
...
IOS 7 이전에는 백그라운드에서 파일을 업로드 하거나 다운로드 하는 것은 신뢰할 수 없습니다. 백그라운드 작업을 실행 하는 데 시간이 제한 되지만 파일을 전송 하는 데 걸리는 시간은 네트워크와 파일 크기에 따라 달라 집니다. IOS 7에서는를 사용 `NSURLSession` 하 여 파일을 성공적으로 업로드 및 다운로드할 수 있습니다. 백그라운드에서 `NSURLSession` 많은 파일의 네트워크 전송을 처리 하는 특정 세션 유형을 *백그라운드 전송 서비스*라고 합니다.
