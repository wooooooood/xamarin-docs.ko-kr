...
> [!IMPORTANT]
> 참고: Entitlements.plist 파일은 기능을 사용하여 응용 프로그램의 올바른 속성을 입력하는 데 도움이 되지만 Apple 개발자 계정에 연결되어 있지 않기 때문에 프로비전 프로필을 생성할 수 없습니다. 응용 프로그램을 배포하려면 개발자 포털을 사용하여 프로비전 프로필을 생성해야 합니다.
