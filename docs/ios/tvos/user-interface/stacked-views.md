...
ms.locfileid: "73022151"
---
# <a name="working-with-tvos-stacked-views-in-xamarin"></a>Xamarin에서 tvOS 누적 보기 사용

`UIStackView`(Stack View control)는 자동 레이아웃 및 크기 클래스의 강력한 기능을 활용 하 여 Apple TV 장치의 콘텐츠 변경 내용 및 화면 크기에 동적으로 응답 하는 가로 또는 세로로 하위 뷰 스택을 관리 합니다.
...
TvOS 앱에서 `UIStackView`를 사용 하는 경우 개발자는 iOS 디자이너의 스토리 보드 내에서 또는 코드에서 C# 하위 뷰를 추가 하 고 제거 하 여 하위 뷰를 정의할 수 있습니다.
...
1. 스택 뷰를 선택 하 **Properties Pad** 의 **위젯 탭** 에서 **맞춤**에 대해 **채우기** 를 선택 하 고, **균등** 하 게 **분포** 를 입력 하 고, **간격**에 대해 `25`을 입력 합니다.
...
1. 스택 뷰를 선택 하 고, **속성 탐색기** 의 **위젯 탭** 에서 맞춤 **을 선택** 하고, **균등** 하 게 **분포** 를 입력 하 고, **간격**에 `25`을 입력 합니다.
...
> [!IMPORTANT]
> 이벤트 처리기를 만들 때 iOS 디자이너의 UI 요소 (예: `UIButton`)에 `TouchUpInside`와 같은 작업을 할당할 수는 있지만, Apple TV에 터치 스크린이 없거나 터치 이벤트가 지원 되기 때문에 호출 되지 않습니다. TvOS 사용자 인터페이스 요소에 대 한 작업을 만들 때 항상 기본 `Action Type`를 사용 해야 합니다.

스토리 보드 사용에 대 한 자세한 내용은 [Hello, tvOS 빠른 시작 가이드](~/ios/tvos/get-started/hello-tvos.md)를 참조 하세요.

이 예제의 경우 세그먼트 컨트롤에 대 한 유출 및 작업과 각 "플레이어 카드"에 대 한 콘센트가 제공 됩니다. 코드에서 현재 세그먼트를 기준으로 플레이어를 숨기고 표시 합니다. 예를 들면,
