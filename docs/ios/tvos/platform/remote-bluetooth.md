...
|**Siri/Search**<br />Siri를 사용 하는 국가에서 음성 제어를 길게 누르고 다른 모든 국가에서는 검색 화면을 표시 합니다.|n/a|n/a|
...
|**Home**<br />키를 눌러 홈 화면으로 돌아간 다음, 두 번 클릭 하 여 실행 중인 앱을 표시 하 고,를 두 번 클릭 하 여 장치를 대기 합니다.|n/a|n/a|
|**볼륨**<br />연결 된 오디오/비디오 장비 볼륨을 제어 합니다.|n/a|n/a|
...
다음 코드에서는 `UIPress` `UIView`에 대 한 하위 수준 이벤트를 처리 하는 예제를 보여 줍니다.
...
이벤트와 `UITouch` 마찬가지로 이벤트 재정의를 구현 해야 하는 `UIPress` 경우 4 가지를 모두 구현 해야 합니다.
...
|기능|일반 앱 사용|게임 앱 사용|
|---|---|---|
|**D-Pad**|UI 요소 (변경 내용 포커스)를 탐색 합니다.|게임에 따라 다릅니다.|
...
- Siri 원격을 대상으로`GCMicroGamepad`하는 마이크로 게임 컨트롤러 프로필 ()이 추가 되었습니다.
...
TvOS 앱에서 게임 컨트롤러 지원을 사용 하도록 설정 하려면 `Info.plist` **솔루션 탐색기** 파일을 두 번 클릭 하 여 편집을 위해 엽니다.
...
Apple TV와 함께 제공 되는 Siri 원격은 제한 된 게임 컨트롤러로 사용할 수 있습니다. 다른 게임 컨트롤러와 마찬가지로 게임 컨트롤러 프레임 워크 `GCController` 에서 개체로 표시 되며 `GCMotion` 및 `GCMicroGamepad` 프로필을 모두 지원 합니다.
...
TvOS에서 게임 컨트롤러 입력을 직접 처리 하려면 `GCEventViewController` (또는 하위 클래스)를 사용 하 여 게임의 사용자 인터페이스를 표시 해야 합니다. 가 첫 번째 응답자 일 때마다 게임 컨트롤러 입력이 캡처되고 게임 컨트롤러 프레임 워크를 통해 앱에 전달 됩니다. `GCEventViewController`

클래스의 속성`UserInteractionEnabled` 을 사용 하 여 이벤트를 처리 하 고 처리 하는 방법을 전환할 수 있습니다. `GCEventViewController`
