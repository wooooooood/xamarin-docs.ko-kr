...
[Azure Functions 앱을 만드는 ![Screenshot](azure-signalr-images/azure-functions-app-cropped.png)](azure-signalr-images/azure-functions-app-full.png#lightbox)
...
@No__t_0 함수는 `AccessToken` 속성 및 `Url` 속성을 포함 하는 `SignalRConnectionInfo` 개체를 사용 하 여 웹 요청에 응답 합니다. 모바일 응용 프로그램은 이러한 값을 사용 하 여 SignalR hub에 자신을 등록 합니다. 다음 코드는 `Negotiate` 함수를 보여 줍니다.
...
@No__t_0 함수는 POST 본문에 메시지 개체를 제공 하는 HTTP POST 요청에 응답 합니다. POST 본문은 `SignalRMessage` 변환 되어 SignalR hub로 전달 됩니다. 다음 코드는 `Talk` 함수를 보여 줍니다.
...
샘플 응용 프로그램에는 Azure Functions 앱의 URL 끝점으로 사용자 지정 해야 하는 **Constants.cs** 클래스가 포함 되어 있습니다. @No__t_0 속성의 값을 Azure Functions 앱 주소로 설정 합니다. 다음 코드는 `HostName` 값 예제를 사용 하 여 **Constants.cs** 속성을 보여 줍니다.
...
@No__t_1 클래스의 `SendMessageAsync` 메서드는 Azure SignalR Service에 연결 된 클라이언트에 메시지를 보내는 데 사용 됩니다. 이 메서드는 JSON 직렬화 된 `Message` 개체를 게시 페이로드로 포함 하 여 Azure Functions 앱에서 호스트 되는 **통신** 함수에 HTTP POST 요청을 수행 합니다. **말하기** 함수는 연결 된 모든 클라이언트에 브로드캐스트하는 메시지를 Azure SignalR 서비스에 전달 합니다. 다음 코드에서는 `SendMessageAsync` 메서드를 보여 줍니다.
...
@No__t_1 클래스의 `ConnectAsync` 메서드는 Azure Functions 앱에서 호스팅된 **Negotiate** 함수에 대해 HTTP GET 요청을 수행 합니다. **Negotiate** 함수는 `NegotiateInfo` 클래스의 인스턴스로 deserialize 되는 JSON을 반환 합니다. @No__t_0 개체가 검색 되 면 `HubConnection` 클래스의 인스턴스를 사용 하 여 Azure SignalR 서비스에 직접 등록 하는 데 사용 됩니다.

ASP.NET Core SignalR는 들어오는 데이터를 열린 연결에서 메시지로 변환 하 고, 개발자가 메시지 유형을 정의 하 고 들어오는 메시지에 유형으로 이벤트 처리기를 바인딩할 수 있도록 합니다. @No__t_0 메서드는 샘플 응용 프로그램의 **Constants.cs** 파일에 정의 된 메시지 이름에 대 한 이벤트 처리기를 등록 합니다 .이 파일은 기본적으로 "newmessage"입니다.
...
@No__t_0 메서드는 이전 코드와 같이 `ConnectAsync` 메시지의 이벤트 처리기로 바인딩됩니다. 메시지가 수신 되 면 `JObject`으로 제공 된 메시지 데이터를 사용 하 여 `AddNewMessage` 메서드가 호출 됩니다. @No__t_0 메서드는 `JObject`를 `Message` 클래스의 인스턴스로 변환 하 고, 바인딩된 경우 `NewMessageReceived`에 대 한 처리기를 호출 합니다. 다음 코드에서는 `AddNewMessage` 메서드를 보여 줍니다.
...
@No__t_1 클래스의 `NewMessageReceived` 이벤트는 Azure SignalR 서비스에서 새 메시지를 받을 때 발생 합니다. @No__t_0 이벤트 처리기 메서드는 `MainPage` 생성자의 `NewMessageReceived` 이벤트에 바인딩됩니다. 이 이벤트 처리기는 들어오는 `Message` 개체를 문자열로 변환 하 고 `AddMessage` 메서드를 사용 하 여 채팅 큐에 추가 합니다. 다음 코드에서는 `SignalR_NewMessageReceived` 이벤트 처리기 메서드를 보여 줍니다.
...
@No__t_0 메서드는 채팅 큐에 `Label` 개체로 새 메시지를 추가 합니다. @No__t_0 메서드는 주로 주 UI 스레드 외부에서 이벤트 처리기에 의해 호출 되므로 예외를 방지 하기 위해 주 스레드에서 UI 업데이트를 강제로 실행 합니다. 다음 코드에서는 `AddMessage` 메서드를 보여 줍니다.
