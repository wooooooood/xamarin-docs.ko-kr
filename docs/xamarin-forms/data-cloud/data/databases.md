...
`TodoItemDatabase` .NET `Lazy` 클래스를 사용 하 여 처음 액세스할 때까지 데이터베이스 초기화를 지연 합니다. 초기화 지연을 사용 하면 데이터베이스 로드 프로세스가 앱 시작을 지연 하지 않습니다. 자세한 내용은 [Lazy&lt;t&gt; 클래스](https://docs.microsoft.com/dotnet/api/system.lazy-1)를 참조 하세요.
...
- [Lazy<T> 클래스](https://docs.microsoft.com//api/system.lazy-1)