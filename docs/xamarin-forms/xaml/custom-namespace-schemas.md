...
사용자 지정 네임 스페이스 스키마 형식을 찾기 위해 Xamarin.ios는 참조 된 어셈블리의 `XmlnsDefinitionAttribute` 인스턴스를 검색 합니다. XAML 파일 `xmlns` 의 요소에 대 한 특성이의 `XmlNamespace` `XmlnsDefinitionAttribute`속성 값과 일치 하면 xamarin.ios는 `XmlnsDefinitionAttribute.ClrNamespace` 속성 값을 사용 하 여 형식을 확인 하려고 시도 합니다. 형식 확인에 실패 하는 경우 Xamarin.ios는 추가 일치 `XmlnsDefinitionAttribute` 인스턴스에 따라 형식 확인을 계속 시도 합니다.
