...
[`CollectionView`](xref:Xamarin.Forms.CollectionView) 은 스크롤이 발생 했음을 나타내기 위해 발생 하는 `Scrolled` 이벤트를 정의 합니다. 다음 XAML 예제에서는 `Scrolled` 이벤트에 대 한 이벤트 처리기를 설정 하는 `CollectionView` 보여 줍니다.
