...
지정 하 여 Xamarin.Forms에 SkiaSharp 그래픽 애니메이션을 적용할 수는 `PaintSurface` 그래픽 약간 다르게 그리기 사용자가 자주 호출 될 메서드입니다. 센터에서 확장 하 고 겉으로 동심원 함께이 문서에 나오는 애니메이션 다음과 같습니다.

![](animation-images/animationexample.png "여러 동심원 겉보기 센터에서 확장")

**Pulsating 타원** 페이지에 [ **SkiaSharpFormsDemos** ](https://developer.xamarin.com/samples/xamarin-forms/SkiaSharpForms/SkiaSharpFormsDemos/) 프로그램 애니메이션 효과 적용 되는 타원의 두 가지 축을 pulsating 수를 표시 되도록 하 고 제어할 수 있습니다는 이 pulsation의 속도:
...
코드 숨김 파일에서 인스턴스화하는 `Stopwatch` 고정밀 클록으로 사용할 개체입니다. `OnAppearing` 설정 재정의 `pageIsActive` 필드를 `true` 라는 메서드를 호출 하 고 `AnimationLoop`합니다. `OnDisappearing` 재정의 설정 하는 `pageIsActive` 필드를 `false`:
...
`AnimationLoop` 메서드가 시작 된 `Stopwatch` , 하는 동안 다음 루프 `pageIsActive` 은 `true`합니다. 이 기본적으로 "무한 루프" 페이지에 활성 상태 이지만 프로그램을 호출 하 여 루프 종료 때문에 중단을 발생 하지 않는 동안 `Task.Delay` 와 `await` 프로그램 함수의 다른 부분을 수 있는 연산자입니다. 에 대 한 인수 `Task.Delay` 받으므로 1/30 초 후 완료 합니다. 이 애니메이션의 프레임 속도 정의합니다.
...
에 `SKPaint` 에서 개체가 생성 되는 `using` 블록입니다. 많은 SkiaSharp 클래스와 같은 `SKPaint` 에서 파생 `SKObject`에서 파생 되는 `SKNativeObject`를 구현 하는 [ `IDisposable` ](https://developer.xamarin.com/api/type/System.IDisposable/) 인터페이스입니다. `SKPaint` 재정의 `Dispose` 메서드가 관리 되지 않는 리소스를 해제 합니다.
 ...
 배치 `SKPaint` 에 `using` 되도록 블록 `Dispose` 이러한 관리 되지 않는 리소스를 해제 하는 블록의 끝에서 호출 합니다. 메모리에서 사용 하는 경우 그래도 이러한 문제가 발생 된 `SKPaint` 개체가 해제 되.NET 가비지 수집기에 의해 하지만 애니메이션 코드에서 것이 다소 더 순차적 방식으로 메모리를 해제 사전 조치를 위해 가장 좋습니다.
...
이 프로그램은 다른 애니메이션의 Xamarin.Forms 기반 접근 방식을 사용 하 여 `Device.StartTimer`합니다. `t` 필드는 0에서 1을 애니메이션 효과가 적용 되어 모든 `cycleTime` 시간 (밀리초):
...
`PaintSurface` 처리기 동심원 5 애니메이션된 반지름 있는 합니다. 경우는 `baseRadius` 으로 다음 100으로 변수는 계산 `t` 0에서 1, 100, 100-200, 200에서 300, 300-400 및 500에 400을 0에서 5 개의 원 증가의 반지름을 애니메이션 효과가 적용 됩니다. 원 중 대부분에 대 한는 `strokeWidth` 50 하지만 첫 번째 원형는 `strokeWidth` 0에서 50 애니메이션을 적용 합니다. 원 중 대부분의 경우 색은 파란색 하지만 마지막 원에 대 한 색 애니메이션 효과가 적용 되어 파랑에서을 투명 하 게 합니다.
