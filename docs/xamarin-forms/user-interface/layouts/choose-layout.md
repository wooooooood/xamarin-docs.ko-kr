...
[`StackLayout`](xref:Xamarin.Forms.StackLayout) 1 차원 스택의 요소를 가로 또는 세로로 구성 합니다. [`Orientation`](xref:Xamarin.Forms.StackLayout.Orientation) 속성은 요소의 방향을 지정 하 고 기본 방향은 [`Vertical`](xref:Xamarin.Forms.StackOrientation)입니다. `StackLayout`은 일반적으로 페이지에서 UI의 하위 섹션을 정렬 하는 데 사용 됩니다.
...
[`StackLayout`](xref:Xamarin.Forms.StackLayout)에서 요소의 크기를 명시적으로 설정 하지 않으면 확장 하 여 사용 가능한 너비 또는 높이를 채웁니다 ( [`Orientation`](xref:Xamarin.Forms.StackLayout.Orientation) 속성이 [`Horizontal`](xref:Xamarin.Forms.StackOrientation)으로 설정 된 경우).

[`StackLayout`](xref:Xamarin.Forms.StackLayout) 는 다른 하위 레이아웃을 포함 하는 부모 레이아웃으로 사용 되는 경우가 많습니다. 그러나 `StackLayout` 개체의 조합을 사용 하 여 [`Grid`](xref:Xamarin.Forms.Grid) 레이아웃을 재현 하는 데 `StackLayout`를 사용 하면 안 됩니다. 다음 코드에서는이 잘못 된 방법의 예를 보여 줍니다.
...
> [!TIP]
> [`StackLayout`](xref:Xamarin.Forms.StackLayout)사용 하는 경우 하나의 자식 요소만 [`LayoutOptions.Expands`](xref:Xamarin.Forms.LayoutOptions.Expands)로 설정 되었는지 확인 합니다. 이 속성은 지정된 자식 요소가 `StackLayout`에서 허용하는 최대의 공간을 차지하도록 하며, 이는 이러한 계산이 두 번 이상 수행되도록 하여 불필요합니다.

자세한 내용은 [Xamarin.ios StackLayout](stack-layout.md)을 참조 하세요.

## <a name="grid"></a>표 형태

[`Grid`](xref:Xamarin.Forms.Grid) 는 비례 또는 절대 크기를 가질 수 있는 행 및 열에 요소를 표시 하는 데 사용 됩니다. 표의 행과 열은 [`RowDefinitions`](xref:Xamarin.Forms.Grid.RowDefinitions) 및 [`ColumnDefinitions`](xref:Xamarin.Forms.Grid.ColumnDefinitions) 속성으로 지정 됩니다.
...
[`FlexLayout`](xref:Xamarin.Forms.FlexLayout) 은 자식 요소를 스택에서 가로 또는 세로로 표시 한다는 점에서 [`StackLayout`](xref:Xamarin.Forms.StackLayout) 와 비슷합니다. 그러나 `FlexLayout`는 단일 행 이나 열에 맞지 않는 경우 자식을 래핑하고 자식 요소의 크기, 방향 및 맞춤을 더욱 세밀 하 게 제어할 수 있습니다.
...
- [`Direction`](xref:Xamarin.Forms.FlexLayout.Direction) 속성은 `Column`로 설정 되며,이로 인해 `FlexLayout`의 자식은 항목의 단일 열에 정렬 됩니다.
- [`AlignItems`](xref:Xamarin.Forms.FlexLayout.AlignItems) 속성이 `Center`로 설정 되어 있으므로 각 항목은 가로로 가운데 맞춤 됩니다.
- [`JustifyContent`](xref:Xamarin.Forms.FlexLayout.JustifyContent) 속성은 `SpaceEvenly`로 설정 됩니다. 그러면 모든 항목 사이, 첫 번째 항목 위 및 마지막 항목 아래에 있는 모든 남아 있는 세로 공간이 동일 하 게 할당 됩니다.

자세한 내용은 [Xamarin.ios 레이아웃](flex-layout.md)을 참조 하세요.

## <a name="relativelayout"></a>RelativeLayout

[`RelativeLayout`](xref:Xamarin.Forms.RelativeLayout) 는 레이아웃 또는 형제 요소의 속성을 기준으로 요소를 배치 하 고 크기를 조정 하는 데 사용 됩니다. 기본적으로 요소는 레이아웃의 왼쪽 위 모퉁이에 배치 됩니다. `RelativeLayout`를 사용 하 여 장치 크기에 비례하여 크기를 조정 하는 Ui를 만들 수 있습니다.

[`RelativeLayout`](xref:Xamarin.Forms.RelativeLayout)내에서 위치와 크기는 제약 조건으로 지정 됩니다. 제약 조건에는 위치 및 크기를 다른 개체의 여러 속성 (또는 조각)으로 정의 하는 데 사용할 수 있는 [`Factor`](xref:Xamarin.Forms.ConstraintExpression.Factor) 및 [`Constant`](xref:Xamarin.Forms.ConstraintExpression.Constant) 속성이 있으며 상수도 있습니다. 또한 상수는 음수일 수 있습니다.

> [!NOTE]
> [`RelativeLayout`](xref:Xamarin.Forms.RelativeLayout) 는 자체 범위 밖에 서 위치 지정 요소를 지원 합니다.
...
[`AbsoluteLayout`](xref:Xamarin.Forms.AbsoluteLayout) 는 명시적 값 또는 레이아웃 크기에 상대적인 값을 사용 하 여 요소를 배치 하 고 크기를 조정 하는 데 사용 됩니다. 위치는 `AbsoluteLayout`의 왼쪽 위 모퉁이를 기준으로 자식의 왼쪽 위 모퉁이에 의해 지정 됩니다.

[`AbsoluteLayout`](xref:Xamarin.Forms.AbsoluteLayout) 은 자식에 크기를 적용할 수 있거나 요소의 크기가 다른 자식의 위치 지정에 영향을 주지 않는 경우에만 사용 되는 특수 한 용도의 레이아웃으로 간주 되어야 합니다. 이 레이아웃의 표준 사용은 다른 컨트롤과 페이지를 포함 하는 오버레이를 만들어 사용자가 페이지의 일반 컨트롤과 상호 작용 하지 못하도록 하는 것입니다.

> [!IMPORTANT]
> `HorizontalOptions` 및 `VerticalOptions` 속성은 `AbsoluteLayout`의 자식에 영향을 주지 않습니다.

[`AbsoluteLayout`](xref:Xamarin.Forms.AbsoluteLayout)내에서 [`AbsoluteLayout.LayoutBounds`](xref:Xamarin.Forms.AbsoluteLayout.LayoutBoundsProperty) 연결 된 속성은 요소의 가로 위치, 세로 위치, 너비 및 높이를 지정 하는 데 사용 됩니다. 또한 연결 된 [`AbsoluteLayout.LayoutFlags`](xref:Xamarin.Forms.AbsoluteLayout.LayoutFlagsProperty) 속성은 레이아웃 경계가 해석 되는 방법을 지정 합니다.
