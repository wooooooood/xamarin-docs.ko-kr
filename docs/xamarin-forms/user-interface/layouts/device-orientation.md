---
title: 디바이스 방향
description: 이 문서에서는 세로 및 가로 방향으로 크게 보이는 Xamarin.ios 응용 프로그램을 레이아웃 하는 방법을 설명 합니다.
ms.prod: xamarin
...
Xamarin.ios를 사용 하는 경우 장치 방향을 제어 하는 지원 되는 방법은 각 개별 프로젝트에 대 한 설정을 사용 하는 것입니다.
...
Android에서 방향을 제어 하려면 **MainActivity.cs** 를 열고 클래스를 `MainActivity` 데코레이팅하는 특성을 사용 하 여 방향을 설정 합니다.
...
- **가로** &ndash; 는 센서 데이터에 관계 없이 응용 프로그램 방향을 가로로 강제 합니다.
- **세로** &ndash; 는 센서 데이터에 관계 없이 응용 프로그램 방향을 세로 방향으로 강제 합니다.
- 사용자가 사용자의 기본 설정 된 방향을 사용 하 여 응용 프로그램을 표시 합니다. **User** &ndash;
- **뒤** &ndash; 에는 응용 프로그램의 방향이 그 뒤의 [작업](xref:Android.App.Activity) 방향과 동일 하 게 됩니다.
- **센서** &ndash; 를 사용 하면 사용자가 자동 회전을 사용 하지 않도록 설정한 경우에도 센서에 의해 응용 프로그램의 방향이 결정 됩니다.
- **SensorLandscape** &ndash; 를 사용 하면 응용 프로그램에서 센서 데이터를 사용 하 여 화면이 마주보는 방향을 변경할 수 있습니다 (화면이 거꾸로 표시 되지 않음).
- **SensorPortrait** &ndash; 를 사용 하면 응용 프로그램에서 센서 데이터를 사용 하는 동안 세로 방향을 사용 하 여 화면이 마주보는 방향을 변경할 수 있습니다. 그러면 화면이 거꾸로 표시 되지 않습니다.
...
- **Fullsensor** &ndash; 를 사용 하면 응용 프로그램에서 센서 데이터를 사용 하 여 올바른 방향 (가능한 4 개)을 선택 합니다.
- **Fulluser** &ndash; 를 사용 하면 응용 프로그램에서 사용자의 방향 기본 설정을 사용 합니다. 자동 회전을 사용 하는 경우 4 방향 모두를 사용할 수 있습니다.
- 사용자가 자동 회전을 사용 하는 경우를 제외 하 고는 사용자가 자동 회전을 사용 하는 경우를 제외 하 고 **는 가로 방향을** 사용 하 여 응용 프로그램에서 방향을 결정 합니다. _ \[\] _ &ndash; 이 옵션을 선택 하면 컴파일이 중단 됩니다.
- **Userportrait** &ndash; _는 지원\] 되지 않습니다. 사용자가 자동 회전을 사용 하도록 설정 하지 않으면 응용 프로그램에서 세로 방향을 사용 하 게 됩니다 .이 경우 센서를 사용 하 여 방향을 결정 \[_ 합니다. 이 옵션을 선택 하면 컴파일이 중단 됩니다.
- **Locked** &ndash; _잠금 \[지원 안\] 됨_ 을 사용 하면 응용 프로그램에서 장치의 실제 방향 변경에 응답 하지 않고 화면 방향을 사용 하 여 시작 시와 동일 하 게 됩니다. 이 옵션을 선택 하면 컴파일이 중단 됩니다.
...
Xamarin.ios는 공유 코드의 방향 변경을 앱에 알리기 위한 네이티브 이벤트를 제공 하지 않습니다. 그러나[xamarin.ios](~/essentials/index.md) 는 방향 변경에 대 한`DeviceDisplay`알림을 제공 하는 [] 클래스를 포함 합니다.

Xamarin.ios가 없는 방향을 감지 하려면의 너비나 높이가 `SizeChanged` `Page` `Page` 변경 될 때 발생 하는의 이벤트를 모니터링 합니다. 의 `Page` 너비가 높이 보다 크면 장치가 가로 모드입니다. 자세한 내용은 [화면 방향에 따라 이미지 표시](https://github.com/xamarin/recipes/tree/master/Recipes/xamarin-forms/Controls/screen-orientation)를 참조 하세요.

또는에서 메서드를 [`OnSizeAllocated`](xref:Xamarin.Forms.Page.OnSizeAllocated*) 재정의 하 여 레이아웃 변경 논리 `Page`를 삽입할 수 있습니다. 메서드 `OnSizeAllocated` 는가 장치를 회전할 `Page` 때마다 발생 하는 새 크기가 할당 될 때마다 호출 됩니다. 의 `OnSizeAllocated` 기본 구현에서는 중요 한 레이아웃 함수를 수행 하므로 재정의에서 기본 구현을 호출 하는 것이 중요 합니다.
...
장치를 회전할 `OnSizeAllocated` 때 메서드를 여러 번 호출할 수 있습니다. 언제 든 지 레이아웃을 변경 하면 불필요 한 리소스를 사용할 수 있으며 깜박임이 발생할 수 있습니다. 페이지 내에서 인스턴스 변수를 사용 하 여 방향이 가로 또는 세로 인지 여부를 추적 하 고 변경 내용이 있을 때만 다시 그리도록 합니다.
...
- 비율과 관련 하 여 특정 가정이 적용 될 때 방향의 **비율로** &ndash; 인해 문제가 발생할 수 있습니다. 예를 들어 세로 방향의 화면 세로 공간에 대 한 1/3의 공간이 충분 한 뷰는 가로 세로 공간의 1/3에 맞지 않을 수 있습니다.
- 세로에서 의미가 **있는** &ndash; 절대 절대값 (픽셀) 값은 가로에서 의미가 없을 수 있습니다. 절대 값이 필요한 경우 중첩 된 레이아웃을 사용 하 여 해당 영향을 격리 합니다. 예를 들어 항목 템플릿에 균일 한 높이가 보장 `TableView` `ItemTemplate` 되는 경우에서 절대 값을 사용 하는 것이 합리적입니다.

위의 규칙은 여러 화면 크기에 대 한 인터페이스를 구현 하는 경우에도 적용 되며 일반적으로 최상의 방법으로 간주 됩니다. 이 가이드의 나머지 부분에서는 Xamarin.ios의 각 기본 레이아웃을 사용 하 여 반응 형 레이아웃의 특정 예를 설명 합니다.

> [!NOTE]
> 명확 하 게 하기 위해 다음 섹션에서는 한 번 `Layout` 에 한 가지 유형의만 사용 하 여 응답성이 뛰어난 레이아웃을 구현 하는 방법을 보여 줍니다. 실제로 각 구성 요소에 대해 보다 간단 하거나 `Layout`직관적 `Layout` 으로를 사용 하 여 원하는 레이아웃을 확보 하는 것이 더 간단 합니다.
...
일부 c #은 장치의 방향에 따라의 `outerStack` 방향을 변경 하는 데 사용 됩니다.
...
- 화면의 `ScrollView` 높이가 단추 및 이미지의 고정 높이의 합계 보다 작은 경우에도를 사용 하 여 레이블을 표시할 수 있습니다.
