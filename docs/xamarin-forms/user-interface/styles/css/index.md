...
스타일 시트를에 추가 하기 전에 [`StyleSheet`](xref:Xamarin.Forms.StyleSheets.StyleSheet) 클래스를 사용 하 여 로드 하 고 구문 [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)분석할 수 있습니다.
...
속성 [`StyleSheet.Source`](xref:Xamarin.Forms.Xaml.StyleSheetExtension.Source) 은 바깥쪽 XAML 파일의 위치를 기준으로 하는 uri로 스타일 시트를 지정 하거나 uri가로 시작 하는 경우 프로젝트 루트에 상대적인 uri `/`로 지정 합니다.

> [!WARNING]
> 빌드 작업이 **EmbeddedResource**으로 설정 되지 않은 경우 CSS 파일을 로드 하지 못합니다.

또는 스타일 시트를에 추가 [`StyleSheet`](xref:Xamarin.Forms.StyleSheets.StyleSheet) [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)하기 전에 클래스를 사용 하 여 로드 하 고 구문 분석할 수 있습니다 ( `CDATA` 섹션에서 인라인).
...
C #에서는에서 `StringReader` 스타일 시트를 로드 하 여에 추가할 수 있습니다. [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)
...
`StyleSheet.FromReader` 메서드에 `TextReader` 대 한 인수는 스타일 시트를 읽은입니다.
...
이 선택기는 스타일 [`StackLayout`](xref:Xamarin.Forms.StackLayout) 시트를 사용 하는 페이지의 모든 요소를 식별 하 고 해당 여백을 균일 두께 20으로 설정 합니다.
...
이 선택기는 스타일 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 시트를 사용 하는 모든 요소를 식별 하 고 배경색을 `lightgray`로 설정 합니다.

> [!NOTE]
> 선택기 `^base` 는 xamarin.ios에 고유 하며 CSS 사양의 일부가 아닙니다.

### <a name="selecting-an-element-by-name"></a>이름으로 요소 선택

시각적 트리의 개별 요소는 대/소문자를 구분 `#id` 하는 선택기를 사용 하 여 선택할 수 있습니다.
...
이 선택기는 [`StyleId`](xref:Xamarin.Forms.Element.StyleId) 속성이로 `listView`설정 된 요소를 식별 합니다. 그러나 `StyleId` 속성을 설정 하지 않으면 선택기는 요소의를 `x:Name` 사용 하는 것으로 대체 합니다. 따라서 다음 XAML 예제에서 선택기 `#listView` 는 [`ListView`](xref:Xamarin.Forms.ListView) `x:Name` 특성이로 `listView`설정 된를 식별 하 고 배경색을로 `lightgray`설정 합니다.
...
대/소문자를 구분 `.class` 하는 선택기를 사용 하 여 특정 클래스 특성을 가진 요소를 선택할 수 있습니다.
...
요소의 [`StyleClass`](xref:Xamarin.Forms.NavigableElement.StyleClass) 속성을 css 클래스 이름으로 설정 하 여 css 클래스를 XAML 요소에 할당할 수 있습니다. 따라서 다음 XAML 예제에서는 클래스에 `.detailPageTitle` 의해 정의 된 스타일이 첫 [`Label`](xref:Xamarin.Forms.Label)번째에 할당 되는 반면 `.detailPageSubtitle` 클래스에서 정의한 스타일은 두 번째 `Label`에 할당 됩니다.
...
시각적 트리의 자식 요소는 대/소문자를 구분 하지 않는 `element element` 선택기를 사용 하 여 선택할 수 있습니다.
...
이 선택기는 [`ListView`](xref:Xamarin.Forms.ListView) 요소의 [`Image`](xref:Xamarin.Forms.Image) 자식인 요소를 식별 하 고 높이와 너비를 60으로 설정 합니다. 따라서 다음 XAML 예제에서 선택기는 `listview image` 의 자식인 [`Image`](xref:Xamarin.Forms.Image) [`ListView`](xref:Xamarin.Forms.ListView)를 식별 하 고 높이와 너비를 60으로 설정 합니다.
...
시각적 트리의 직접 자식 요소는 대/소문자를 구분 하지 않는 `element>element` 선택기로 선택할 수 있습니다.
...
이 선택기는 [`StackLayout`](xref:Xamarin.Forms.StackLayout) 요소의 [`Image`](xref:Xamarin.Forms.Image) 직계 자식인 모든 요소를 식별 하 고 높이와 너비를 200으로 설정 합니다. 따라서 다음 XAML 예제에서 선택기는 `stacklayout>image` 의 직계 자식인 [`Image`](xref:Xamarin.Forms.Image) [`StackLayout`](xref:Xamarin.Forms.StackLayout)를 식별 하 고 높이와 너비를 200으로 설정 합니다.
...
> [!NOTE]
> 선택기 `element>element` 를 사용 하려면 자식 요소가 부모의 _직계_ 자식 이어야 합니다.
...
|선택기|예제|Description|
|---|---|---|
|`.class`|`.header`|' Header '가 포함 `StyleClass` 된 속성을 사용 하 여 모든 요소를 선택 합니다. 이 선택기는 대/소문자를 구분 합니다.|
|`#id`|`#email`|가로 설정 된 `StyleId` 모든 요소 `email`를 선택 합니다. 가 `StyleId` 설정 되지 않은 경우로 `x:Name`대체 합니다. XAML을 `x:Name` 사용 하는 경우 보다 `StyleId`우선 합니다. 이 선택기는 대/소문자를 구분 합니다.|
|`*`|`*`|모든 요소를 선택 합니다.|
|`element`|`label`|형식의 `Label`모든 요소를 선택 하지만 서브 클래스는 선택 하지 않습니다. 이 선택기는 대/소문자를 구분 하지 않습니다.|
|`^base`|`^contentpage`|자체를 포함 하 `ContentPage` `ContentPage` 여를 기본 클래스로 사용 하는 모든 요소를 선택 합니다. 이 선택기는 대/소문자를 구분 하지 않으며 CSS 사양의 일부가 아닙니다.|
|`element,element`|`label,button`|모든 `Button` 요소와 모든 `Label` 요소를 선택 합니다. 이 선택기는 대/소문자를 구분 하지 않습니다.|
|`element element`|`stacklayout label`|내에서 `Label` 모든 요소를 `StackLayout`선택 합니다. 이 선택기는 대/소문자를 구분 하지 않습니다.|
|`element>element`|`stacklayout>label`|을 직계 `Label` 부모로 사용 `StackLayout` 하 여 모든 요소를 선택 합니다. 이 선택기는 대/소문자를 구분 하지 않습니다.|
|`element+element`|`label+entry`|바로 다음 `Entry` 에 오는 모든 요소 `Label`를 선택 합니다. 이 선택기는 대/소문자를 구분 하지 않습니다.|
|`element~element`|`label~entry`|뒤에 `Entry` 있는 모든 요소를 `Label`선택 합니다. 이 선택기는 대/소문자를 구분 하지 않습니다.|

일치 하는 선택기를 사용 하는 스타일은 정의 순서 대로 연속적으로 적용 됩니다. 특정 항목에 대해 정의 된 스타일은 항상 마지막에 적용 됩니다.

> [!TIP]
> 선택기는와 `StackLayout>ContentView>label.email`같은 제한 없이 결합 될 수 있습니다.
...
다음 CSS 속성은 Xamarin.ios에서 지원 됩니다. **값** 열에서 형식은 _기울임꼴_이 고 문자열 리터럴은 `gray`입니다.
...
|`flex-basis`|`VisualElement`|_float_ \| float `auto` . \| `initial` 또한 `%` 부호를 사용 하 여 0%에서 100% 범위의 백분율을 지정할 수 있습니다.|`flex-basis: 25%;`|
...
- `all: initial`.
- 레이아웃 속성 (상자 또는 그리드).
- `font`, 및 `border`와 같은 줄임 속성입니다.

또한 값이 없으므로 `inherit` 상속이 지원 되지 않습니다. 따라서 예를 들어 레이아웃의 `font-size` 속성을 설정 하 고 레이아웃의 모든 [`Label`](xref:Xamarin.Forms.Label) 인스턴스가 값을 상속 하도록 할 수는 없습니다. 한 가지 예외는 `direction` 속성 이며 기본값은 `inherit`입니다.

요소 `Span` 를 대상으로 지정 하는 경우 요소와 이름 모두에서 기호를 `#` 사용 하 여 CSS 스타일의 대상이 되는 것을 방지 하는 알려진 문제가 있습니다. 요소 `Span` 는 `StyleClass` 속성이 없는 `GestureElement`에서 파생 되므로 범위는 CSS 클래스 대상을 지원 하지 않습니다. 자세한 내용은 [범위 제어에 CSS 스타일을 적용할 수 없음](https://github.com/xamarin/Xamarin.Forms/issues/5979)을 참조 하세요.

### <a name="xamarinforms-specific-properties"></a>Xamarin.ios 관련 속성

다음 Xamarin.ios 관련 CSS 속성도 지원 됩니다 ( **값** 열에서 형식은 _기울임꼴_, 문자열 리터럴은 다음과 같습니다 `gray`).
...
|`-xf-orientation`|`ScrollView`, `StackLayout`| `horizontal` \| `vertical` \| `both` \| `initial`. `both`는 `ScrollView`에서만 지원 됩니다. |`-xf-orientation: horizontal;`|
...
다음 Xamarin. Forms 셸 특정 CSS 속성도 지원 됩니다 ( **값** 열에서 형식은 _기울임꼴_, 문자열 리터럴은 다음과 같습니다 `gray`).
...
지원 되 `color` 는 값은 다음과 같습니다.

- `X11`CSS 색, UWP 미리 정의 된 색 및 Xamarin.ios 색과 일치 하는 [색](https://en.wikipedia.org/wiki/X11_color_names)입니다. 이러한 색 값은 대/소문자를 구분 하지 않습니다.
...
각각 공백으로 구분 되는 1, 2 `thickness` , 3 또는 4 개의 값이 지원 됩니다.
...
> [!NOTE]
> CSS `thickness` 값은 XAML [`Thickness`](xref:Xamarin.Forms.Thickness) 값과 다릅니다. 예를 들어 XAML에서 두 값 `Thickness` 은 가로, 세로 두께를 나타내고 네 값 `Thickness` 은 왼쪽, 위쪽, 오른쪽, 아래쪽 두께를 나타냅니다. 또한 XAML `Thickness` 값은 쉼표로 구분 됩니다.

### <a name="namedsize"></a>NamedSize

다음과 같은 대/ `namedsize` 소문자를 구분 하지 않는 값이 지원 됩니다.
