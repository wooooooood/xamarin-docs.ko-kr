...
`DatePicker`의 `BindingContext` 속성이 `MinDate``DateTime` 형식의 속성이 포함 된 viewmodel의 인스턴스로 설정 된 경우 (예를 들어) 다음과 같이 `MaxDate`을 인스턴스화할 수 있습니다.
...
이 예제에서는 세 개의 속성이 모두 viewmodel의 해당 속성으로 초기화 됩니다. `Date` 속성에는 `TwoWay`의 바인딩 모드가 있으므로 사용자가 선택 하는 모든 새 날짜는 viewmodel에 자동으로 반영 됩니다.
...
이 샘플은 처음 실행 한 경우, 둘 다 `DatePicker` 뷰 오늘 날짜에 초기화 됩니다. 다음 스크린샷에서는 iOS 및 Android에서 실행 되는 프로그램을 보여 줍니다.
