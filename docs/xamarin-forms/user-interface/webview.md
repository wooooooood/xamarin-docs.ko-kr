...
인터넷에서 웹 사이트를 표시 하려면의 `WebView` [`Source`](xref:Xamarin.Forms.WebViewSource) 속성을 문자열 URL로 설정 합니다.
...
버전 9부터 iOS는 응용 프로그램이 기본적으로 최상의 보안을 구현 하는 서버와 통신할 수 있도록 허용 합니다. 안전 하지 않은 서버와 `Info.plist` 의 통신을 사용 하려면에서 값을 설정 해야 합니다.

> [!NOTE]
> 응용 프로그램에 보안 되지 않은 웹 사이트에 대 한 연결이 필요한 경우에는를 사용 하 여 ATS `NSExceptionDomains` off를 완전히 사용 `NSAllowsArbitraryLoads`하는 대신를 사용 하 여 도메인을 항상 예외로 입력 해야 합니다. `NSAllowsArbitraryLoads`매우 긴급 한 상황 에서만 사용 해야 합니다.
...
코드에 동적으로 정의 된 HTML 문자열을 표시 하려면 다음과 같이의 [`HtmlWebViewSource`](xref:Xamarin.Forms.HtmlWebViewSource)인스턴스를 만들어야 합니다.
...
> [!NOTE]
> 가 자식인 레이아웃에 따라 `WidthRequest` 의 및 `HeightRequest` 속성 [`WebView`](xref:Xamarin.Forms.WebView) 을 설정 하 여 HTML 콘텐츠를 확인 해야 할 수도 있습니다. `WebView` 예를 들어이는에 필요 [`StackLayout`](xref:Xamarin.Forms.StackLayout)합니다.
...
을 `WebView`사용 하 여 로컬 콘텐츠를 표시 하려면 HTML 파일을 다른 것 처럼 연 다음의 `Html` 속성에 문자열을 문자열로 로드 해야 합니다. `HtmlWebViewSource` 파일 열기에 대 한 자세한 내용은 [파일 작업](~/xamarin-forms/data-cloud/data/files.md)을 참조 하세요.
...
이를 해결 하려면 파일 시스템에서 파일을 `WebView` 찾을 위치를 알려 주어 야 합니다. 에서 `BaseUrl` `HtmlWebViewSource` 사용 하는에 대 한 속성을 설정 하 `WebView`여이 작업을 수행 합니다.

각 운영 체제의 파일 시스템은 서로 다르기 때문에 각 플랫폼에서 해당 URL을 확인 해야 합니다. Xamarin.ios는 런타임에 각 플랫폼 `DependencyService` 에서 종속성을 확인 하기 위한를 제공 합니다.

를 사용 `DependencyService`하려면 먼저 각 플랫폼에서 구현할 수 있는 인터페이스를 정의 합니다.

```csharp
public interface IBaseUrl { string Get(); }
```

각 플랫폼에서 인터페이스가 구현 될 때까지 앱이 실행 되지 않습니다. 공통 프로젝트에서를 `BaseUrl` 사용 하 여를 설정 해야 합니다. `DependencyService`
...
Android에서을로 `BaseUrl` `"file:///android_asset/"`설정 해야 합니다.
...
Android에서 **자산** 폴더의 파일은 속성에 `MainActivity.Instance` 의해 노출 되는 현재 Android 컨텍스트를 통해 액세스할 수도 있습니다.
...
는 `BaseUrl` 로 `"ms-appx-web:///"`설정 해야 합니다.
...
- **Goforward ()** &ndash; `CanGoForward` 가 true 이면를 호출 `GoForward` 하면 다음에 방문한 페이지로 이동 합니다.
- **GoBack ()** &ndash; `CanGoBack` 이 true 이면를 호출 `GoBack` 하면 마지막으로 방문한 페이지로 이동 합니다.
- **CanGoBack** &ndash; `true` 브라우저가 시작 URL에 `false` 있으면 다시 탐색할 페이지가 있으면 CanGoBack.
- **CanGoForward** &ndash; CanGoForward `true` 사용자가 뒤로 탐색 하 고 이미 방문한 페이지로 이동할 수 있습니다.

페이지 내에서 `WebView` 는 멀티 터치 제스처를 지원 하지 않습니다. 콘텐츠가 모바일에 최적화 되 고 확대/축소를 요구 하지 않고 표시 되는지 확인 하는 것이 중요 합니다.

응용 프로그램에서 장치 브라우저가 아니라 내 `WebView`에 링크를 표시 하는 것이 일반적입니다. 이러한 상황에서 일반적인 탐색을 허용 하는 것이 유용 하지만, 시작 링크에 있는 동안 사용자가 다시 방문 하면 앱이 일반 앱 보기로 돌아옵니다.
...
이벤트 [`WebNavigatingEventArgs`](xref:Xamarin.Forms.WebNavigatingEventArgs) 와 [`Navigating`](xref:Xamarin.Forms.WebView.Navigating) 함께 제공 되는 개체에는 다음과 같은 네 가지 속성이 있습니다.
...
이벤트 [`WebNavigatedEventArgs`](xref:Xamarin.Forms.WebNavigatedEventArgs) 와 [`Navigated`](xref:Xamarin.Forms.WebView.Navigated) 함께 제공 되는 개체에는 다음과 같은 네 가지 속성이 있습니다.
...
`Reload` 메서드가 호출 되 면 현재 콘텐츠 `ReloadRequested` 를 다시 로드 하도록 요청 되었음을 나타내는 이벤트가 발생 합니다.

## <a name="performance"></a>성능

널리 사용 되는 웹 브라우저는 하드웨어 가속 렌더링 및 JavaScript 컴파일과 같은 기술을 채택 합니다. Xamarin. Forms 4.4 이전에는 `WebView` `UIWebView` 클래스를 통해 iOS에서 xamarin.ios를 구현 했습니다. 그러나이 구현에서는 이러한 기술을 대부분 사용할 수 없었습니다. 따라서 Xamarin.ios 4.4부터 Xamarin.ios `WebView` 는 `WkWebView` 클래스를 통해 iOS에서 구현 되므로 더 빠른 검색을 지원 합니다.

> [!NOTE]
> IOS에서에는 `WkWebViewRenderer` `WkWebViewConfiguration` 인수를 허용 하는 생성자 오버 로드가 있습니다. 이렇게 하면 렌더러를 만들 때 구성할 수 있습니다.

응용 프로그램은 호환성을 위해 iOS `UIWebView` 클래스를 사용 하 여 xamarin.ios `WebView`를 구현할 수 있습니다. 응용 프로그램에 대 한 iOS 플랫폼 프로젝트의 **AssemblyInfo.cs** 파일에 다음 코드를 추가 하 여이 작업을 수행할 수 있습니다.
...
가 작동 `WebView` 하려면 각 플랫폼에 대 한 사용 권한이 설정 되어 있는지 확인 해야 합니다. 일부 플랫폼 `WebView` 에서는 디버그 모드에서 작동 하지만 릴리스를 위해 빌드할 때는 작동 하지 않습니다. Android에서 인터넷에 액세스 하는 것과 같은 일부 사용 권한은 기본적으로 디버그 모드에서 Mac용 Visual Studio 하 여 설정 되기 때문입니다.

- **UWP** &ndash; 에는 네트워크 콘텐츠를 표시할 때 인터넷 (클라이언트 & 서버) 기능이 필요 합니다.
- **Android** &ndash; 는 `INTERNET` 네트워크에서 콘텐츠를 표시 하는 경우에만 필요 합니다. 로컬 콘텐츠에는 특별 한 권한이 필요 하지 않습니다.
- **iOS** &ndash; 에는 특별 한 권한이 필요 하지 않습니다.

## <a name="layout"></a>레이아웃

대부분의 다른 Xamarin.ios 뷰와 달리, `WebView` stacklayout 또는 `HeightRequest` RelativeLayout `WidthRequest` 에 포함 된 경우 및가 지정 되어야 합니다. 이러한 속성을 지정 하지 않으면 `WebView` 가 렌더링 되지 않습니다.

다음 예제에서는 렌더링 `WebView`을 수행 하는 레이아웃을 보여 줍니다.
...
[`WebView`](xref:Xamarin.Forms.WebView)c #에서 JavaScript 함수를 호출 하 고 모든 결과를 호출 하는 c # 코드에 반환 하는 기능을 포함 합니다. 이 작업은 [`WebView.EvaluateJavaScriptAsync`](xref:Xamarin.Forms.WebView.EvaluateJavaScriptAsync*) [웹 보기](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-webview) 샘플의 다음 예제에 표시 된 메서드를 사용 하 여 수행 됩니다.
...
메서드 [`WebView.EvaluateJavaScriptAsync`](xref:Xamarin.Forms.WebView.EvaluateJavaScriptAsync*) 는 인수로 지정 된 JavaScript를 평가 하 고 모든 결과를 `string`로 반환 합니다. 이 예제에서는의 `factorial` `number` 계승을 반환 하는 JavaScript 함수가 호출 됩니다. 이 JavaScript 함수는가 [`WebView`](xref:Xamarin.Forms.WebView) 로드 하는 로컬 HTML 파일에 정의 되어 있으며 다음 예제에 나와 있습니다.
...
에 대해 쿠키 [`WebView`](xref:Xamarin.Forms.WebView)를 설정할 수 있습니다. 그러면이 쿠키는 웹 요청과 함께 지정 된 URL에 전송 됩니다. 이렇게 하려면 `Cookie` `CookieContainer`개체를에 추가 하 여이를 수행 합니다. 그러면이 개체는 `WebView.Cookies` 바인딩 가능한 속성의 값으로 설정 됩니다. 다음 코드에서 그 예를 볼 수 있습니다.
...
이 예제에서는 `Cookie` `CookieContainer` 개체에 단일가 추가 되 고,이 개체는 `WebView.Cookies` 속성의 값으로 설정 됩니다. 에서 [`WebView`](xref:Xamarin.Forms.WebView) 지정 된 URL로 웹 요청을 보내는 경우 쿠키는 요청과 함께 전송 됩니다.

## <a name="uiwebview-deprecation-and-app-store-rejection-itms-90809"></a>UIWebView 보기 사용 중단 및 앱 스토어 거부 (ITMS-90809)

4 월 2020부터 [Apple은](https://developer.apple.com/news/?id=12232019b) 아직 사용 되지 `UIWebView` 않는 API를 사용 하는 앱을 거부 합니다. Xamarin.ios가 기본값으로 전환 `WKWebView` 되었지만 xamarin.ios 이진 파일에는 여전히 이전 SDK에 대 한 참조가 있습니다. 현재 [iOS 링커](~/ios/deploy-test/linker.md) 동작은이를 제거 하지 않습니다. 따라서 앱 스토어에 제출할 `UIWebView` 때 사용 되지 않는 API가 여전히 앱에서 참조 되는 것으로 나타납니다.

이 문제를 해결 하기 위해 링커의 미리 보기 버전을 사용할 수 있습니다. 미리 보기를 사용 하도록 설정 하려면 링커에 추가 인수 `--optimize=experimental-xforms-product-type` 를 제공 해야 합니다.
...
링커가 참조를 제거 `UIWebView` 하려면 다음 단계를 수행 합니다.

1. **Ios 프로젝트 속성** &ndash; 을 엽니다. ios 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 **속성**을 선택 합니다.
1. Ios 빌드 &ndash; **섹션으로 이동 하** 여 **ios 빌드** 섹션을 선택 합니다.
1. **추가 mtouch 인수** 에서 추가 **mtouch 인수** &ndash; 를 업데이트 합니다 .이 플래그 `--optimize=experimental-xforms-product-type` 는 이미 여기에 있을 수 있는 값과 함께 추가 합니다. 참고:이 플래그는 **SDK로만** 설정 된 **링커 동작과** 함께 작동 하거나 **모두 연결**합니다. 어떤 이유로 든 링커 동작을 모두로 설정할 때 오류가 표시 되는 경우이는 응용 프로그램 코드 또는 링커 안전 하지 않은 타사 라이브러리에서 문제가 될 가능성이 높습니다. 링커에 대 한 자세한 내용은 [Xamarin.ios 앱 연결](~/ios/deploy-test/linker.md)을 참조 하세요.
1. **모든 빌드 구성** &ndash; 업데이트 모든 빌드 구성을 업데이트 하려면 창 맨 위의 **구성** 및 **플랫폼** 목록을 사용 합니다. 업데이트 하는 가장 중요 한 구성은 **릴리스/iPhone** 구성으로, 일반적으로 앱 스토어 전송용 빌드를 만드는 데 사용 되기 때문입니다.
...
1. **Ios 프로젝트 옵션** &ndash; 열기 ios 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 **옵션**을 선택 합니다.
1. Ios 빌드 &ndash; **섹션으로 이동 하** 여 **ios 빌드** 섹션을 선택 합니다.
1. **_mtouch_ ** **추가 _mtouch_ 인수** 에서 추가 mtouch 인수 &ndash; 를 업데이트 합니다 .이 플래그 `--optimize=experimental-xforms-product-type` 는 이미 여기에 있을 수 있는 값과 함께 추가 합니다. 참고:이 플래그는 **SDK로만** 설정 된 **링커 동작과** 함께 작동 하거나 **모두 연결**합니다. 어떤 이유로 든 링커 동작을 모두로 설정할 때 오류가 표시 되는 경우이는 응용 프로그램 코드 또는 링커 안전 하지 않은 타사 라이브러리에서 문제가 될 가능성이 높습니다. 링커에 대 한 자세한 내용은 [Xamarin.ios 앱 연결](~/ios/deploy-test/linker.md)을 참조 하세요.
1. **모든 빌드 구성** &ndash; 업데이트 모든 빌드 구성을 업데이트 하려면 창 맨 위의 **구성** 및 **플랫폼** 목록을 사용 합니다. 업데이트 하는 가장 중요 한 구성은 **릴리스/iPhone** 구성으로, 일반적으로 앱 스토어 전송용 빌드를 만드는 데 사용 되기 때문입니다.
