...
[`Text`](xref:Xamarin.Forms.Button.Text) 속성은 `Button`에 나타나는 텍스트를 지정합니다. 합니다 [ `Clicked` ](xref:Xamarin.Forms.Button.Clicked) 이벤트를 명명 된 이벤트 처리기로 `OnButtonClicked`합니다. 이 처리기를 코드 숨김 파일에 위치한 **BasicButtonClickPage.xaml.cs**:
...
> [!NOTE]
> `Button` 클래스에 [ `Margin` ](xref:Xamarin.Forms.View.Margin) 하 고 [ `Padding` ](xref:Xamarin.Forms.Button.Padding) 레이아웃 동작을 제어 하는 속성을 `Button`합니다. 자세한 내용은 [여백 및 패딩](~/xamarin-forms/user-interface/layouts/margin-and-padding.md)을 참조하세요.
