...
이미지 파일 각 응용 프로그램 프로젝트에 추가 하 고 Xamarin.Forms 공유 코드에서 참조할 수 있습니다. 이미지를 배포하는 이 메서드는 다양한 플랫폼 또는 약간 다른 디자인의 다양한 해결 방법을 사용하는 경우와 같이 이미지가 특정 플랫폼을 지정하는 경우에 필요합니다.
...
- [`Page`](xref:Xamarin.Forms.Page). `Page`에서 파생 되는 모든 페이지 형식에는 파일, 포함 리소스, URI 또는 스트림이 할당 될 수 있는 [`IconImageSource`](xref:Xamarin.Forms.Page.IconImageSource) 및 [`BackgroundImageSource`](xref:Xamarin.Forms.Page.BackgroundImageSource) 속성이 있습니다. 때와 같이 특정 상황을 [ `NavigationPage` ](xref:Xamarin.Forms.NavigationPage) 표시 되는 [ `ContentPage` ](xref:Xamarin.Forms.ContentPage), 플랫폼에서 지원 되는 경우 아이콘이 표시 됩니다.

  > [!IMPORTANT]
  > Ios의 경우는 [ `Page.IconImageSource` ](xref:Xamarin.Forms.Page.IconImageSource) 이미지 자산 카탈로그 이미지 집합에서 속성을 채울 수 없습니다. 대신 파일, 포함 리소스, URI 또는 스트림에서 `Page.IconImageSource` 속성에 대 한 아이콘 이미지를 로드 합니다.
