...
description: 이미지는 Xamarin.ios를 사용 하 여 플랫폼 간에 공유할 수 있으며, 각 플랫폼에 대해 특별히 로드 하거나 표시를 위해 다운로드할 수 있습니다.
ms.prod: xamarin
...
ms.locfileid: "82139072"
---
# <a name="images-in-xamarinforms"></a>Xamarin.ios의 이미지
...
Xamarin.ios는 [`Image`](xref:Xamarin.Forms.Image) 뷰를 사용 하 여 페이지에 이미지를 표시 합니다. 여기에는 두 가지 중요 한 속성이 있습니다.
...
속성 [`Aspect`](xref:Xamarin.Forms.Image.Aspect) 은 이미지가 표시 영역에 맞게 조정 되는 방법을 결정 합니다.
...
이미지는 [로컬 파일](#local-images)에서 로드 하거나, [포함 된 리소스](#embedded-images)에서 로드 하거나, [다운로드](#download-images)하거나, 스트림에서 로드할 수 있습니다. 또한 `FontImageSource` 개체에 글꼴 아이콘 데이터를 지정 하 여 [`Image`](xref:Xamarin.Forms.Image) 보기에서 글꼴 아이콘을 표시할 수 있습니다. 자세한 내용은 글꼴 가이드에서 [글꼴 아이콘 표시](~/xamarin-forms/user-interface/text/fonts.md#display-font-icons) [를 참조](~/xamarin-forms/user-interface/text/fonts.md) 하세요.

## <a name="local-images"></a>로컬 이미지

이미지 파일은 각 응용 프로그램 프로젝트에 추가 되 고 Xamarin.ios 공유 코드에서 참조 될 수 있습니다. 이미지를 배포하는 이 메서드는 다양한 플랫폼 또는 약간 다른 디자인의 다양한 해결 방법을 사용하는 경우와 같이 이미지가 특정 플랫폼을 지정하는 경우에 필요합니다.
...
이 코드 예제에 `Device.RuntimePlatform` 표시 된 것 처럼 더 많은 유연성을 위해 속성을 사용 하 여 일부 또는 모든 플랫폼에 대해 다른 이미지 파일 또는 경로를 선택할 수 있습니다.
...
iOS, Android 및 UWP에는 다양 한 이미지 해상도에 대 한 지원이 포함 되어 있습니다. 여기서 운영 체제는 장치 기능에 따라 런타임에 적절 한 이미지를 선택 합니다. Xamarin.ios는 네이티브 플랫폼의 Api를 사용 하 여 로컬 이미지를 로드 하므로 파일의 이름이 올바르고 프로젝트에 위치 하는 경우 대체 해상도를 자동으로 지원 합니다.
...
IOS 9 이전에는 **@2x** **@3x** 레 티 나 버전의 이미지를 **리소스** 폴더에 배치할 수 있습니다. 즉, 파일 확장명 앞에 파일 이름 (예: **myimage@2x.png**). 그러나 iOS 앱에서 이미지를 사용 하는이 방법은 Apple에서 더 이상 사용 되지 않습니다. 자세한 내용은 [이미지 크기 및 파일 이름](~/ios/app-fundamentals/images-icons/displaying-an-image.md)을 참조 하세요.
...
UWP 이미지 파일 이름 [ `.scale-xxx` 앞에는 파일 확장명 앞](https://docs.microsoft.com/windows/uwp/app-resources/images-tailored-for-scale-theme-contrast)에 접미사를 추가할 `xxx` 수 있습니다. 여기서은 자산에 적용 되는 크기 조정의 백분율입니다 (예: **myimage**). 그런 다음 크기 조정 한정자 없이 코드 또는 XAML에서 이미지를 참조할 수 있습니다 (예: **myimage .png**). 이 플랫폼은 디스플레이의 현재 DPI를 기준으로 가장 가까운 적절 한 자산 크기를 선택 합니다.

### <a name="additional-controls-that-display-images"></a>이미지를 표시 하는 추가 컨트롤

일부 컨트롤에는 다음과 같은 이미지를 표시 하는 속성이 있습니다.

- [`Button`](xref:Xamarin.Forms.Button)에는 [`ImageSource`](xref:Xamarin.Forms.Button.ImageSource) `Button`에 표시할 비트맵 이미지로 설정할 수 있는 속성이 있습니다. 자세한 내용은 [단추를 사용 하 여 비트맵 사용](~/xamarin-forms/user-interface/button.md#using-bitmaps-with-buttons)을 참조 하세요.
- [`ImageButton`](xref:Xamarin.Forms.Button)에는 [`Source`](xref:Xamarin.Forms.ImageButton.Source) `ImageButton`에 표시할 이미지에 설정할 수 있는 속성이 있습니다. 자세한 내용은 [이미지 원본 설정](~/xamarin-forms/user-interface/imagebutton.md#setting-the-image-source)을 참조 하세요.
...
- [`Page`](xref:Xamarin.Forms.Page). 에서 `Page` 파생 되는 모든 페이지 형식 [`IconImageSource`](xref:Xamarin.Forms.Page.IconImageSource) 에 [`BackgroundImageSource`](xref:Xamarin.Forms.Page.BackgroundImageSource) 는 파일, 포함 리소스, URI 또는 스트림이 할당 될 수 있는 및 속성이 있습니다. 가를 표시 [`NavigationPage`](xref:Xamarin.Forms.NavigationPage) [`ContentPage`](xref:Xamarin.Forms.ContentPage)하는 경우와 같은 특정 상황에서는 플랫폼이 지 원하는 경우 아이콘이 표시 됩니다.

  > [!IMPORTANT]
  > IOS에서는 자산 카탈로그 [`Page.IconImageSource`](xref:Xamarin.Forms.Page.IconImageSource) 이미지 집합의 이미지에서 속성을 채울 수 없습니다. 대신 파일, 포함 리소스, URI `Page.IconImageSource` 또는 스트림에서 속성의 아이콘 이미지를 로드 합니다.
...
포함 이미지를 로드 하는 코드는 아래와 같이 단순히 **리소스 ID** 를 [`ImageSource.FromResource`](xref:Xamarin.Forms.ImageSource.FromResource*) 메서드에 전달 합니다.
...
> [!NOTE]
> 유니버설 Windows 플랫폼에 릴리스 모드에서 포함 이미지를 표시할 수 있도록 지원 하려면 이미지를 검색할 소스 어셈블리를 지정 `ImageSource.FromResource` 하는의 오버 로드를 사용 해야 합니다.

현재 리소스 식별자에 대 한 암시적 변환은 없습니다. 대신 또는 [`ImageSource.FromResource`](xref:Xamarin.Forms.ImageSource.FromResource*) `new ResourceImageSource()` 를 사용 하 여 포함 이미지를 로드 해야 합니다.
...
에서 `string` 로 `ResourceImageSource`의 기본 제공 형식 변환기가 없으므로 이러한 형식의 이미지를 XAML에서 고유 하 게 로드할 수 없습니다. 대신 XAML로 지정 된 **리소스 ID** 를 사용 하 여 이미지를 로드 하도록 간단한 사용자 지정 xaml 태그 확장을 작성할 수 있습니다.
...
이 확장을 사용 하려면 프로젝트의 `xmlns` 올바른 네임 스페이스 및 어셈블리 값을 사용 하 여 사용자 지정를 XAML에 추가 합니다. 그런 다음이 구문을 사용 하 여 이미지 소스를 설정할 `{local:ImageResource WorkingWithImages.beach.jpg}`수 있습니다. 전체 XAML 예제는 다음과 같습니다.
...
기본적으로 메서드는 `ImageSource.FromResource` 메서드를 `ImageSource.FromResource` 호출 하는 코드와 동일한 어셈블리에서 이미지를 찾습니다. 위의 디버그 코드를 사용 하 여 `typeof()` 문을 각 어셈블리에 있는 `Type` 알려진으로 변경 하 여 특정 리소스를 포함 하는 어셈블리를 확인할 수 있습니다.
...
메서드 [`ImageSource.FromUri`](xref:Xamarin.Forms.ImageSource.FromUri(System.Uri)) 는 `Uri` 개체를 필요로 하며에서 읽는 새 [`UriImageSource`](xref:Xamarin.Forms.UriImageSource) 을 반환 합니다. `Uri`
...
- [`CachingEnabled`](xref:Xamarin.Forms.UriImageSource.CachingEnabled)-캐싱이 기본적`true` 으로 사용 되는지 여부입니다.
- [`CacheValidity`](xref:Xamarin.Forms.UriImageSource.CacheValidity)-이미지가 `TimeSpan` 로컬로 저장 되는 기간을 정의 하는입니다.
...
Xamarin.ios에는 작은 애니메이션 Gif를 표시 하기 위한 지원이 포함 되어 있습니다. 이 작업을 수행 하려면 속성 [`Image.Source`](xref:Xamarin.Forms.Image.Source) 을 애니메이션 GIF 파일로 설정 합니다.
...
기본적으로 애니메이션 GIF가 로드 되 면 재생 되지 않습니다. 이는 애니메이션 GIF `IsAnimationPlaying` 의 재생 또는 중지 여부를 제어 하는 속성이의 `false`기본값 이기 때문입니다. 형식의 `bool`이 속성은 [`BindableProperty`](xref:Xamarin.Forms.BindableProperty) 개체에 의해 지원 됩니다. 즉, 데이터 바인딩의 대상이 될 수 있고 스타일을 지정할 수 있습니다.

따라서 애니메이션 GIF가 로드 될 때 `IsAnimationPlaying` 속성이로 `true`설정 될 때까지 재생 되지 않습니다. 그런 다음 `IsAnimationPlaying` 속성을로 `false`설정 하 여 재생을 중지할 수 있습니다. 이 속성은 비 GIF 이미지 원본을 표시할 때 영향을 주지 않습니다.
...
응용 프로그램 아이콘 및 시작 [`Image`](xref:Xamarin.Forms.Image) 화면은 뷰와 관련이 없지만 xamarin.ios 프로젝트에서 이미지를 사용 하는 것도 중요 합니다.

Xamarin. Forms 앱에 대 한 아이콘 및 시작 화면 설정은 각 응용 프로그램 프로젝트에서 수행 됩니다. 즉, iOS, Android 및 UWP에 맞게 올바른 크기의 이미지를 생성 합니다. 이러한 이미지는 각 플랫폼 요구 사항에 따라 이름이 지정 되 고 배치 되어야 합니다.
...
또한 `FontImageSource` 개체에 글꼴 아이콘 데이터를 지정 하 여 [`Image`](xref:Xamarin.Forms.Image) 보기에서 글꼴 아이콘을 표시할 수 있습니다. 자세한 내용은 글꼴 가이드에서 [글꼴 아이콘 표시](~/xamarin-forms/user-interface/text/fonts.md#display-font-icons) [를 참조](~/xamarin-forms/user-interface/text/fonts.md) 하세요.
