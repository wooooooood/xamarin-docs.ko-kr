...
Xamarin.ios [`ListView`](xref:Xamarin.Forms.ListView) 를 사용 하면 목록의 각 행에 대 한 [`ViewCell`](xref:Xamarin.Forms.ViewCell) 인스턴스와 함께 목록의 표시를 사용자 지정할 수 있습니다.
...
위의 코드에서 형식의 `Add` `PageTypeGroup`인스턴스인의 `Groups`요소에 대해를 호출할 수도 있습니다. 는에서 `PageTypeGroup` `List<PageModel>`상속 되므로이 메서드는 가능 합니다.
...
`Header` 및/ `Footer` 또는`string` 를 값으로 설정 하거나 보다 복잡 한 레이아웃으로 설정할 수 있습니다. 이 밖에도 `HeaderTemplate` 고 `FooterTemplate` 수 있는 속성이 데이터 바인딩을 지 원하는 헤더 및 바닥글에 대 한 더 복잡 한 레이아웃을 만듭니다.
...
클래스에는 `HorizontalScrollBarVisibility` 가로 `VerticalScrollBarVisibility` 또는 세로 스크롤 막대가 표시 될 때 [`ScrollBarVisibility`](xref:Xamarin.Forms.ScrollBarVisibility) 를 나타내는 값을 가져오거나 설정 하는 및 속성이 있습니다. [`ListView`](xref:Xamarin.Forms.ListView) 두 속성은 모두 다음 값으로 설정할 수 있습니다.
...
개별 행 높이가 다른 원하는 경우 설정할 수 있습니다 합니다 `HasUnevenRows` 속성을 `true`입니다. 높이가 xamarin.ios에 의해 자동으로 계산 되기 때문 `HasUnevenRows` 에 행 높이를 `true`수동으로 설정 하지 않아도 됩니다.
