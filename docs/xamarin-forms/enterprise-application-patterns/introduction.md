...
이 가이드에서는 이러한 문제에 대 한 지침을 제공 하 고 Xamarin.ios를 사용 하 여 플랫폼 간 엔터프라이즈 앱을 빌드하기 위한 핵심 패턴 및 아키텍처에 초점을 맞춘 것입니다. 이 지침에서는 일반적인 Xamarin.ios enterprise 앱 개발 시나리오를 해결 하 고, 프레젠테이션, 프레젠테이션 논리 및 엔터티를 지 원하는 기능을 제공 하 여 MVVM (모델-뷰-ViewModel) 패턴입니다.
...
- IOS, Android 및 유니버설 Windows 플랫폼 (UWP)를 지 원하는 Xamarin.ios를 사용 하 여 개발 된 모바일 앱입니다.
...
EShopOnContainers 모바일 앱은 다음과 같은 Xamarin Forms 기능을 연습 합니다.
...
이 기능에 대 한 자세한 내용은 [xamarin.ios 설명서](~/xamarin-forms/index.yml)및 [xamarin.ios를 사용 하 여 Mobile Apps 만들기](https://aka.ms/xamebook)를 참조 하세요.
