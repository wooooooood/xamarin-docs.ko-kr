...
Xamarin.ios 응용 프로그램에서 뷰는 일반적으로 [`Page`](xref:Xamarin.Forms.Page)파생 된 클래스 또는 [`ContentView`](xref:Xamarin.Forms.ContentView)파생 클래스입니다. 그러나 뷰는 표시 될 때 개체를 시각적으로 나타내는 데 사용 되는 UI 요소를 지정 하는 데이터 템플릿으로 나타낼 수도 있습니다. 뷰로 표시 되는 데이터 템플릿에는 코드 숨김이 없으며 특정 뷰 모델 유형에 바인딩되도록 설계 되었습니다.
...
보기의 상호 작용에 대 한 응답으로 보기 모델에서 코드를 실행 하기 위한 몇 가지 옵션이 있습니다 (예: 단추 클릭 또는 항목 선택). 컨트롤이 명령을 지 원하는 경우 컨트롤의 `Command` 속성 `ICommand` 을 뷰 모델의 속성에 데이터 바인딩할 수 있습니다. 컨트롤의 명령이 호출 되 면 뷰 모델의 코드가 실행 됩니다. 명령 외에도 동작은 뷰의 개체에 연결 될 수 있으며 호출 되거나 발생 될 이벤트를 수신 대기할 수 있습니다. 이에 대 한 응답으로 동작은 뷰 모델 `ICommand` 또는 뷰 모델의 메서드에 대해를 호출할 수 있습니다.
...
뷰 모델에서 뷰를 사용 하 여 양방향 데이터 바인딩에 참여 시키려면 해당 속성에서 `PropertyChanged` 이벤트를 발생 시켜야 합니다. 뷰 모델은 `INotifyPropertyChanged` 인터페이스를 구현 하 고 속성이 변경 될 때 `PropertyChanged` 이벤트를 발생 시켜이 요구 사항을 충족 합니다.

컬렉션의 경우 보기 친화적인 `ObservableCollection<T>` 가 제공 됩니다. 이 컬렉션은 개발자가 컬렉션에서 `INotifyCollectionChanged` 인터페이스를 구현할 필요가 하므로 컬렉션 변경 알림을 구현 합니다.
...
> [!TIP]
> 보기 모델과 뷰를 독립적으로 유지 합니다. 뷰를 데이터 원본의 속성에 바인딩하는 것은 해당 뷰 모델에 대 한 뷰의 주 종속성 이어야 합니다. 특히 [`Button`](xref:Xamarin.Forms.Button) [및`ListView`](xref:Xamarin.Forms.ListView)와 같은 뷰 모델에서 뷰 유형을 참조 하지 마세요. 여기에 설명 된 원칙에 따라 뷰 모델을 격리 하 여 테스트할 수 있으므로 범위를 제한 하 여 소프트웨어 결함의 가능성을 줄일 수 있습니다.
...
가 만들어지면의 인스턴스가 자동으로 생성 [`BindingContext`](xref:Xamarin.Forms.BindableObject.BindingContext) 되고뷰의로설정됩니다.`LoginViewModel` [`ContentPage`](xref:Xamarin.Forms.ContentPage)
...
뷰 모델 로케이터는 뷰 모델의 인스턴스화 및 뷰에 대 한 연결을 관리 하는 사용자 지정 클래스입니다. EShopOnContainers 모바일 앱 `ViewModelLocator` 에서 클래스에는 뷰 모델을 뷰와 연결 하 `AutoWireViewModel`는 데 사용 되는 연결 된 속성인가 있습니다. 뷰의 XAML에서이 연결 된 속성은 다음 코드 예제와 같이 뷰 모델이 뷰에 자동으로 연결 되어야 함을 나타내기 위해 true로 설정 됩니다.

```xaml
viewModelBase:ViewModelLocator.AutoWireViewModel="true"
```

속성은 false로 초기화 되는 바인딩 가능한 속성 이며, 해당 값이 `OnAutoWireViewModelChanged` 변경 되 면 이벤트 처리기가 호출 됩니다. `AutoWireViewModel` 이 메서드는 뷰에 대 한 뷰 모델을 확인 합니다. 다음 코드 예제에서는이 작업을 수행 하는 방법을 보여 줍니다.
...
메서드 `OnAutoWireViewModelChanged` 는 규칙 기반 접근 방법을 사용 하 여 뷰 모델을 확인 하려고 합니다. 이 규칙은 다음을 가정 합니다.
...
마지막으로 메서드 `OnAutoWireViewModelChanged` 는 뷰 형식의 [`BindingContext`](xref:Xamarin.Forms.BindableObject.BindingContext) 를 확인 된 뷰 모델 형식으로 설정 합니다. 뷰 모델 유형을 확인 하는 방법에 대 한 자세한 내용은 [해결](~/xamarin-forms/enterprise-application-patterns/dependency-injection.md#resolution)을 참조 하세요.
...
뷰에 액세스할 수 있는 모든 뷰 모델 및 모델 클래스는 인터페이스를 `INotifyPropertyChanged` 구현 해야 합니다. 뷰 모델 또는 모델 클래스에서이 인터페이스를 구현 하면 내부 속성 값이 변경 될 때 클래스에서 뷰의 데이터 바인딩된 컨트롤에 변경 알림을 제공할 수 있습니다.

다음 요구 사항을 충족 하 여 적절 한 속성 변경 알림을 사용 하도록 앱을 설계 해야 합니다.

- 공용 속성의 `PropertyChanged` 값이 변경 되는 경우 항상 이벤트를 발생 시킵니다. XAML 바인딩이 발생 하는 방법 `PropertyChanged` 에 대 한 정보로 인해 이벤트 발생을 무시할 수 있다고 가정 하지 마세요.
- 모델 또는 모델 `PropertyChanged` 의 다른 속성에서 값을 사용 하는 계산 된 속성에 대해서는 항상 이벤트를 발생 시킵니다.
- 속성을 변경 `PropertyChanged` 하는 메서드의 끝에서 항상 이벤트를 발생 시킵니다. 또는 개체가 안전한 상태에 있는 것으로 알려져 있습니다. 이벤트를 발생 시키면 이벤트의 처리기를 동기적으로 호출 하 여 작업이 중단 됩니다. 작업 중간에 발생 하는 경우 개체를 노출 하 여 안전 하지 않은 부분적으로 업데이트 된 상태에 있을 때 함수를 콜백 할 수 있습니다. 또한 이벤트에 의해 `PropertyChanged` 연계 변경을 트리거할 수도 있습니다. 일반적으로 연계 변경을 수행 하려면 먼저 업데이트를 완료 해야 합니다.
- 속성이 변경 되지 `PropertyChanged` 않으면 이벤트를 발생 시 키 지 않습니다. 즉, 이벤트를 `PropertyChanged` 발생 시키기 전에 이전 값과 새 값을 비교 해야 합니다.
- 속성을 초기화 `PropertyChanged` 하는 경우 뷰 모델의 생성자 중에 이벤트를 발생 시 키 지 않습니다. 뷰의 데이터 바인딩된 컨트롤은이 시점에서 변경 알림을 받도록 구독 하지 않습니다.
- 클래스의 공용 메서드에 대 `PropertyChanged` 한 단일 동기 호출 내에서 동일한 속성 이름 인수를 사용 하 여 이벤트를 두 개 이상 발생 시 키 지 않습니다. 예를 들어 백업 저장소가 `NumberOfItems` `_numberOfItems` 필드인 속성이 지정 된 경우 루프를 실행 하는 동안 메서드가 `_numberOfItems` 50 번 증가 하면 속성의 `NumberOfItems` 속성 변경 알림만 한 번만 발생 해야 합니다. 모든 작업이 완료 된 후 비동기 메서드의 경우 비동기 연속 체인 `PropertyChanged` 의 각 동기 세그먼트에서 지정 된 속성 이름에 대 한 이벤트를 발생 시킵니다.

EShopOnContainers 모바일 앱은 다음 코드 `ExtendedBindableObject` 예제와 같이 클래스를 사용 하 여 변경 알림을 제공 합니다.
...
Xamarin.ios의 [`BindableObject`](xref:Xamarin.Forms.BindableObject) 클래스는 `INotifyPropertyChanged` 인터페이스를 구현 하 고 [메서드를제공합니다.`OnPropertyChanged`](xref:Xamarin.Forms.BindableObject.OnPropertyChanged(System.String)) 클래스 `ExtendedBindableObject` 는 속성 변경 `RaisePropertyChanged` 알림을 호출 하는 메서드를 제공 하며, 이렇게 하면 `BindableObject` 클래스에서 제공 하는 기능을 사용 합니다.

EShopOnContainers 모바일 앱의 각 뷰 모델 클래스는 클래스에서 파생 `ViewModelBase` 되며이 클래스는 `ExtendedBindableObject` 클래스에서 파생 됩니다. 따라서 각 뷰 모델 클래스는 `RaisePropertyChanged` `ExtendedBindableObject` 클래스의 메서드를 사용 하 여 속성 변경 알림을 제공 합니다. 다음 코드 예제에서는 eShopOnContainers mobile 앱에서 람다 식을 사용 하 여 속성 변경 알림을 호출 하는 방법을 보여 줍니다.
...
뷰 모델은 일반적으로 뷰의 바인딩에 대해 `ICommand` 인터페이스를 구현 하는 개체 인스턴스인 명령 속성을 노출 합니다. 많은 Xamarin Forms 컨트롤은 뷰 모델에서 제공 `Command` `ICommand` 하는 개체에 데이터 바인딩될 수 있는 속성을 제공 합니다. 인터페이스 `ICommand` 는 작업 자체 `Execute` 를 `CanExecute` 캡슐화 하는 메서드를 정의 하 고, 메서드를 호출할 수 있는지 여부를 나타내는 메서드를 정의 하 `CanExecuteChanged` 고, 다음에 영향을 주는 변경 내용이 발생할 때 발생 하는 이벤트를 정의 합니다. 명령을 실행 해야 합니다. Xamarin.ios [`Command`](xref:Xamarin.Forms.Command) 에서 [`Command<T>`](xref:Xamarin.Forms.Command) 제공 하는 및 클래스는 `ICommand` 인터페이스를 구현 합니다. 여기서 `T` 은 및 `CanExecute`에 대 `Execute` 한 인수의 형식입니다.

뷰 모델 내에는 유형의 뷰 모델 [`Command`](xref:Xamarin.Forms.Command) `ICommand`에 있는 각 공용 속성 [`Command<T>`](xref:Xamarin.Forms.Command) 에 대 한 또는 유형의 개체가 있어야 합니다. 또는 `Command` `ICommand.Execute` `Action` 생성자에는 메서드가 호출 될 때 호출 되는 콜백 개체가 필요 합니다. `Command<T>` 메서드 `CanExecute` 는 선택적 생성자 매개 변수 이며, `Func` 는을 반환 `bool`하는입니다.

다음 코드에서는 레지스터 명령을 나타내는 [`Command`](xref:Xamarin.Forms.Command) 인스턴스가 `Register` 뷰 모델 메서드에 대리자를 지정 하 여 생성 되는 방법을 보여 줍니다.

```csharp
public ICommand RegisterCommand => new Command(Register);
```

명령은에 대 한 참조 `ICommand`를 반환 하는 속성을 통해 뷰에 노출 됩니다. 메서드는 `Execute` [`Command`](xref:Xamarin.Forms.Command) 개체에 대해 호출 될 때 `Command` 생성자에 지정 된 대리자를 통해 뷰 모델의 메서드에 대 한 호출을 전달 하기만 합니다.

명령의 `async` `Execute` 대리자를 지정 하는 경우 및 `await` 키워드를 사용 하 여 명령에 의해 비동기 메서드를 호출할 수 있습니다. 이는 콜백이 `Task` 이 고 대기 해야 함을 나타냅니다. 예를 들어 다음 코드는 [`Command`](xref:Xamarin.Forms.Command) `SignInAsync` 뷰 모델 메서드에 대리자를 지정 하 여 로그인 명령을 나타내는 인스턴스가 생성 되는 방법을 보여 줍니다.

```csharp
public ICommand SignInCommand => new Command(async () => await SignInAsync());
```

클래스를 사용 `Execute` `CanExecute` 하 여 명령을 인스턴스화하여 매개 변수를 및 동작에 전달할 수 있습니다. [`Command<T>`](xref:Xamarin.Forms.Command) 예를 들어 다음 코드에서는 `Command<T>` `NavigateAsync` 메서드가 형식의 `string`인수를 요구 함을 나타내는 데 사용 되는 방법을 보여 줍니다.

```csharp
public ICommand NavigateCommand => new Command<string>(NavigateAsync);
```

및 클래스에서 각 생성자의 `CanExecute` 메서드에 대 한 대리자는 선택 사항입니다. [`Command<T>`](xref:Xamarin.Forms.Command) [`Command`](xref:Xamarin.Forms.Command) 대리자가 지정 `Command` 되지 않은 경우에 대해 `CanExecute`이 `true` 반환 됩니다. 그러나 뷰 모델은 `CanExecute` `Command` 개체에 대해 메서드를 `ChangeCanExecute` 호출 하 여 명령의 상태 변경을 나타낼 수 있습니다. 이로 인해 `CanExecuteChanged` 이벤트가 발생 합니다. 명령에 바인딩된 UI의 모든 컨트롤은 해당 사용 상태를 업데이트 하 여 데이터 바인딩된 명령의 가용성을 반영 합니다.

#### <a name="invoking-commands-from-a-view"></a>뷰에서 명령 호출

다음 코드 예제에서는 인스턴스를 [`Grid`](xref:Xamarin.Forms.Grid) `RegisterCommand` `LoginViewModel` `LoginView` [사용`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer) 하 여의가 클래스의에 바인딩하는 방법을 보여 줍니다.
...
명령 매개 변수는 속성을 [`CommandParameter`](xref:Xamarin.Forms.TapGestureRecognizer.CommandParameter) 사용 하 여 선택적으로 정의할 수도 있습니다. 필요한 인수의 형식은 `Execute` 및 `CanExecute` 대상 메서드에 지정 되어 있습니다. [`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer) 사용자가 연결 된 컨트롤과 상호 작용할 때가 대상 명령을 자동으로 호출 합니다. 명령 매개 변수 (제공 된 경우)는 명령 `Execute` 대리자에 인수로 전달 됩니다.
...
클래스 `BindableBehavior<T>` 는 동작의를 `OnAttachedTo` [`BindingContext`](xref:Xamarin.Forms.BindableObject.BindingContext) 설정 하는 재정의 가능한 메서드와를 `BindingContext`정리 하는 재정의 `OnDetachingFrom` 가능한 메서드를 제공 합니다. 또한 클래스는 연결된 컨트롤에 대한 참조를 `AssociatedObject` 속성에 저장합니다.

EShopOnContainers 모바일 앱에는 발생 `EventToCommandBehavior` 하는 이벤트에 대 한 응답으로 명령을 실행 하는 클래스가 포함 되어 있습니다. 이 클래스는 동작이 사용 `BindableBehavior<T>` 될 때 동작에서 `Command` 속성에 `ICommand` 지정 된를 바인딩하고 실행할 수 있도록 클래스에서 파생 됩니다. 다음 코드 예제는 `EventToCommandBehavior` 클래스를 보여줍니다.
...
및 메서드는 `EventName` 속성에 정의 된 이벤트에 대 한 이벤트 처리기를 등록 및 등록 취소 하는 데 사용 됩니다. `OnDetachingFrom` `OnAttachedTo` 그런 다음 이벤트가 발생 `OnFired` 하면 메서드를 호출 하 여 명령을 실행 합니다.

이벤트가 발생할 때 명령을 실행 `EventToCommandBehavior` 하는 데를 사용 하는 이점은 명령과 상호 작용 하도록 설계 되지 않은 컨트롤과 연결 될 수 있다는 것입니다. 또한 모델을 볼 수 있도록 이벤트 처리 코드를 이동 하 여 단위 테스트를 수행할 수 있습니다.

#### <a name="invoking-behaviors-from-a-view"></a>뷰에서 동작 호출

는 `EventToCommandBehavior` 명령을 지원 하지 않는 컨트롤에 명령을 연결 하는 데 특히 유용 합니다. 예를 들어, `ProfileView` 는 다음 `EventToCommandBehavior` 코드와 같이를 사용 [`ItemTapped`](xref:Xamarin.Forms.ListView.ItemTapped) 하 여 사용자의 [`ListView`](xref:Xamarin.Forms.ListView) 주문이 나열 된에서 이벤트가 발생 하는 `OrderDetailCommand` 경우를 실행 합니다.
...
런타임에는 `EventToCommandBehavior` [`ListView`](xref:Xamarin.Forms.ListView)와의 상호 작용에 응답 합니다. 에서 `ListView` `OrderDetailCommand` `ProfileViewModel`항목을 선택 하면 이벤트가 발생 하 고에서이 실행 됩니다. [`ItemTapped`](xref:Xamarin.Forms.ListView.ItemTapped) 기본적으로 이벤트에 대 한 이벤트 인수가 명령에 전달 됩니다. 이 데이터는 `EventArgsConverter` 속성에 지정 된 변환기에서 소스와 대상 사이에 전달 될 때 변환 `ListView` 되며에서 [`ItemTappedEventArgs`](xref:Xamarin.Forms.ItemTappedEventArgs)의를 [`Item`](xref:Xamarin.Forms.ItemTappedEventArgs.Item) 반환 합니다. 따라서 `OrderDetailCommand` 가 실행 되 면 선택한 `Order` 가 등록 된 작업에 매개 변수로 전달 됩니다.
