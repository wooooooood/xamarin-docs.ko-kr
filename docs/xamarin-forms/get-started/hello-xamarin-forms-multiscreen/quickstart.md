...
1. **새 항목 추가** 대화 상자에서 **Visual C# 항목 > Xamarin.Forms > 콘텐츠 페이지**를 선택하고, 새 파일의 이름을 **CallHistoryPage**로 지정하고, **추가** 단추를 클릭합니다. 그러면 **CallHistoryPage**란 이름의 페이지가 프로젝트에 추가됩니다.

  ![](quickstart-images/vs/add-callhistorypage-class.png "Xamarin.Forms 프로젝트 템플릿")

1. **CallHistoryPage.xaml**에서 템플릿 코드를 모두 제거하고 다음 코드로 바꿉니다. 이 코드는 페이지에 대한 사용자 인터페이스를 선언적으로 정의합니다.
...
1. **솔루션 탐색기**에서 **MainPage.xaml**을 두 번 클릭하여 엽니다.

  ![](quickstart-images/vs/open-mainpage-xaml.png "MainPage.xaml 열기")

1. **MainPage.xaml**에서 [`Button`](https://developer.xamarin.com/api/type/Xamarin.Forms.Button/) 컨트롤을 [`StackLayout`](https://developer.xamarin.com/api/type/Xamarin.Forms.StackLayout/) 컨트롤의 끝에 추가합니다. 단추는 호출 기록 페이지로 이동하기 위해 사용됩니다.
...
1. **솔루션 탐색기**에서 **MainPage.xaml.cs**을 두 번 클릭하여 엽니다.

  ![](quickstart-images/vs/open-mainpage-codebehind.png "MainPage.xaml.cs 열기")

1. **MainPage.xaml.cs**에서 `dialer` 변수가 `null`이 아닐 경우 `OnCallHistory` 이벤트 처리기 메서드를 추가하고, 변환된 전화번호를 `App.PhoneNumbers` 컬렉션에 추가하고 `callHistoryButton`을 활성화할 `OnCall` 이벤트 처리기 메서드를 수정합니다.
...
1. **솔루션 탐색기**에서 **Phoneword.Droid** 프로젝트를 마우스 오른쪽 단추로 클릭한 다음, **시작 프로젝트로 설정**을 선택합니다.
...
1. **새 파일** 대화 상자에서 **Forms > Forms ContentPage Xaml**을 선택하고, 새 파일에 **CallHistoryPage**라는 이름을 지정하고 **새로 만들기** 단추를 클릭합니다. 그러면 **CallHistoryPage**란 이름의 페이지가 프로젝트에 추가됩니다.

  ![](quickstart-images/xs/add-callhistorypage-class.png "Add Forms ContentPage")

1. **Solution Pad**에서 **CallHistoryPage.xaml**을 두 번 클릭하여 엽니다.
