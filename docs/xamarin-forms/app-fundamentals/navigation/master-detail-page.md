...
- 세부 정보 페이지를 설정 하 여 사용자에 게 표시 되는 [ `MasterDetailPage.IsPresented` ](xref:Xamarin.Forms.MasterDetailPage.IsPresented) 속성을 `false`입니다. 이 속성의 마스터 / 세부 정보 페이지 표시 되 고 있는지 여부를 제어 합니다. 마스터 페이지를 표시 하려면 `true` 로 설정하고, 세부 정보 페이지를 표시하려면 `false` 로 설정합니다.
