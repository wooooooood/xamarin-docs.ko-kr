...
> [!TIP]
> Android AppCompat의 경우 `TabbedPageRenderer`에는 사용자 지정된 `GetIconDrawable`에서 탭 아이콘을 로드하는 데 사용할 수 있는 재정의 가능한 `Drawable` 메서드가 포함됩니다. 이 재정의를 통해 `TabbedPage`에서 SVG 이미지를 아이콘으로 사용할 수 있고 위아래 탭 표시줄 모두에서 작업할 수 있습니다. 또는 위쪽 탭 표시줄의 경우 재정의 가능한 `SetTabIcon` 메서드를 사용하여 사용자 지정 `Drawable`에서 탭 아이콘을 로드할 수 있습니다.
...
- 탭 표시줄의 배경색인 [`BarBackgroundColor`](xref:Xamarin.Forms.TabbedPage.BarBackgroundColor) 형식의 [`Color`](xref:Xamarin.Forms.Color).
- 탭 표시줄의 텍스트 색인 [`BarTextColor`](xref:Xamarin.Forms.TabbedPage.BarTextColor) 형식의 [`Color`](xref:Xamarin.Forms.Color).
- 선택된 탭의 색인 [`SelectedTabColor`](xref:Xamarin.Forms.TabbedPage.SelectedTabColor) 형식의 [`Color`](xref:Xamarin.Forms.Color).
- 선택 취소된 탭의 색인 [`UnselectedTabColor`](xref:Xamarin.Forms.TabbedPage.UnselectedTabColor) 형식의 [`Color`](xref:Xamarin.Forms.Color).
...
> [!NOTE]
> [`Children`](xref:Xamarin.Forms.MultiPage`1.Children*)가 파생되는 [`MultiPage<T>`](xref:Xamarin.Forms.MultiPage`1) 클래스의 [`TabbedPage`](xref:Xamarin.Forms.TabbedPage) 속성은 `ContentProperty`의 `MultiPage<T>`입니다. 따라서 XAML에서 [ 속성에 `Page`](xref:Xamarin.Forms.Page)`Children` 개체를 명시적으로 할당할 필요가 없습니다.
...
이 예제에서 [`TabbedPage`](xref:Xamarin.Forms.TabbedPage)는 두 개의 [`Page`](xref:Xamarin.Forms.ContentPage) 개체로 채워집니다. 첫 번째 자식은 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 개체이고 두 번째 자식은 [ 개체를 포함하는 `NavigationPage`](xref:Xamarin.Forms.NavigationPage)`ContentPage`입니다.

다음 스크린샷에서는 [`ContentPage`](xref:Xamarin.Forms.ContentPage)의 [`TabbedPage`](xref:Xamarin.Forms.TabbedPage) 개체를 보여줍니다.
...
[`ContentPage`](xref:Xamarin.Forms.ContentPage) 개체가 [`NavigationPage`](xref:Xamarin.Forms.NavigationPage) 개체에 래핑된 경우 탭 내 탐색을 수행할 수 있습니다. 이렇게 하려면 [`PushAsync`](xref:Xamarin.Forms.NavigationPage.PushAsync*) 개체의 [`Navigation`](xref:Xamarin.Forms.NavigableElement.Navigation) 속성에서 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 메서드를 호출합니다.
...
[`TabbedPage`](xref:Xamarin.Forms.TabbedPage) 속성에 데이터 컬렉션을 할당하고 데이터를 [`ItemsSource`](xref:Xamarin.Forms.MultiPage`1.ItemsSource) 개체로 템플릿화하는 [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) 속성에 [`ItemTemplate`](xref:Xamarin.Forms.MultiPage`1.ItemTemplate)을 할당하여 [`Page`](xref:Xamarin.Forms.Page)를 페이지로 채울 수 있습니다. 이 작업은 다음과 같이 XAML에서 수행합니다.
...
이 예제에서 각 탭은 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 및 [`Image`](xref:Xamarin.Forms.Image) 개체를 사용하여 탭의 데이터를 표시하는 [`Label`](xref:Xamarin.Forms.Label) 개체로 구성됩니다.
